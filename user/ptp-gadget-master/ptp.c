...
#if 0 //#ifdef FORMAT_SUPPORT //must be disabled for write permission on Win 10
#define SUPPORTED_FORMATS                                       \
        __constant_cpu_to_le16(PIMA15740_FMT_A_UNDEFINED),      \
        __constant_cpu_to_le16(PIMA15740_FMT_A_TEXT),           \
        __constant_cpu_to_le16(PIMA15740_FMT_A_MPEG),           \
...
#define THUMB_EXT_NAME      "jpg"
//#define THUMB_ARTWORK_USED
#endif
...
static int cancel_ptp_request = 0, bulk_ep_shutdown = 0;
...
static int send_object_or_thumb(void *recv_buf, void *send_buf, size_t send_len, int thumb)
{
        ...
        if (!thumb) {
                ...
                *dot = '\0';                    /* We know there is a dot in the name */
                snprintf(name, sizeof(name) - 1, "%s." THUMB_EXT_NAME, obj->name);
                *dot = '.';
                name[sizeof(name) - 1] = '\0';
#ifdef THUMB_ARTWORK_USED
                if (__le16_to_cpu(obj->info.object_format) == PIMA15740_FMT_I_EXIF_JPEG) {
                        ret = chdir(root);
                } else
#endif
                {
                        ...
                        snprintf(subdir, sizeof(subdir), "%s" "/" THUMB_SUB_DIR,
                                 root);
                        ret = chdir(subdir);
                }
...
out:
        ...
        if (!ret) {
                /* Prepare response */
                if (total)
                        make_response(s_container, r_container, PIMA15740_RESP_INCOMPLETE_TRANSFER, sizeof(*s_container));
                else
                        make_response(s_container, r_container, PIMA15740_RESP_OK, sizeof(*s_container));
        }
...
static void delete_thumb(struct obj_list *obj)
{
        ...
        *dot = 0;
        snprintf(thumb, sizeof(thumb), "%s" "/" THUMB_SUB_DIR "/" "%s." THUMB_EXT_NAME,
                 root, obj->name);
        *dot = '.';
...
static int process_send_object_info(void *recv_buf, void *send_buf)
{
        ...
        if (verbose) {
                fprintf(stderr, "store_id 0x%lux, parent handle 0x%lux\n",
...
#ifdef THUMB_SUPPORT
static int generate_thumb(const char *, uint16_t);
#endif
static int process_send_object(void *recv_buf, void *send_buf)
{
...
#ifdef FORMAT_SUPPORT
        {
                ...
                dot = strrchr(object_info_p->name, '.');

                if (dot && strlen(dot) >= 3) {
                        switch (dot[1]) {
                        case 'j':
                        case 'J':
                                oi->info.object_format = __cpu_to_le16(PIMA15740_FMT_I_EXIF_JPEG);
                                break;
#if 0
                        case 'm':
                        case 'M':
                                oi->info.object_format = __cpu_to_le16(PIMA15740_FMT_A_MPEG);
                                break;
#endif
                        default:
                                break;
        ...
        if (oi->info.object_format != PIMA15740_FMT_A_UNDEFINED &&
            oi->info.object_format != PIMA15740_FMT_A_TEXT) {
                ret = generate_thumb(object_info_p->name, oi->info.object_format);
                if (ret > 0) {
...
_redo:
                ...
                if (ret < 0 && errno == ESHUTDOWN) {
                        bulk_ep_shutdown = 1;
                        fprintf(stderr, "Transport endpoint shutdown!\n");
                }
        ...
        case USB_REQ_PTP_CANCEL_REQUEST:
                {
                        ...
                        err = read(control, tmp, 6);
                        if (err != 6) // err always be equal to zero
                                fprintf(stderr, "USB_REQ_PTP_CANCEL_REQUEST %d\n", err);

                        cancel_ptp_request = 1;
                }
                return;
...
#ifdef THUMB_SUPPORT
        if ((format != PIMA15740_FMT_A_TEXT) && (format != PIMA15740_FMT_A_UNDEFINED)) {
                thumb_size = generate_thumb(filename, format);
                if (thumb_size < 0) {
#if 1 //chris add
                        format = PIMA15740_FMT_A_UNDEFINED;
#else //org
...
#ifdef THUMB_SUPPORT
static int generate_thumb(const char *file_name, uint16_t obj_format)
{
        ...
        *dot = '\0';
#ifdef THUMB_ARTWORK_USED
        if (__le16_to_cpu(obj_format) == PIMA15740_FMT_I_EXIF_JPEG)
                snprintf(thumb, sizeof(thumb), "%s" "/" "%s." THUMB_EXT_NAME,
                         root, file_name);
        else
#endif
                snprintf(thumb, sizeof(thumb), "%s" "/" THUMB_SUB_DIR "/" "%s." THUMB_EXT_NAME,
