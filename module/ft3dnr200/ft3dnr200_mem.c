...
{
    int drvMemCnt, EntityMemCnt;

    drvMemCnt = atomic_read(&priv.mem_cnt);
    EntityMemCnt = ft3dnr_get_entity_mem_cnt();

    seq_printf(sfile, "\nDRV mem count : %d\n", drvMemCnt);
    seq_printf(sfile, "ENTITY mem count : %d\n\n", EntityMemCnt);
...
int ft3dnr_mem_proc_init(struct proc_dir_entry *entity_proc)
{
    ...
    mem_cache_alloc_cntproc->proc_fops  = &mem_proc_param_ops;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
