...
#define VCAP_VG_RECORD_PROPERTY_MAX     50
...
#define VCAP_VG_ENTITY_FD(_type, _eng, _min)    ENTITY_FD(_type, 0, _eng, _min)
#else
#define VCAP_VG_ENTITY_FD(_type, _eng, _min)    ENTITY_FD(_type, _eng, _min)
#endif

#define VCAP_VG_FD_ENGINE(vimode, ch)           (((vimode&0x3)<<6)|(ch&0x3f))
#define VCAP_VG_FD_MINOR(s_ch, path)            (((s_ch&0x3f)<<2)|(path&0x3))
#define VCAP_VG_FD(vimode, ch, s_ch, path)      VCAP_VG_ENTITY_FD(TYPE_CAPTURE, VCAP_VG_FD_ENGINE(vimode, ch), VCAP_VG_FD_MINOR(s_ch, path))
...
struct vcap_vg_grp_pool_t {
    ...
    int               align_on;     ///< job alignment start
    int               align_cnt;    ///< job alignment trigger count
...
struct vcap_vg_job_item_t {
    ...
    struct vcap_vg_grp_pool_t   *grp_pool;      ///< group pool of multi-job, must record to root job
    VCAP_VG_JOB_STATUS_T        grp_status;     ///< group job global status, must record to root job
};
