 ...
 *
 * $Revision: 1.17 $
 * $Date: 2015/04/02 01:15:29 $
 *
 * ChangeLog:
 *  $Log: vcap_md.c,v $
 *  Revision 1.17  2015/04/02 01:15:29  jerson_l
...
static inline void vcap_md_gau_check(struct vcap_dev_info_t *pdev_info)
{
    ...
    u32 tmp, gau_m0, gau_m1, gau_l_m0, gau_l_m1;
    unsigned long flags;
    ...
    for(ch=0; ch<VCAP_CHANNEL_MAX; ch++) {
        ...
        if(pdev_info->ch[ch].do_md_gau_chk == 1) {    ///< clear gaussian value at first time
            ...
            /* get MB#0 gaussian value of first MB row */
            gau_m0 = ((u32 *)pdev_info->ch[ch].md_gau_buf.vaddr)[0];
            gau_m1 = ((u32 *)pdev_info->ch[ch].md_gau_buf.vaddr)[1];

            /* get MB#0 gaussian value of last MB row */
            if(pdev_info->ch[ch].param.md.x_num && (pdev_info->ch[ch].param.md.y_num > 1)) {
                gau_l_m0 = ((u32 *)pdev_info->ch[ch].md_gau_buf.vaddr)[(pdev_info->ch[ch].param.md.x_num*(pdev_info->ch[ch].param.md.y_num-1)*4)];
                gau_l_m1 = ((u32 *)pdev_info->ch[ch].md_gau_buf.vaddr)[(pdev_info->ch[ch].param.md.x_num*(pdev_info->ch[ch].param.md.y_num-1)*4)+1];
            }
            else {
                gau_l_m0 = gau_m0;
                gau_l_m1 = gau_m1;
            ...
            if(((gau_m0 != 0) && (gau_m1 == 0)) || ((gau_l_m0 == 0) && (gau_l_m1 != 0))) {      ///< md gaussian value incorrect, trigger md reset to recover md engine
