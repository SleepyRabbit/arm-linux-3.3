...
static struct video_property_t g_job_in_property[MAX_JOBS][MAX_PROPERTYS];
static struct video_property_t g_job_out_property[MAX_JOBS][MAX_PROPERTYS];
#endif
...
#ifdef CFG_VG_V2_SUPPORT
    /* clear job property */
    memset(g_job_in_property,  0, sizeof(struct video_property_t)*MAX_JOBS*MAX_PROPERTYS);
    memset(g_job_out_property, 0, sizeof(struct video_property_t)*MAX_JOBS*MAX_PROPERTYS);
#endif
        ...
        for(i=0; i<MAX_JOBS; i++) {
            if(g_job_cfg[i].enable) {
                for(j=0; j<g_job_cfg[i].round_set; j++) {
                    ...
                    job_cases[job_num].in_prop.max_p_num  = MAX_PROPERTYS;
                    job_cases[job_num].in_prop.p          = g_job_in_property[job_num];
                    job_cases[job_num].out_prop.max_p_num = MAX_PROPERTYS;
                    job_cases[job_num].out_prop.p         = g_job_out_property[job_num];
                    if((test_idx == 7) || (test_idx == 8)) {
                    ...
                    job_cases[job_num].in_mmap_size[0]  = g_buf_cfg.out_size+g_buf_cfg.out2_size;
                    job_cases[job_num].out_mmap_size[0] = g_buf_cfg.out_size+g_buf_cfg.out2_size;

                    build_property(job_cases[job_num].in_prop.p, g_job_cfg[i].param);
#else
                    job_cases[job_num].enable = 1;
                    if(g_buf_cfg.split_multi_job) {
                        job_cases[job_num].engine = (g_job_cfg[i].engine | VCAP_VG_FD_ENGINE_VIMODE_SPLIT_IDX);
...
#ifdef CFG_VG_V2_SUPPORT
                if(test_cases.job_cases[i].out_prop.p[j].id == 0)   // last property
                    break;

                param.type  = test_cases.type;
                param.id    = test_cases.job_cases[i].out_prop.p[j].id;
                param.value = test_cases.job_cases[i].out_prop.p[j].value;
#else
