 ...
 *
 * $Revision: 1.81 $
 * $Date: 2016/04/29 08:37:37 $
 *
 * ChangeLog:
 *  $Log: vcap_lib.c,v $
 *  Revision 1.81  2016/04/29 08:37:37  jerson_l
 *  1. correct lli update pool locker not init problem.
 *  2. correct memset to clear or set hardware register may not work problem.
 *
 *  Revision 1.80  2015/03/19 06:47:47  jerson_l
 *  1. correct md frame rate of md group channel.
 *
 *  Revision 1.79  2015/03/05 02:11:19  jerson_l
 *  1. correct sc rolling setting to support VI 4CH mux progressive format.
 *
 *  Revision 1.78  2015/01/26 11:47:42  jerson_l
 *  1. fix md last mb gaussion output zero problem of each mb row.
 *
...
#define VCAP_DEV_REG_SIZE   0x600
    ...
    for (i=0, j=0; i<(VCAP_DEV_REG_SIZE>>2); i++, j++) {
        ...
        mbuf_point += snprintf(mbuf+mbuf_point, VCAP_DEV_MSG_BUF_SIZE-mbuf_point, "  %08x", VCAP_REG_RD(pdev_info, (VCAP_VI_OFFSET(0) + (i<<2))));
    }
...
static void vcap_dev_dump_ch(struct vcap_dev_info_t *pdev_info, int ch, struct seq_file *sfile)
{
    ...
    mbuf_point += snprintf(mbuf+mbuf_point, VCAP_DEV_MSG_BUF_SIZE-mbuf_point, "\n=== CH#%d ===", ch);
    for (i=0, j=0; i<(VCAP_CH_REG_SIZE>>2); i++, j++) {
        if(mbuf_point>=VCAP_DEV_MSG_THRESHOLD){
            if(sfile)
                seq_printf(sfile, mbuf);
            else
        ...
        mbuf_point += snprintf(mbuf+mbuf_point, VCAP_DEV_MSG_BUF_SIZE-mbuf_point, "  %08x", VCAP_REG_RD(pdev_info, (ch_offset + (i<<2))));
    }
...
#define VCAP_DEV_SPLIT_REG_SIZE   (0x18*VCAP_SPLIT_CH_MAX)
    ...
    for (i=0, j=0; i<(VCAP_DEV_SPLIT_REG_SIZE>>2); i++, j++) {
        if(mbuf_point >= VCAP_DEV_MSG_THRESHOLD){
            if(sfile)
                seq_printf(sfile, mbuf);
            else
        ...
        mbuf_point += snprintf(mbuf+mbuf_point, VCAP_DEV_MSG_BUF_SIZE-mbuf_point, "  %08x", VCAP_REG_RD(pdev_info, (VCAP_SPLIT_CH_OFFSET(0, 0) + (i<<2))));
    }
...
static void vcap_dev_dump_global_cfg(struct vcap_dev_info_t *pdev_info, struct seq_file *sfile)
{
...
#endif
#ifdef PLAT_MD_LAST_MB_GAU_ZERO_CORRECT
    char *gau_zero_msg[] = {"Disable", "Skip_Pixel", "OSD_Delay"};
#endif
    ...
    mbuf_point+= snprintf(mbuf+mbuf_point, VCAP_DEV_MSG_BUF_SIZE-mbuf_point,
                          "MD_GAU_Zero  : %s\n",
                          (((pdev_info->m_param)->md_gau_zero >= VCAP_MD_GAU_ZERO_CORRECT_MAX) ? gau_zero_msg[0] : gau_zero_msg[(pdev_info->m_param)->md_gau_zero]));
#endif
...
static void vcap_dev_dump_channel_cfg(struct vcap_dev_info_t *pdev_info, int ch, struct seq_file *sfile)
{
    ...
    mbuf_point += snprintf(mbuf+mbuf_point, VCAP_DEV_MSG_BUF_SIZE-mbuf_point,
                           "Scaler_Sub_W : %-8d %-8d %-8d %-8d\n",
                           pcurr_param->sd[0].sub_out_w,
                           pcurr_param->sd[1].sub_out_w,
                           pcurr_param->sd[2].sub_out_w,
                           pcurr_param->sd[3].sub_out_w);
...
static int vcap_dev_proc_diag_global_show(struct seq_file *sfile, void *v)
{
    ...
    mbuf_point += snprintf(mbuf+mbuf_point, VCAP_DEV_MSG_BUF_SIZE-mbuf_point,
                           "\nMD read traffic jam         : %u", pdev_info->diag.global.md_rc_jam);
    mbuf_point += snprintf(mbuf+mbuf_point, VCAP_DEV_MSG_BUF_SIZE-mbuf_point,
...
void vcap_dev_set_reg_default(struct vcap_dev_info_t *pdev_info)
{
    ...
    /* SC */
    for(i=0; i<VCAP_SC_REG_SIZE; i+=4) {
        VCAP_REG_WR(pdev_info, VCAP_SC_OFFSET(i), 0);
    }

    /* Palette */
    for(i=0; i<VCAP_PALETTE_REG_SIZE; i+=4) {
        VCAP_REG_WR(pdev_info, VCAP_PALETTE_OFFSET(i), 0);
    }

    /* Mask all status and clear status */
    for(i=0; i<VCAP_STATUS_REG_SIZE; i+=4) {
        if(i == 0xE8 || i == 0xEC)  ///< skip ip_version and ip_info
            continue;
        VCAP_REG_WR(pdev_info, VCAP_STATUS_OFFSET(i), 0xffffffff);
    }

    /* Clear Channel Parameter memory */
    for(i=0; i<VCAP_CHANNEL_MAX; i++) {
        if(i == VCAP_CASCADE_CH_NUM) {
            for(j=0; j<VCAP_CH_REG_SIZE; j+=4) {
                VCAP_REG_WR(pdev_info, VCAP_CAS_SUBCH_OFFSET(j), 0);
            }
        }
        else {
            for(j=0; j<VCAP_CH_REG_SIZE; j+=4) {
                VCAP_REG_WR(pdev_info, VCAP_CH_SUBCH_OFFSET(i, j), 0);
            }
        }
    }
    ...
    for(i=0; i<VCAP_SPLIT_CH_MAX; i++) {
        for(j=0; j<VCAP_SPLIT_REG_SIZE; j+=4) {
            VCAP_REG_WR(pdev_info, VCAP_SPLIT_CH_OFFSET(i, j), 0);
        }
    }
#endif
}
...
int vcap_dev_vi_setup(struct vcap_dev_info_t *pdev_info, int vi_idx)
{
    ...
    for(i=0; i<VCAP_VI_CH_MAX; i++) {
        ...
        if(((pvi->prog == VCAP_VI_PROG_PROGRESSIVE) && ((pvi->tdm_mode == VCAP_VI_TDM_MODE_BYPASS) || (pvi->tdm_mode == VCAP_VI_TDM_MODE_FRAME_INTERLEAVE))) ||
    ...
    for(i=0; i<VCAP_VI_CH_MAX; i++) {
        ...
        if(pdev_info->ch[ch].active) {
            ...
            for(j=0; j<VCAP_SCALER_MAX; j++) {
                ...
                pcurr_param->sd[j].out_w     = pcurr_param->sd[j].sub_out_w = pcurr_param->sd[j].src_w;
...
int vcap_dev_param_setup(struct vcap_dev_info_t *pdev_info, int ch, int path, void *param)
{
    ...
    u16 md_x, md_y, md_x_size, md_y_size, md_x_num, md_y_num, md_src_w, md_src_h, md_valid_src_w;
    ...
    ptemp_param->sd[path].out_w     = ptemp_param->sd[path].sub_out_w = sd_w;
    ...
    if((pdev_info->m_param)->cap_md) {
        if(job_param->md_enb) {
...
#ifdef PLAT_OSD_TC_SWAP
            md_src_w  = md_valid_src_w = tc_w;   ///< MD image from TC output
            md_src_h  = tc_h;
#else
            md_src_w  = md_valid_src_w = sd_w;   ///< MD image from SD output
            md_src_h  = sd_h;
#endif
...
#ifdef PLAT_MD_LAST_MB_GAU_ZERO_CORRECT
            switch((pdev_info->m_param)->md_gau_zero) {
                case VCAP_MD_GAU_ZERO_CORRECT_SKIP_PIXEL:   ///< skip last 32 pixel of each line
                    md_valid_src_w = (md_src_w > VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY_PIXEL) ? (md_src_w - VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY_PIXEL) : md_src_w;
                    break;
                case VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY:
                    if((PLAT_MD_LAST_MB_GAU_ZERO_TYPE == 1) && (md_src_w >= VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY_W_MAX))
                        md_valid_src_w = (md_src_w > VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY_PIXEL) ? (md_src_w - VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY_PIXEL) : md_src_w;
                    break;
                default:
                    break;
             ...
             */
            while(md_x_num) {
                if((md_x + (md_x_size*md_x_num)) > md_valid_src_w)
                    md_x_num--;
                else
                    break;
            }
...
#ifdef PLAT_MD_LAST_MB_GAU_ZERO_CORRECT
            /*
             * Increase sub channel sd width to let osd engine delay each line done signal
             * for md engine to complete last mb gaussian transfer
             */
            if((pdev_info->m_param)->md_gau_zero == VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY) {
                if((PLAT_MD_LAST_MB_GAU_ZERO_TYPE != 1) ||
                   ((PLAT_MD_LAST_MB_GAU_ZERO_TYPE == 1) && (md_src_w < VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY_W_MAX))) {
                    if((scaler_hsize + VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY_PIXEL) <= VCAP_SCALER_HORIZ_SIZE) {
                        ptemp_param->sd[path].sub_out_w += VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY_PIXEL;
                        ptemp_param->sd[path].hsize     += VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY_PIXEL;
                    }
                }
            }
#endif
...
void vcap_dev_param_sync(struct vcap_dev_info_t *pdev_info, int ch, int path)
{
    if(pdev_info->ch[ch].param_sync) {
        ...
        if(pdev_info->ch[ch].param_sync & VCAP_PARAM_SYNC_COMM) {
            ...
            pcurr_param->comm.osd_en          = ptemp_param->comm.osd_en;
            pcurr_param->comm.mask_en         = ptemp_param->comm.mask_en;
            pcurr_param->comm.mark_en         = ptemp_param->comm.mark_en;
            pcurr_param->comm.border_en       = ptemp_param->comm.border_en;
            pcurr_param->comm.field_order     = ptemp_param->comm.field_order;
            pcurr_param->comm.osd_frm_mode    = ptemp_param->comm.osd_frm_mode;
            pcurr_param->comm.accs_win_en     = ptemp_param->comm.accs_win_en;
            pcurr_param->comm.accs_data_thres = ptemp_param->comm.accs_data_thres;
        }

        /* Common Flip and MD */
        if(pdev_info->ch[ch].param_sync & VCAP_PARAM_SYNC_COMM_FLIP_MD) {
