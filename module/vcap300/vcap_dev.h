...
#define VCAP_PARAM_SYNC_COMM            BIT0        ///< sync common setting without flip and md
...
#define VCAP_PARAM_SYNC_COMM_FLIP_MD    BIT18       ///< sync common flip and md setting
...
struct vcap_global_diag_t {
    ...
    u32                     md_rc_jam;          ///< MD global read traffic jam
...
typedef enum {
    VCAP_MD_GAU_ZERO_CORRECT_DISABLE = 0,   ///< Disable correction
    VCAP_MD_GAU_ZERO_CORRECT_SKIP_PIXEL,    ///< Enable  correction by skip last 32 pixel of each line
    VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY,     ///< Enable  correction by hardware osd delay 32 pixel. Because GM828x HSize problem if md resolution width over 1920, the rule will switch to skip pixel mode.
    VCAP_MD_GAU_ZERO_CORRECT_MAX
} VCAP_MD_GAU_ZERO_CORRECT_T;

#define VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY_PIXEL   32
#define VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY_W_MAX   1920
...
struct vcap_dev_module_param_t {
    ...
    VCAP_MD_GAU_ZERO_CORRECT_T  md_gau_zero;                    ///< correct mode for fix md last mb zero gaussian value
};
