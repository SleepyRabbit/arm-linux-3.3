/**
 * @file dh9920.c
 * VCAP300 DH9920 Input Device Driver
...
#define DEV_MAX                 DH9920_DEV_MAX
#define DEV_VPORT_MAX           DH9920_DEV_VOUT_MAX    ///< VOUT#0, VOUT#1, VOUT#2, VOUT#3
#define DEV_CH_MAX              DH9920_DEV_CH_MAX
...
static struct dh9920_dev_t    dh9920_dev[DEV_MAX];
static struct proc_dir_entry *dh9920_proc_root[DEV_MAX]    = {[0 ... (DEV_MAX - 1)] = NULL};
static struct proc_dir_entry *dh9920_proc_norm[DEV_MAX]    = {[0 ... (DEV_MAX - 1)] = NULL};
static struct proc_dir_entry *dh9920_proc_timeout[DEV_MAX] = {[0 ... (DEV_MAX - 1)] = NULL};

static int dh9920_module_get(void)
{
    return try_module_get(THIS_MODULE);
}

static void dh9920_module_put(void)
{
    module_put(THIS_MODULE);
}

static int dh9920_vfmt_notify_handler(int id, int ch, struct dh9920_video_fmt_t *vfmt)
{
    int vout, dev_ch;
    int norm_switch = 0;
    struct vcap_input_dev_t *pinput = NULL;
    struct dh9920_dev_t     *pdev = &dh9920_dev[id];
    ...
    vout = dh9920_get_vout_id(pdev->index, ch);
    ...
    if(pdev->vfmt[ch] != vfmt->fmt_idx) {
        ...
        if(pinput->mode == VCAP_INPUT_MODE_BYPASS) {
            ...
            if(vfmt->prog == 0) {   ///< interlace
                switch(dh9920_get_vout_format(pdev->index)) {
                    case DH9920_VOUT_FORMAT_BT656_DUAL_HEADER:
                        pinput->interface = VCAP_INPUT_INTF_SDI8BIT_INTERLACE;
                        norm_switch++;
                        break;
                    case DH9920_VOUT_FORMAT_BT1120:
                        pinput->interface = VCAP_INPUT_INTF_BT1120_INTERLACE;
                        norm_switch++;
                        break;
                    case DH9920_VOUT_FORMAT_BT656:
            ...
            dev_ch = dh9920_get_vout_seq_id(pdev->index, ch)*2;  ///< vout_seq_id would be 0 and 1, the input device channel use 0 and 2 for 2ch dual edge mode
...
static int dh9920_vlos_notify_handler(int id, int ch, int vlos)
{
    int vout, dev_ch;
    struct dh9920_dev_t *pdev = &dh9920_dev[id];
...
static int dh9920_device_init(int id, int norm)
{
    int i, j, ret = 0;
    struct vcap_input_dev_t *pinput;

    if((id >= DEV_MAX) || (id >= dh9920_get_device_num()))
        return -1;

    /* Update input device norm */
    for(i=0; i<DEV_VPORT_MAX; i++) {
        pinput = dh9920_dev[id].port[i];
        if(!pinput)
            continue;
        ...
        if(pinput->norm.mode != norm) {
            ...
            if(dh9920_get_vout_mode(id) == DH9920_VOUT_MODE_2CH_DUAL_EDGE_HALF_720P)
                pinput->norm.width = 960;
            pinput->norm.height = (pinput->mode == VCAP_INPUT_MODE_2CH_BYTE_INTERLEAVE_HYBRID) ? (DEFAULT_NORM_HEIGHT*2) : DEFAULT_NORM_HEIGHT;
            if(dh9920_get_vout_mode(id) == DH9920_VOUT_MODE_2CH_DUAL_EDGE_HALF_1080P)
                pinput->norm.height = 2160;
...
static int dh9920_proc_norm_show(struct seq_file *sfile, void *v)
{
    ...
    for(i=0; i<DEV_CH_MAX; i++) {
        vout = dh9920_get_vout_id(pdev->index, i);
        if((vout < DEV_VPORT_MAX) && pdev->port[vout]) {
            vout_ch = dh9920_get_vout_seq_id(pdev->index, i);
            if((pdev->port[vout])->mode == VCAP_INPUT_MODE_2CH_BYTE_INTERLEAVE_HYBRID)
            ...
            seq_printf(sfile, "%-5d %-5d %-7d %-7d\n",
                       i,
                       dh9920_get_vch_id_ext(pdev->index, vout, vout_ch),
...
static int dh9920_proc_timeout_show(struct seq_file *sfile, void *v)
{
    int i, vout, vout_ch, dev_ch;
    struct dh9920_dev_t *pdev = (struct dh9920_dev_t *)sfile->private;
...
static int dh9920_proc_norm_open(struct inode *inode, struct file *file)
{
    return single_open(file, dh9920_proc_norm_show, PDE(inode)->data);
}

static int dh9920_proc_timeout_open(struct inode *inode, struct file *file)
{
    return single_open(file, dh9920_proc_timeout_show, PDE(inode)->data);
}

static struct file_operations dh9920_proc_norm_ops = {
    .owner  = THIS_MODULE,
    .open   = dh9920_proc_norm_open,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations dh9920_proc_timeout_ops = {
    .owner  = THIS_MODULE,
    .open   = dh9920_proc_timeout_open,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};
...
static void dh9920_proc_remove(int id)
{
    if(dh9920_proc_root[id]) {
        if(dh9920_proc_norm[id])
            vcap_input_proc_remove_entry(dh9920_proc_root[id], dh9920_proc_norm[id]);

        if(dh9920_proc_timeout[id])
            vcap_input_proc_remove_entry(dh9920_proc_root[id], dh9920_proc_timeout[id]);

        vcap_input_proc_remove_entry(NULL, dh9920_proc_root[id]);
    }
}

static int dh9920_proc_init(struct dh9920_dev_t *pdev)
{
    int ret = 0;
    int id  = pdev->index;
    char name[32];
    ...
    /* root */
    sprintf(name, "dh9920.%d", id);
    dh9920_proc_root[id] = vcap_input_proc_create_entry(name, S_IFDIR|S_IRUGO|S_IXUGO, NULL);
    if(!dh9920_proc_root[id]) {
    ...
    /* norm */
    dh9920_proc_norm[id] = vcap_input_proc_create_entry("norm", S_IRUGO|S_IXUGO, dh9920_proc_root[id]);
    if(!dh9920_proc_norm[id]) {
        vcap_err("create proc node '%s/norm' failed!\n", name);
        ret = -EINVAL;
        goto err;
    }
    dh9920_proc_norm[id]->proc_fops = &dh9920_proc_norm_ops;
    dh9920_proc_norm[id]->data      = (void *)pdev;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    dh9920_proc_norm[id]->owner     = THIS_MODULE;
#endif

    /* timeout */
    dh9920_proc_timeout[id] = vcap_input_proc_create_entry("timeout", S_IRUGO|S_IXUGO, dh9920_proc_root[id]);
    if(!dh9920_proc_timeout[id]) {
        vcap_err("create proc node '%s/timeout' failed!\n", name);
        ret = -EINVAL;
        goto err;
    }
    dh9920_proc_timeout[id]->proc_fops = &dh9920_proc_timeout_ops;
    dh9920_proc_timeout[id]->data      = (void *)pdev;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    dh9920_proc_timeout[id]->owner     = THIS_MODULE;
#endif
...
static int __init dh9920_input_init(void)
{
    int i, j;
    int dev_num;
    struct vcap_input_dev_t *pinput;
    ...
    /* clear buffer */
    memset(dh9920_dev, 0, sizeof(struct dh9920_dev_t)*DEV_MAX);

    /* Get DH9920 Device Number */
    dev_num = dh9920_get_device_num();
        ...
        if(vport[i]) {
            ...
            for(j=0; j<DEV_VPORT_MAX; j++) {
                if((vport[i]>>(4*j)) & 0xf) {   ///< VPort# Connect to X_CAP#
                    pinput = dh9920_dev[i].port[j] = kzalloc(sizeof(struct vcap_input_dev_t), GFP_KERNEL);
                    if(pinput == NULL) {
                        vcap_err("DH9920#%d.%d allocate vcap_input_dev_t failed\n", i, j);
                        ret = -ENOMEM;
                        goto err;
                    }

                    /* input device name */
                    snprintf(pinput->name, VCAP_INPUT_NAME_SIZE-1, "dh9920.%d.%d", i, j);
                    ...
                    pinput->mode        = (dh9920_get_vout_mode(i) >= DH9920_VOUT_MODE_2CH_DUAL_EDGE) ? VCAP_INPUT_MODE_2CH_BYTE_INTERLEAVE_HYBRID : VCAP_INPUT_MODE_BYPASS;
                    ...
                    pinput->init        = dh9920_device_init;
                    pinput->norm.mode   = -1;                   ///< init value
                    pinput->module_get  = dh9920_module_get;
                    pinput->module_put  = dh9920_module_put;

                    /* interface & speed */
                    switch(dh9920_get_vout_format(i)) {
                        case DH9920_VOUT_FORMAT_BT656_DUAL_HEADER:
...
//                            pinput->probe_chid = VCAP_INPUT_PROBE_CHID_HBLANK;      ///< detect channel id from horizontal blanking
                            ...
                            pinput->ch_id[0]   = dh9920_get_vch_id_ext(i, j, 0);    ///< vout sequence id 0
                            pinput->ch_id[2]   = dh9920_get_vch_id_ext(i, j, 1);    ///< vout sequence id 1
                    ...
                    if(ret < 0) {
                        vcap_err("register DH9920#%d.%d input device failed\n", i, j);
                        goto err;
                    }
                }
            }
            ...
            /* device proc init */
            ret = dh9920_proc_init(&dh9920_dev[i]);
            if(ret < 0)
                goto err;

            /* device init */
            ret = dh9920_device_init(i, 0);
            if(ret < 0)
                goto err;

            /* register dh9920 video loss notify handler */
            dh9920_notify_vlos_register(i, dh9920_vlos_notify_handler);

            /* register dh9920 video foramt notify handler */
            dh9920_notify_vfmt_register(i, dh9920_vfmt_notify_handler);

            vcap_info("Register DH9920#%d Input Device\n", i);
        }
    }
...
err:
    for(i=0; i<DEV_MAX; i++) {
        dh9920_notify_vlos_deregister(i);
        dh9920_notify_vfmt_deregister(i);
        dh9920_proc_remove(i);
        for(j=0; j<DEV_VPORT_MAX; j++) {
            if(dh9920_dev[i].port[j]) {
                vcap_input_device_unregister(dh9920_dev[i].port[j]);
                kfree(dh9920_dev[i].port[j]);
            }
        }
    }
    return ret;
...
static void __exit dh9920_input_exit(void)
{
    int i, j;
...
module_init(dh9920_input_init);
module_exit(dh9920_input_exit);

MODULE_DESCRIPTION("Grain Media Video Capture300 DH9920 Input Driver");
