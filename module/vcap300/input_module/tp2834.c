/**
 * @file tp2834.c
 * VCAP300 TP2834 Input Device Driver
 * Copyright (C) 2015 GM Corp. (http://www.grain-media.com)
 *
 * $Revision: 1.1 $
 * $Date: 2015/11/26 07:26:58 $
 *
 * ChangeLog:
 *  $Log: tp2834.c,v $
 *  Revision 1.1  2015/11/26 07:26:58  shawn_hu
...
#define DEV_MAX                 TP2834_DEV_MAX
#define DEV_VPORT_MAX           TP2834_DEV_VOUT_MAX    ///< VD#1, VD#2, VD#3, VD#4
#define DEV_CH_MAX              TP2834_DEV_CH_MAX
...
static struct tp2834_dev_t    tp2834_dev[DEV_MAX];
static struct proc_dir_entry *tp2834_proc_root[DEV_MAX]    = {[0 ... (DEV_MAX - 1)] = NULL};
static struct proc_dir_entry *tp2834_proc_norm[DEV_MAX]    = {[0 ... (DEV_MAX - 1)] = NULL};
static struct proc_dir_entry *tp2834_proc_timeout[DEV_MAX] = {[0 ... (DEV_MAX - 1)] = NULL};

static int tp2834_module_get(void)
{
    return try_module_get(THIS_MODULE);
}

static void tp2834_module_put(void)
{
    module_put(THIS_MODULE);
}

static int tp2834_vfmt_notify_handler(int id, int ch, struct tp2834_video_fmt_t *vfmt)
{
    int vout, dev_ch;
    int norm_switch = 0;
    struct vcap_input_dev_t *pinput = NULL;
    struct tp2834_dev_t     *pdev   = &tp2834_dev[id];
    ...
    vout = tp2834_get_vout_id(pdev->index, ch);
    ...
    if(pdev->vfmt[ch] != vfmt->fmt_idx) {
        ...
        if(pinput->mode == VCAP_INPUT_MODE_BYPASS) {
            ...
            if(vfmt->prog == 0) {   ///< interlace
                switch(tp2834_get_vout_format(pdev->index)) {
                    case TP2834_VOUT_FORMAT_BT1120:
                        pinput->interface = VCAP_INPUT_INTF_BT1120_INTERLACE;
                        norm_switch++;
                        break;
                    case TP2834_VOUT_FORMAT_BT656:
            ...
            if((vfmt->fmt_idx == TP2834_VFMT_960H30) || (vfmt->fmt_idx == TP2834_VFMT_960H25)) {
            ...
            dev_ch = tp2834_get_vout_seq_id(pdev->index, ch)*2;  ///< vout_seq_id would be 0 and 1, the input device channel use 0 and 2 for 2ch dual edge mode
    ...
    if(pinput->mode != VCAP_INPUT_MODE_BYPASS) { ///< 2CH dual edge mode 
        ...
        dev_ch = tp2834_get_vout_seq_id(pdev->index, ch)*2;
        if((vfmt->fmt_idx == TP2834_VFMT_720P30) || (vfmt->fmt_idx == TP2834_VFMT_720P25) || ((pinput->ch_param[dev_ch==0?2:0].width == 1280))) {
        ...
        else if((vfmt->fmt_idx == TP2834_VFMT_960H30) || (vfmt->fmt_idx == TP2834_VFMT_960H25)) {
...
static int tp2834_vlos_notify_handler(int id, int ch, int vlos)
{
    int vout, dev_ch;
    struct tp2834_dev_t *pdev = &tp2834_dev[id];

    if((id >= DEV_MAX) || (ch >= TP2834_DEV_CH_MAX))
        return -1;
...
static int tp2834_device_init(int id, int norm)
{
    int i, j, ret = 0;
    struct vcap_input_dev_t *pinput;

    if((id >= DEV_MAX) || (id >= tp2834_get_device_num()))
        return -1;

    /* Update input device norm */
    for(i=0; i<DEV_VPORT_MAX; i++) {
        pinput = tp2834_dev[id].port[i];
        if(!pinput)
            continue;
...
static int tp2834_proc_norm_show(struct seq_file *sfile, void *v)
{
    ...
    for(i=0; i<DEV_CH_MAX; i++) {
        vout = tp2834_get_vout_id(pdev->index, i);
        if((vout < DEV_VPORT_MAX) && pdev->port[vout]) {
            vout_ch = tp2834_get_vout_seq_id(pdev->index, i);
            if((pdev->port[vout])->mode == VCAP_INPUT_MODE_2CH_BYTE_INTERLEAVE_HYBRID)
            ...
            seq_printf(sfile, "%-5d %-5d %-7d %-7d\n",
                       i,
                       tp2834_get_vch_id(pdev->index, vout, vout_ch),
...
static int tp2834_proc_timeout_show(struct seq_file *sfile, void *v)
{
    int i, vout, vout_ch, dev_ch;
    struct tp2834_dev_t *pdev = (struct tp2834_dev_t *)sfile->private;
...
static int tp2834_proc_norm_open(struct inode *inode, struct file *file)
{
    return single_open(file, tp2834_proc_norm_show, PDE(inode)->data);
}

static int tp2834_proc_timeout_open(struct inode *inode, struct file *file)
{
    return single_open(file, tp2834_proc_timeout_show, PDE(inode)->data);
}

static struct file_operations tp2834_proc_norm_ops = {
    .owner  = THIS_MODULE,
    .open   = tp2834_proc_norm_open,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations tp2834_proc_timeout_ops = {
    .owner  = THIS_MODULE,
    .open   = tp2834_proc_timeout_open,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};
...
static void tp2834_proc_remove(int id)
{
    if(tp2834_proc_root[id]) {
        if(tp2834_proc_norm[id])
            vcap_input_proc_remove_entry(tp2834_proc_root[id], tp2834_proc_norm[id]);

        if(tp2834_proc_timeout[id])
            vcap_input_proc_remove_entry(tp2834_proc_root[id], tp2834_proc_timeout[id]);

        vcap_input_proc_remove_entry(NULL, tp2834_proc_root[id]);
    }
}

static int tp2834_proc_init(struct tp2834_dev_t *pdev)
{
    int ret = 0;
    int id  = pdev->index;
    char name[32];
    ...
    /* root */
    sprintf(name, "tp2834.%d", id);
    tp2834_proc_root[id] = vcap_input_proc_create_entry(name, S_IFDIR|S_IRUGO|S_IXUGO, NULL);
    if(!tp2834_proc_root[id]) {
    ...
    /* norm */
    tp2834_proc_norm[id] = vcap_input_proc_create_entry("norm", S_IRUGO|S_IXUGO, tp2834_proc_root[id]);
    if(!tp2834_proc_norm[id]) {
        vcap_err("create proc node '%s/norm' failed!\n", name);
        ret = -EINVAL;
        goto err;
    }
    tp2834_proc_norm[id]->proc_fops = &tp2834_proc_norm_ops;
    tp2834_proc_norm[id]->data      = (void *)pdev;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    tp2834_proc_norm[id]->owner     = THIS_MODULE;
#endif

    /* timeout */
    tp2834_proc_timeout[id] = vcap_input_proc_create_entry("timeout", S_IRUGO|S_IXUGO, tp2834_proc_root[id]);
    if(!tp2834_proc_timeout[id]) {
        vcap_err("create proc node '%s/timeout' failed!\n", name);
        ret = -EINVAL;
        goto err;
    }
    tp2834_proc_timeout[id]->proc_fops = &tp2834_proc_timeout_ops;
    tp2834_proc_timeout[id]->data      = (void *)pdev;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    tp2834_proc_timeout[id]->owner     = THIS_MODULE;
#endif
...
static int __init tp2834_input_init(void)
{
    int i, j;
    int dev_num;
    struct vcap_input_dev_t *pinput;
    ...
    /* clear buffer */
    memset(tp2834_dev, 0, sizeof(struct tp2834_dev_t)*DEV_MAX);

    /* Get Device Number */
    dev_num = tp2834_get_device_num();
        ...
        if(vport[i]) {
            ...
            for(j=0; j<DEV_VPORT_MAX; j++) {
                if((vport[i]>>(4*j)) & 0xf) {   ///< VPort# Connect to X_CAP#
                    pinput = tp2834_dev[i].port[j] = kzalloc(sizeof(struct vcap_input_dev_t), GFP_KERNEL);
                    if(pinput == NULL) {
                        vcap_err("tp2834#%d.%d allocate vcap_input_dev_t failed\n", i, j);
                        ret = -ENOMEM;
                        goto err;
                    }

                    /* input device name */
                    snprintf(pinput->name, VCAP_INPUT_NAME_SIZE-1, "tp2834.%d.%d", i, j);
                    ...
                    pinput->mode        = (tp2834_get_vout_mode(i) == TP2834_VOUT_MODE_2CH_DUAL_EDGE) ? VCAP_INPUT_MODE_2CH_BYTE_INTERLEAVE_HYBRID : VCAP_INPUT_MODE_BYPASS;
                    pinput->yc_swap     = (yc_swap[i]>>(4*j)) & 0x1;
                    pinput->data_swap   = data_swap[i];
                    pinput->ch_id_mode  = 0;
                    pinput->inv_clk     = (inv_clk[i]>>(4*j)) & 0x1;
                    pinput->init        = tp2834_device_init;
                    pinput->module_get  = tp2834_module_get;
                    pinput->module_put  = tp2834_module_put;
                    pinput->norm.mode   = -1;                            ///< init value

                    /* interface */
                    switch(tp2834_get_vout_format(i)) {
                        case TP2834_VOUT_FORMAT_BT1120:
                            pinput->interface = VCAP_INPUT_INTF_BT1120_PROGRESSIVE;
                            break;
                        case TP2834_VOUT_FORMAT_BT656:
                    ...
                    switch(pinput->mode) {
                        case VCAP_INPUT_MODE_2CH_BYTE_INTERLEAVE_HYBRID:
                            ...
                            pinput->ch_id[0]   = tp2834_get_vch_id(i, j, 0);    ///< vout sequence id 0
                            pinput->ch_id[2]   = tp2834_get_vch_id(i, j, 1);    ///< vout sequence id 1
                    ...
                    if(ret < 0) {
                        vcap_err("register tp2834#%d.%d input device failed\n", i, j);
                        goto err;
                    }
                }
            }
            ...
            /* device proc init */
            ret = tp2834_proc_init(&tp2834_dev[i]);
            if(ret < 0)
                goto err;

            /* device init */
            ret = tp2834_device_init(i, 0);
            if(ret < 0)
                goto err;

            /* register tp2834 video loss notify handler */
            tp2834_notify_vlos_register(i, tp2834_vlos_notify_handler);

            /* register tp2834 video foramt notify handler */
            tp2834_notify_vfmt_register(i, tp2834_vfmt_notify_handler);

            vcap_info("Register TP2834#%d Input Device\n", i);
        }
    }
...
err:
    for(i=0; i<DEV_MAX; i++) {
        tp2834_notify_vlos_deregister(i);
        tp2834_notify_vfmt_deregister(i);
        tp2834_proc_remove(i);
        for(j=0; j<DEV_VPORT_MAX; j++) {
            if(tp2834_dev[i].port[j]) {
                vcap_input_device_unregister(tp2834_dev[i].port[j]);
                kfree(tp2834_dev[i].port[j]);
            }
        }
    }
    return ret;
...
static void __exit tp2834_input_exit(void)
{
    int i, j;
...
module_init(tp2834_input_init);
module_exit(tp2834_input_exit);

MODULE_DESCRIPTION("Grain Media Video Capture300 TP2834 Input Driver");
