 ...
 *
 * $Revision: 1.4 $
 * $Date: 2015/10/14 02:15:03 $
 *
 * ChangeLog:
 *  $Log: nvp6124.c,v $
 *  Revision 1.4  2015/10/14 02:15:03  shawn_hu
 *  Rearrange NVP6124 output channel sequence (Reg@0xC0~C7) to fix the problem of a straight line shown on the video.
 *
 *  Revision 1.3  2015/09/10 06:49:22  shawn_hu
 *  Fix the "show" function of "norm" & "timeout" proc node in input module.
 *
 *  Revision 1.2  2015/03/24 07:17:54  shawn_hu
...
static int nvp6124_proc_norm_show(struct seq_file *sfile, void *v)
{
    ...
    seq_printf(sfile, "--------------------------\n");
    for(i=0; i<NVP6124_DEV_VPORT_MAX; i++) {
        if(pdev->port[i]) {
            for(j=0; j<vport_chnum; j++) {
                seq_printf(sfile, "%-5d %-5d %-7d %-7d\n",
                           (i*vport_chnum+j),
                           nvp6124_get_vch_id(pdev->index, i, j),
                ...
                if((vport[i]>>(4*j)) & 0xf) {   ///< VPort# Connect to X_CAP#
                    ...
                    switch(mode[i]) {
                        ...
                        default: /* 144MHz ==> 4CH */
                            ...
                            pinput->ch_id[0] = nvp6124_get_vch_id(i, j, 0);
                            pinput->ch_id[1] = nvp6124_get_vch_id(i, j, 1);
                            pinput->ch_id[2] = nvp6124_get_vch_id(i, j, 2);
                            pinput->ch_id[3] = nvp6124_get_vch_id(i, j, 3);
                            break;
                    }
