 ...
 * ChangeLog:
 *  $Log: vcap_lli.c,v $
 *  Revision 1.66  2016/04/29 08:37:37  jerson_l
 *  1. correct lli update pool locker not init problem.
 *  2. correct memset to clear or set hardware register may not work problem.
 *
 *  Revision 1.65  2016/01/11 06:33:20  jerson_l
 *  1. fix BT656 2CH DaulEdge mode dynamic CHID probe incorrect problem.
 *
 *  Revision 1.64  2015/04/02 01:15:29  jerson_l
 *  1. fix md read traffic jam and DMA#0 overflow to cause channel md freeze problem.
 *
 *  Revision 1.63  2015/03/19 06:47:47  jerson_l
 *  1. correct md frame rate of md group channel.
 *
 *  Revision 1.62  2015/01/26 11:47:42  jerson_l
...
static inline int vcap_lli_prepare_update_reg(struct vcap_dev_info_t *pdev_info, int ch, int path)
{
    ...
    /* SD */
    if((pcurr_param->sd[path].sub_out_w != ptemp_param->sd[path].sub_out_w) ||
       (pcurr_param->sd[path].out_h     != ptemp_param->sd[path].out_h)) {
        ...
        reg_data.mask   = 0xf;
        reg_data.value  = ptemp_param->sd[path].sub_out_w | (ptemp_param->sd[path].out_h<<16);
    ...
    /* Common Filp and MD */
    if((pcurr_param->comm.h_flip != ptemp_param->comm.h_flip) ||
       ...
       (pcurr_param->comm.md_src != ptemp_param->comm.md_src)) {
        ...
        reg_data.mask   = 0x8;
        reg_data.value  = (ptemp_param->comm.md_src<<24) |
        ...
        param_sync |= VCAP_PARAM_SYNC_COMM_FLIP_MD;
    }
...
static inline int vcap_lli_wrap_dummy(struct vcap_dev_info_t *pdev_info, int ch, int path, int loop)
{
    ...
    if((pch->temp_param.sd[path].out_w != pch->temp_param.tc[path].width) ||
       ...
       (pch->temp_param.sd[path].out_w != pch->temp_param.sd[path].sub_out_w) ||
...
static inline int vcap_lli_wrap_md_dummy(struct vcap_dev_info_t *pdev_info, int ch, int path, int md_enb)
{
    ...
    if((pcurr_param->sd[path].out_w != pcurr_param->tc[path].width)     ||
       ...
       (pcurr_param->sd[path].out_w != pcurr_param->sd[path].sub_out_w) ||
...
static inline int vcap_lli_wrap_hblank_command(struct vcap_dev_info_t *pdev_info, int ch, int path)
{
    ...
    if(sd_w != pcurr_param->sd[path].sub_out_w || sd_h != pcurr_param->sd[path].out_h) {
    ...
    if(sync_sd_param) {
        pcurr_param->sd[path].sub_out_w = ptemp_param->sd[path].sub_out_w = sd_w;
    ...
    for(num=0; num<job_param->grab_num; num++) {
        ...
        if(num == 0) {
            ...
            if((pch->param.md.x_num != pch->temp_param.md.x_num) || (pch->param.md.y_num != pch->temp_param.md.y_num)) {
                if(pdev_info->ch[ch].do_md_gau_chk == 0)
...
static int vcap_lli_pool_create(struct vcap_dev_info_t *pdev_info)
{
    ...
    /* clear LL memory */
    for(i=0; i<VCAP_LLI_MEM_SIZE; i+=4) {
        VCAP_REG_WR(pdev_info, VCAP_LLI_MEM_OFFSET(i), 0);
    }
    ...
    /* update pool locker init */
    spin_lock_init(&plli->update_pool.lock);
...
static inline void vcap_lli_detect_vi_hblank_chid(struct vcap_dev_info_t *pdev_info)
{
    ...
    for(i=0; i<VCAP_VI_MAX; i++) {
        ...
        if(pdev_info->vi[i].grab_mode == VCAP_VI_GRAB_MODE_HBLANK) {
            ...
            if(pdev_info->vi[i].format == VCAP_VI_FMT_BT656) {
                if((((volatile u8 *)(pdev_info->ch[ch].hblank_buf.vaddr))[0]) != 0xff) {                ///< bt656 header not macth
                    continue;
                }
            }
...
update_cnt:
    ...
    if(check_reg_55ac || (reg_0x55ac & 0x0000003e)) {
        ...
        /* global md read traffic jam */
        if(reg_0x55ac & BIT2) {
            if((pdev_info->dbg_mode >= 2) && printk_ratelimit())
                vcap_err("[%d] MD read traffic jam!\n", pdev_info->index);
            pdev_info->diag.global.md_rc_jam++;

            /* channel md read traffic jam */
            for(ch=0; ch<VCAP_CHANNEL_MAX; ch++) {
                if(!pdev_info->ch[ch].active)
                    continue;
                bitmask = (ch == VCAP_CASCADE_CH_NUM) ? BIT31 : (0x1<<ch);  ///< cascade channel use bit#31 as status
                if(reg_0x5568 & bitmask) {
                    if(pdev_info->dbg_mode && printk_ratelimit())
                        vcap_err("[%d] ch#%d MD read traffic jam!\n", pdev_info->index, ch);
                    pdev_info->diag.ch[ch].md_traffic_jam++;
#ifdef VCAP_MD_GAU_VALUE_CHECK
                    /* md read traffic jam may cause channel md freeze, must to trigger md gaussian value check */
                    if(pdev_info->ch[ch].do_md_gau_chk == 0)
                        pdev_info->ch[ch].do_md_gau_chk = 1;
#endif
                }
