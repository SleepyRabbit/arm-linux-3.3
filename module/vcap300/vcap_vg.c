 ...
 *
 * $Revision: 1.56 $
 * $Date: 2016/08/10 10:47:58 $
 *
 * ChangeLog:
 *  $Log: vcap_vg.c,v $
 *  Revision 1.56  2016/08/10 10:47:58  jerson_l
 *  1. support gm_grpah_v2 with wrapper
 *
 *  Revision 1.55  2015/07/29 05:46:44  jerson_l
 *  1. add vg layer group job auto alignment procedure.
 *
 *  Revision 1.54  2015/03/10 12:27:58  jerson_l
 *  1. display job timestamp info to vg log message.
 *
 *  Revision 1.53  2015/03/10 07:34:00  jerson_l
 *  1. correct job status display string of panic job dump api.
 *
...
struct vcap_vg_property_rec_t {
    int                     job_id;
    struct video_property_t property[VCAP_VG_RECORD_PROPERTY_MAX];
};
...
struct vcap_vg_info_t {
    ...
    struct tasklet_struct           callback_tasklet;       ///< job callback tasklet
...
static int vcap_vg_grp_run_ver_thres   = 0;                     ///< group job old version job count threshold
static int vcap_vg_grp_job_align       = 0;                     ///< group job auto alignment, 0:disable 1:enable
static int vcap_vg_grp_job_align_thres = 2;                     ///< group job alignment threshold, default 2 jobs
...
static inline int is_print(int engine, int minor)
{
    ...
    if (vcap_vg_log_inc_filter[0] >= 0) {
        for (i=0; i<VCAP_VG_LOG_MAX_FILTER; i++)
            if (vcap_vg_log_inc_filter[i] == VCAP_VG_ENTITY_FD(0, engine, minor))
                return 1;
    }

    if (vcap_vg_log_exc_filter[0] >= 0) {
        for (i=0; i<VCAP_VG_LOG_MAX_FILTER; i++)
            if (vcap_vg_log_exc_filter[i] == VCAP_VG_ENTITY_FD(0, engine, minor))
                return 0;
    }
    return 1;
}
...
static int vcap_vg_group_pool_get_run_job_cnt(struct vcap_vg_grp_pool_t *grp_pool)
{
    int job_cnt = 0;
    struct vcap_vg_job_item_t *curr_job, *next_job;

    list_for_each_entry_safe(curr_job, next_job, &grp_pool->job_head, group_list) {
        if(curr_job->grp_status != VCAP_VG_JOB_STATUS_KEEP)
            job_cnt++;
        else
            break;
    }
...
static int vcap_vg_group_pool_get_job_offset(struct vcap_vg_grp_pool_t *grp_pool, int job_id)
{
    int offset = 0;
    struct vcap_vg_job_item_t *curr_job, *next_job;
...
static void vcap_vg_group_pool_job_check(struct vcap_vg_info_t *pvg_info)
{
    ...
    list_for_each_entry_safe(curr_pool, next_pool, &pvg_info->grp_pool_head, pool_list) {
        ...
        /* check job alignment */
        if(vcap_vg_grp_job_align && curr_pool->align_on) {
            if(vcap_vg_group_pool_get_run_job_cnt(curr_pool) > vcap_vg_grp_job_align_thres)
                continue;
            else
                goto chk_job;
        }
...
chk_job:
                ...
                if(curr_job->grp_status == VCAP_VG_JOB_STATUS_KEEP) {
                    if(list_empty(&curr_job->m_job_head)) {
                        ...
                        if(vcap_vg_log_level)
                            vcap_log("job#%d release pending, ver:%d engine:%d minor:%d root#%d\n", curr_job->job_id, curr_job->job_ver, curr_job->engine, curr_job->minor, curr_job->job_id);
                    }
                    else {
                        list_for_each_entry_safe(job_item, job_item_next, &curr_job->m_job_head, m_job_list) {
                            ...
                            if(vcap_vg_log_level)
                                vcap_log("job#%d release pending, ver:%d engine:%d minor:%d root#%d\n", job_item->job_id, job_item->job_ver, job_item->engine, job_item->minor, curr_job->job_id);
                        }
                    }
                    curr_job->grp_status = VCAP_VG_JOB_STATUS_STANDBY;  ///< only change root job group status
                }
        ...
        /* clear job alignment check */
        if(curr_pool->align_on)
            curr_pool->align_on = 0;
    }
            ...
            for(minor=0; minor<pvg_info->entity.minors; minor++) {
                if((disp_path == minor) || (disp_path >= pvg_info->entity.minors)) {
                    ...
                    do {
                        ...
                        i++;
                    } while(i < VCAP_VG_RECORD_PROPERTY_MAX);
                    seq_printf(sfile, "=============================================================\n");
                }
            }
        }
...
static int vcap_vg_proc_group_info_show(struct seq_file *sfile, void *v)
{
    ...
    seq_printf(sfile, "Group Job Alignmnet          : %s\n", vcap_vg_grp_job_align ? "Enable" : "Disable");
    seq_printf(sfile, "Group Job Alignment Threshold: %d\n", vcap_vg_grp_job_align_thres);
    seq_printf(sfile, "Group Pool Count             : %d\n", pvg_info->grp_pool_cnt);
    seq_printf(sfile, "=====================================================================\n");

    list_for_each_entry_safe(curr_pool, next_pool, &pvg_info->grp_pool_head, pool_list) {
        seq_printf(sfile, "[Group#%d-Ver:%d, Align_On:%d Align_Cnt:%d]\n", curr_pool->id, curr_pool->ver, curr_pool->align_on, curr_pool->align_cnt);
        job_cnt = 0;
        list_for_each_entry_safe(curr_job, next_job, &curr_pool->job_head, group_list) {
            job_cnt++;
            seq_printf(sfile, "%d[V:%d,%s]%s",
                       curr_job->job_id,
                       curr_job->job_ver,
                       ((curr_job->grp_status > 0 && curr_job->grp_status <= VCAP_VG_JOB_STATUS_KEEP) ? job_sts_str[curr_job->grp_status] : job_sts_str[0]),
                       ((job_cnt%4 == 0) ? "\n" : " "));
        }
        seq_printf(sfile, "%s---------------------------------------------------------------------\n", (((job_cnt%4) != 0) ? "\n" : ""));
    }
    ...
    seq_printf(sfile, "mode  => 0: normal\n");
    seq_printf(sfile, "         1: wait old version done\n");
    seq_printf(sfile, "echo [align] [thres] to setup group job alignment and threshold\n");
    seq_printf(sfile, "align => 8: job alignment disable\n");
    seq_printf(sfile, "         9: job alignment enable\n");
    ...
    if(mode == 0) { //exclude
        for(i=0; i<VCAP_VG_LOG_MAX_FILTER; i++) {
            if (vcap_vg_log_exc_filter[i] == -1) {
                vcap_vg_log_exc_filter[i] = VCAP_VG_ENTITY_FD(0, engine, minor);
                break;
            }
        }
    }
        ...
        for(i=0; i<VCAP_VG_LOG_MAX_FILTER; i++) {
            if(vcap_vg_log_inc_filter[i] == -1) {
                vcap_vg_log_inc_filter[i] = VCAP_VG_ENTITY_FD(0, engine, minor);
                break;
            }
        }
    }
...
static ssize_t vcap_vg_proc_group_info_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if(run_mode == 8 || run_mode == 9) {    ///< group job alignment control
        run_mode -= 8;
        if(vcap_vg_grp_job_align != run_mode) {
            pool_check++;       ///< group pool job status recheck

            vcap_vg_grp_job_align = run_mode;
        }

        if(vcap_vg_grp_job_align_thres != ver_thres) {
            if(vcap_vg_grp_job_align) {
                pool_check++;   ///< group pool job status recheck

                vcap_vg_grp_job_align_thres = ver_thres;
            }
        }
    }
    else {
...
static inline void *vcap_vg_alloc_subjob(struct vcap_vg_job_item_t *job_item, int job_fmt)
{
    ...
    job_item2->grp_pool      = job_item->grp_pool;
    job_item2->grp_status    = job_item->grp_status;
...
static int vcap_vg_putjob(void *param)
{
    ...
    if(vcap_vg_grp_job_align || (vcap_vg_grp_run_mode == VCAP_VG_GRP_RUN_MODE_WAIT_OLD_VER_DONE)) {
    ...
    do {
        ...
        job_item->grp_pool   = grp_pool;

        /* set job status */
        if((grp_pool) &&
           ((grp_pool->align_on) ||
            ((vcap_vg_grp_run_mode == VCAP_VG_GRP_RUN_MODE_WAIT_OLD_VER_DONE) &&
             (grp_pool->ver != job->ver)                                      &&
             (vcap_vg_group_pool_get_run_ver_cnt(grp_pool) > vcap_vg_grp_run_ver_thres)))) {
            /*
             * 1. group job alignment active, force to pending job
             * 2. group job version  change, check not finish old version job count to set job status
             */
            job_item->status = job_item->grp_status = VCAP_VG_JOB_STATUS_KEEP;
        }
        else {
            job_item->status = job_item->grp_status = VCAP_VG_JOB_STATUS_STANDBY;
        }
...
#ifdef CFG_VG_V2_SUPPORT
static int vcap_vg_stop(unsigned int fd)
{
    unsigned long flags;
    int idx, ret = 0;
    int ch, path;
    ...
    if(ENTITY_FD_TYPE(fd) != TYPE_CAPTURE) {
        VCAP_VG_LOG(VCAP_VG_LOG_ERROR, engine, minor, "invalid entity type (fd:0x%08x)\n", fd);
        ret = -1;
        goto end;
    }

    pvg_info = vcap_vg_info[0];
    if(pvg_info == NULL) {
...
static int vcap_vg_getproperty(void *param, int engine, int minor, char *string)
{
    ...
    for(i=0; i<VCAP_VG_RECORD_PROPERTY_MAX; i++) {
            ...
            list_for_each_entry_safe(job_item, job_item_next, &pvg_info->minor_head[idx], minor_list) {
                ...
                if(job_item->status == VCAP_VG_JOB_STATUS_FINISH) {
                    ...
                    if(job_item->type == VCAP_VG_JOB_TYPE_MULTI) {
                        if(vcap_vg_grp_job_align && job_item->grp_pool && ((job_item->grp_pool)->align_on == 0)) {
                            job_offset = vcap_vg_group_pool_get_job_offset(job_item->grp_pool, root_job_item->job_id);
                            if(job_offset > vcap_vg_grp_job_align_thres) {
                                (job_item->grp_pool)->align_on = 1;
                                (job_item->grp_pool)->align_cnt++;
                                VCAP_VG_LOG(VCAP_VG_LOG_QUIET, root_job_item->engine, root_job_item->minor,
                                            "group#%d pool(ver:%d) job#%d(r:%d) trigger job alignment(offset:%d)\n",
                                            (job_item->grp_pool)->id,
                                            (job_item->grp_pool)->ver,
                                            job_item->job_id,
                        ...
                        if(list_empty(&root_job_item->m_job_head)) {    ///< all job done of multi-job
                            if(root_job_item->need_callback) {
                                ...
                                job->callback(job);
                                VCAP_VG_LOG(VCAP_VG_LOG_QUIET, root_job_item->engine, root_job_item->minor,
                                            "multi-job#%d callback finish, ver:%d root_engine:%d root_minor:%d T:%llu\n",
                                            root_job_item->job_id,
                                            root_job_item->job_ver,
                                            root_job_item->engine,
                                            root_job_item->minor,
                                            root_job_item->timestamp);
                            }
                        }
                        ...
                        if(job_item->need_callback) {
                            job->callback(job);
                            VCAP_VG_LOG(VCAP_VG_LOG_QUIET, job_item->engine, job_item->minor,
                                        "job#%d callback finish, ver:%d engine:%d minor:%d T:%llu\n",
                                        job_item->job_id,
                                        job_item->job_ver,
                                        job_item->engine,
                                        job_item->minor,
                    ...
                    if(job_item->type == VCAP_VG_JOB_TYPE_MULTI) {
                        ...
                        if(list_empty(&root_job_item->m_job_head)) {    ///< all job done of multi-job
                            if(root_job_item->need_callback) {
                                ...
                                job->callback(job);
                                VCAP_VG_LOG(VCAP_VG_LOG_QUIET, root_job_item->engine, root_job_item->minor,
                                            "multi-job#%d callback fail, ver:%d root_engine:%d root_minor:%d T:%llu\n",
                                            root_job_item->job_id,
                                            root_job_item->job_ver,
                                            root_job_item->engine,
                                            root_job_item->minor,
                        ...
                        if(job_item->need_callback) {
                            job->callback(job);
                            VCAP_VG_LOG(VCAP_VG_LOG_QUIET, job_item->engine, job_item->minor,
                                        "job#%d callback fail, ver:%d engine:%d minor:%d T:%llu\n",
                                        job_item->job_id,
                                        job_item->job_ver,
                                        job_item->engine,
                                        job_item->minor,
...
#if defined(CFG_VG_V2_SUPPORT)
static void vcap_vg_callback_tasklet(unsigned long data)
{
    if(vcap_vg_log_level) {
        vcap_log("vg callback tasklet enter\n");
    }

    vcap_vg_callback_scheduler((void *)data);
}
#elif defined(CFG_VG_CALLBACK_USE_KTHREAD)
...
int vcap_vg_init(int id, char *name, int engine_num, int minor_num, void *private_data)
{
    ...
    /* create job and group pool cache buffer */
    if(sizeof(struct vcap_vg_job_item_t) > sizeof(struct vcap_vg_grp_pool_t))
        mem_size = sizeof(struct vcap_vg_job_item_t);
    else
        mem_size = sizeof(struct vcap_vg_grp_pool_t);
    vcap_vg_info[id]->job_cache = kmem_cache_create(vcap_vg_info[id]->name, mem_size, 0, 0, NULL);
...
#if defined(CFG_VG_V2_SUPPORT)
    tasklet_init(&vcap_vg_info[id]->callback_tasklet, vcap_vg_callback_tasklet, (unsigned long)vcap_vg_info[id]);
    task_inited++;
#elif defined(CFG_VG_CALLBACK_USE_KTHREAD)
...
#if defined(CFG_VG_V2_SUPPORT)
        if(task_inited)
            tasklet_kill(&vcap_vg_info[id]->callback_tasklet);
#elif defined(CFG_VG_CALLBACK_USE_KTHREAD)
...
void vcap_vg_trigger_callback_finish(void *param)
{
...
#if defined(CFG_VG_V2_SUPPORT)
    tasklet_schedule(&pvg_info->callback_tasklet);
#elif defined(CFG_VG_CALLBACK_USE_KTHREAD)
    ...
    VCAP_VG_LOG(VCAP_VG_LOG_QUIET, job_item->engine, job_item->minor,
                "job#%d trigger finish, ver:%d engine:%d minor:%d T:%llu\n",
                job_item->job_id,
                job_item->job_ver,
                job_item->engine,
                job_item->minor,
...
void vcap_vg_trigger_callback_fail(void *param)
{
    ...
    VCAP_VG_LOG(VCAP_VG_LOG_QUIET, job_item->engine, job_item->minor,
                "job#%d trigger fail, ver:%d engine:%d minor:%d T:%llu\n",
                job_item->job_id,
                job_item->job_ver,
                job_item->engine,
                job_item->minor,
...
void vcap_vg_panic_dump_job(void)
{
    ...
    struct vcap_vg_job_item_t *job_item, *next_item;
    char *type_msg[]   = {"S", "M"};
    char *status_msg[] = {"None", "STANDBY", "ONGOING", "FINISH", "FAIL", "KEEP"};
    ...
    if(fd) {
        switch(ntype) {
            case VCAP_VG_NOTIFY_NO_JOB:
                ret = video_entity_notify(&pvg_info->entity, fd, VG_NO_NEW_JOB, 1);
                break;
            case VCAP_VG_NOTIFY_HW_DELAY:
                ret = video_entity_notify(&pvg_info->entity, fd, VG_HW_DELAYED, 1);
                break;
            case VCAP_VG_NOTIFY_SIGNAL_LOSS:
                ret = video_entity_notify(&pvg_info->entity, fd, VG_SIGNAL_LOSS, 1);
                break;
            case VCAP_VG_NOTIFY_SIGNAL_PRESENT:
                ret = video_entity_notify(&pvg_info->entity, fd, VG_SIGNAL_PRESENT, 1);
                break;
            case VCAP_VG_NOTIFY_FRAMERATE_CHANGE:
                ret = video_entity_notify(&pvg_info->entity, fd, VG_FRAMERATE_CHANGE, 1);
                break;
            case VCAP_VG_NOTIFY_HW_CONFIG_CHANGE:
                ret = video_entity_notify(&pvg_info->entity, fd, VG_HW_CONFIG_CHANGE, 1);
                break;
            case VCAP_VG_NOTIFY_TAMPER_ALARM:
                ret = video_entity_notify(&pvg_info->entity, fd, VG_TAMPER_ALARM, 1);
                break;
            case VCAP_VG_NOTIFY_TAMPER_ALARM_RELEASE:
                ret = video_entity_notify(&pvg_info->entity, fd, VG_TAMPER_ALARM_RELEASE, 1);
                break;
            default:
                ret = -1;
                break;
