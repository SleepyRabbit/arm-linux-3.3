 ...
 *
 * $Revision: 1.11 $
 * $Date: 2015/01/26 11:46:49 $
 *
 * ChangeLog:
 *  $Log: vcap_dev.c,v $
 *  Revision 1.11  2015/01/26 11:46:49  jerson_l
 *  1. add md_gau_zero module parameter to specify md gaussian zero correct method.
 *
...
static int md_gau_zero = VCAP_MD_GAU_ZERO_CORRECT_OSD_DELAY;
module_param(md_gau_zero, int, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(md_gau_zero, "MD Gaussian Zero Correction => 0: Disable 1: Skip_Pixel 2: OSD_Delay");
...
static int __init vcap_dev_init(void)
{
    ...
    /* check md gaussian zero correct method */
#ifdef PLAT_MD_LAST_MB_GAU_ZERO_CORRECT
    vcap_dev_m_param.md_gau_zero = (md_gau_zero >= VCAP_MD_GAU_ZERO_CORRECT_MAX) ? VCAP_MD_GAU_ZERO_CORRECT_DISABLE : md_gau_zero;
#else
    vcap_dev_m_param.md_gau_zero = VCAP_MD_GAU_ZERO_CORRECT_DISABLE;
#endif
