/**
 * @file nvp1914_lib.c
 * Nextchip NVP1914 4-CH 960H/D1 Video Decoders and Audio Codecs Library
 * Copyright (C) 2015 GM Corp. (http://www.grain-media.com)
 *
 * $Revision: 1.2 $
 * $Date: 2015/01/20 13:00:22 $
 *
 * ChangeLog:
 *  $Log: nvp1914_lib.c,v $
 *  Revision 1.2  2015/01/20 13:00:22  shawn_hu
 *  Update NVP1914 register setting (bank#0, 0x37) for wrong identification of TVI-720P video input in hybrid design.
 *
...
/** 
 * Note: 1. int nvp1914_video_get_mode(int id) was updated to 1-CH mode
 *       2. VPortX_SEQx (Bank1, 0xC0~0xC7) was updated to be inversed
 */
...
/*30*/   0x12,0x12,0x12,0x12,0x2F,0x82,0x0B,0x43,0x0A,0x0A,0x0A,0x0A,0x80,0x80,0x80,0x80,
...
/*20*/   0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,
/*30*/   0x11,0x11,0x11,0x11,0x2F,0x02,0x0B,0x43,0x0A,0x0A,0x0A,0x0A,0x80,0x80,0x80,0x80,
...
static int nvp1914_clock_init(int id)
{
    ...
    ret = nvp1914_i2c_write(id, 0xCC, 0x10); if(ret < 0) goto exit;
    ret = nvp1914_i2c_write(id, 0xCD, 0x10); if(ret < 0) goto exit;
...
static int nvp1914_ntsc_720h_1ch(int id)
{
    int ret;
    ...
    ret = nvp1914_i2c_write(id, 0xB4, 0x10); if(ret < 0) goto exit; ///< MPP1 clk & delay setting
    ret = nvp1914_i2c_write(id, 0xB5, 0x10); if(ret < 0) goto exit; ///< MPP2 clk & delay setting
    ret = nvp1914_i2c_write(id, 0xB6, 0x10); if(ret < 0) goto exit; ///< MPP3 clk & delay setting
    ret = nvp1914_i2c_write(id, 0xB7, 0x10); if(ret < 0) goto exit; ///< MPP4 clk & delay setting

    ret = nvp1914_i2c_write(id, 0xC0, 0x33); if(ret < 0) goto exit; ///< VPortA_SEQ2, VPortA_SEQ1
    ret = nvp1914_i2c_write(id, 0xC1, 0x33); if(ret < 0) goto exit; ///< VPortA_SEQ4, VPortA_SEQ3
    ret = nvp1914_i2c_write(id, 0xC2, 0x22); if(ret < 0) goto exit; ///< VPortB_SEQ2, VPortB_SEQ1
    ret = nvp1914_i2c_write(id, 0xC3, 0x22); if(ret < 0) goto exit; ///< VPortB_SEQ4, VPortB_SEQ3
    ret = nvp1914_i2c_write(id, 0xC4, 0x11); if(ret < 0) goto exit; ///< VPortC_SEQ2, VPortC_SEQ1
    ret = nvp1914_i2c_write(id, 0xC5, 0x11); if(ret < 0) goto exit; ///< VPortC_SEQ4, VPortC_SEQ3
    ret = nvp1914_i2c_write(id, 0xC6, 0x00); if(ret < 0) goto exit; ///< VPortD_SEQ2, VPortD_SEQ1
    ret = nvp1914_i2c_write(id, 0xC7, 0x00); if(ret < 0) goto exit; ///< VPortD_SEQ4, VPortD_SEQ3

    ret = nvp1914_i2c_write(id, 0xC8, 0x00); if(ret < 0) goto exit;
    ret = nvp1914_i2c_write(id, 0xC9, 0x00); if(ret < 0) goto exit;
    ret = nvp1914_i2c_write(id, 0xCC, 0x10); if(ret < 0) goto exit;
    ret = nvp1914_i2c_write(id, 0xCD, 0x10); if(ret < 0) goto exit;
    ret = nvp1914_i2c_write(id, 0xCE, 0x10); if(ret < 0) goto exit;
    ret = nvp1914_i2c_write(id, 0xCF, 0x10); if(ret < 0) goto exit;
    ...
    ret = nvp1914_reversion2_fix(id, NVP1914_VMODE_NTSC_720H_1CH); if(ret < 0) goto exit;
    ret = nvp1914_system_init(id); if(ret < 0) goto exit;

    printk("NVP1914#%d ==> NTSC 720H 36MHz MODE!!\n", id);

exit:
    return ret;
}

static int nvp1914_pal_720h_1ch(int id)
{
    int ret;
    ...
    ret = nvp1914_reversion2_fix(id, NVP1914_VMODE_PAL_720H_1CH); if(ret < 0) goto exit;
    ret = nvp1914_system_init(id); if(ret < 0) goto exit;

    printk("NVP1914#%d ==> PAL 720H 36MHz MODE!!\n", id);

exit:
    return ret;
}

static int nvp1914_ntsc_960h_1ch(int id)
{
    int ret;
    ...
    ret = nvp1914_reversion2_fix(id, NVP1914_VMODE_NTSC_960H_1CH); if(ret < 0) goto exit;
    ret = nvp1914_system_init(id); if(ret < 0) goto exit;

    printk("NVP1914#%d ==> NTSC 960H 36MHz MODE!!\n", id);

exit:
    return ret;
}

static int nvp1914_pal_960h_1ch(int id)
{
    int ret;
    ...
    ret = nvp1914_reversion2_fix(id, NVP1914_VMODE_PAL_960H_1CH); if(ret < 0) goto exit;
    ret = nvp1914_system_init(id); if(ret < 0) goto exit;

    printk("NVP1914#%d ==> PAL 960H 36MHz MODE!!\n", id);
...
int nvp1914_reversion2_fix(int id, NVP1914_VMODE_T mode)
{
    ...
    for(i=0;i<4;i++)
    {
        if(mode == NVP1914_VMODE_PAL_960H_1CH || mode ==  NVP1914_VMODE_NTSC_960H_1CH)
    ...
    for(i=0;i<4;i++)
    {
        if(mode == NVP1914_VMODE_PAL_960H_1CH)
            ret = nvp1914_i2c_write(id, (0x81+i*2), 0x28);
	else if (mode ==  NVP1914_VMODE_NTSC_960H_1CH)
...
int nvp1914_video_set_mode(int id, NVP1914_VMODE_T mode)
{
    ...
    switch(mode) {
        case NVP1914_VMODE_NTSC_720H_1CH:
            ret = nvp1914_ntsc_720h_1ch(id);
            break;
        case NVP1914_VMODE_NTSC_960H_1CH:
            ret = nvp1914_ntsc_960h_1ch(id);
            break;
        case NVP1914_VMODE_PAL_720H_1CH:
            ret = nvp1914_pal_720h_1ch(id);
            break;
        case NVP1914_VMODE_PAL_960H_1CH:
            ret = nvp1914_pal_960h_1ch(id);
            break;
        default:
            ret = -1;
...
int nvp1914_video_get_mode(int id)
{
    ...
    if(is_ntsc) {
        if(is_960h)
            ret = NVP1914_VMODE_NTSC_960H_1CH;
        else
            ret = NVP1914_VMODE_NTSC_720H_1CH;
    }
    else {
        if(is_960h)
            ret = NVP1914_VMODE_PAL_960H_1CH;
        else
            ret = NVP1914_VMODE_PAL_720H_1CH;
    }
