...
static int tp2802_chip_rev[TP2802_DEV_MAX] = {[0 ... (TP2802_DEV_MAX - 1)] = -1}; ///< -1: Unknown revision, 0: Rev.C & before, 1: Rev.D
static int is_v2_cam[TP2802_DEV_MAX*TP2802_DEV_CH_MAX] = {[0 ... (TP2802_DEV_MAX*TP2802_DEV_CH_MAX - 1)] = 0}; ///< Check if video input is TVI v2 camera
static int pre_fmt[TP2802_DEV_MAX*TP2802_DEV_CH_MAX] = {[0 ... (TP2802_DEV_MAX*TP2802_DEV_CH_MAX - 1)] = -1}; ///< Previous video input format, -1: Unknown
...
static const unsigned char TP2802_COMM_REV_D[] = {
    0x07, 0xc0,
    0x0b, 0xc0,
    0x22, 0x38,
    0x2e, 0x60,
    ...
    0x4d, 0x0f,
    0x4e, 0xff,
    0x40, 0x01, ///< For TP2801A: begin
    0x50, 0x00,
    0x51, 0x00,
    0x52, 0x00,
    0x7f, 0x00,
    0x80, 0x00,
    0x81, 0x00, ///< For TP2801A: end
    0x40, 0x00, ///< For TX: begin
    0x50, 0x00,
    0x51, 0x0b,
    0x52, 0x0c,
    0x53, 0x19,
    0x54, 0x78,
    0x55, 0x21,
    0x7e, 0x0f, ///< For TX: end
    0x7f, 0x00, ///< For RX: begin
    0x80, 0x07,
    0x81, 0x08,
    0x82, 0x04,
    0x83, 0x00,
    ...
    0x89, 0x39,
    0x8a, 0x27,
    0xb9, 0x0f  ///< For RX: end
};

int tp2802_get_chip_revision(int id)
{
    int val;

    val = (tp2802_i2c_read(id, 0xfe)<<8) | tp2802_i2c_read(id, 0xff);
    if((val&0xFF00) == 0x2800)
       printk("Detected TP%4x ", val);
    else {
       printk("Invalid chip TP%4x\n", val);
       return -1;
    }

    val = tp2802_i2c_read(id, 0xfd);
    tp2802_chip_rev[id] = val&0x07;
    printk("revision %s\n", tp2802_chip_rev[id]?"D":"C");
...
int tp2802_video_get_video_output_format(int id, int ch, struct tp2802_video_fmt_t *vfmt)
{
    ...
    switch(npxl_value) {
        case 0x0672:
            if(is_v2_cam[id*TP2802_DEV_CH_MAX+ch] == 1)
...
int tp2802_video_set_video_output_format(int id, int ch, TP2802_VFMT_T vfmt, TP2802_VOUT_FORMAT_T vout_fmt)
{
    ...
    if(is_v2_cam[id*TP2802_DEV_CH_MAX+ch] == 1 && (tp2802_chip_rev[id] == 1)) { ///< Only Rev.D support TVI V2 camera
        clk = tp2802_i2c_read(id, ch<2?0xFA:0xFB);
        clk &= ch%2?0x8F:0xF8;
        clk |= ch%2?0x10:0x01;
        ret = tp2802_i2c_write(id, ch<2?0xFA:0xFB, clk); ///< Clock Output Control
        if(ret < 0)
            goto exit;
        clk = tp2802_i2c_read(id, 0xF5);
        clk |= 0x1<<ch;
        ret = tp2802_i2c_write(id, 0xF5, clk); ///< System Clock Control
        if(ret < 0)
            goto exit;
    }
    else { ///< TVI V1 camera support
        clk = tp2802_i2c_read(id, ch<2?0xFA:0xFB);
        clk &= ch%2?0x8F:0xF8;
        clk &= ~(ch%2?0x10:0x01);
        ret = tp2802_i2c_write(id, ch<2?0xFA:0xFB, clk); ///< Clock Output Control
    ...
    if(pre_fmt[id*TP2802_DEV_CH_MAX+ch] != vfmt) { ///< Update output setting only when video format is changed
        switch(vfmt) {
        ...
        if(tp2802_chip_rev[id] == 1) { ///< Rev.D setting
            if((vfmt == TP2802_VFMT_1080P30) || (vfmt == TP2802_VFMT_1080P25)) {
                ret = tp2802_i2c_write(id, 0x15, 0x03);
                if(ret < 0)
                    goto exit;
                ret = tp2802_i2c_write(id, 0x16, 0xd2);
                if(ret < 0)
                    goto exit;
            }
            else {
                ret = tp2802_i2c_write(id, 0x15, 0x13);
                if(ret < 0)
                    goto exit;
                ret = tp2802_i2c_write(id, 0x16, 0x15);
                if(ret < 0)
                    goto exit;
            }
        }
        pre_fmt[id*TP2802_DEV_CH_MAX+ch] = vfmt;
    }
...
int tp2802_status_get_video_loss(int id, int ch)
{
    ...
    if(vlos)
        is_v2_cam[id*TP2802_DEV_CH_MAX+ch] = 0; ///< Clear the flag of v2 camera when video input was lost
exit:
    return vlos;
}
...
int tp2802_status_get_video_input_format(int id, int ch, struct tp2802_video_fmt_t *vfmt)
{
    ...
    if(status < 0x30) {
        ...
        if((fmt_idx == TP2802_VFMT_720P30) || (fmt_idx == TP2802_VFMT_720P25)) { ///< TP2802 will identify v2 camera as 720P30/25 when EQ locked
            if(is_v2_cam[id*TP2802_DEV_CH_MAX+ch] == 0) {
                is_v2_cam[id*TP2802_DEV_CH_MAX+ch] = (tp2802_i2c_read(id, 0x03)&0x08)?1:0; ///< See if v2 camera input when EQ locked
                if(is_v2_cam[id*TP2802_DEV_CH_MAX+ch] == 1)
                    printk("TP2802#%d-CH#%d: TVI V2 camera detected!\n", id, ch);
            }
        }
...
int tp2802_video_init(int id, TP2802_VOUT_FORMAT_T vout_fmt)
{
    ...
    if(vout_fmt == TP2802_VOUT_FORMAT_BT1120) {
        ...
        for(i=0; i<ARRAY_SIZE(TP2802_COMM); i+=2) {
            if(tp2802_chip_rev[id] == 0)
                ret = tp2802_i2c_write(id, TP2802_COMM[i], TP2802_COMM[i+1]);
            else ///< Rev.D
                ret = tp2802_i2c_write(id, TP2802_COMM_REV_D[i], TP2802_COMM_REV_D[i+1]);
            if(ret < 0)
                goto exit;
        }
    }
