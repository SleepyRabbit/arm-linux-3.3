 ...
 *
 * $Revision: 1.5 $
 * $Date: 2015/02/16 12:17:19 $
 *
 * ChangeLog:
 *  $Log: tp2802_drv.c,v $
 *  Revision 1.5  2015/02/16 12:17:19  shawn_hu
 *  Add TVI V2 camera support on TP2802 Rev.D chips (Rev.C doesn't support V2 cameras).
 *
 *  Revision 1.4  2015/01/26 08:04:22  shawn_hu
 *  Update HDTVI Techpoint TP2802 driver for supporting chip revision D.
 *
...
static int tp2802_device_init(int id)
{
    ...
    ret = tp2802_get_chip_revision(id);
    if(ret < 0)
        goto exit;
            ...
            for(i=0; i<dev_num; i++) {
                ...
                for(ch=0; ch<TP2802_DEV_CH_MAX; ch++) {
                    ...
                    if((pdev->cur_plugin[ch] == 1) && (pdev->pre_plugin[ch] == 0)) {        ///< cable plugged-in
                        ...
                        /* If HD cable is plugged-out, force black image output to 720P25 */
                        ret = tp2802_video_set_video_output_format(i, ch, TP2802_VFMT_720P25, vout_format[i]);
                        if(ret < 0)
                            goto sts_update;
