 ...
 *
 * $Revision: 1.25 $
 * $Date: 2016/02/04 05:54:57 $
 *
 * ChangeLog:
 *  $Log: plat_8287.c,v $
 *  Revision 1.25  2016/02/04 05:54:57  sainthuang
 *  SSP2's main clock is from PLL5/cntp
 *
 *  Revision 1.24  2016/01/21 10:07:25  sainthuang
 *  Main clock is not consist with clock formula in audio_drv
 *  make main clock of SSP2 is 24M
 *
 *  Revision 1.23  2015/08/20 09:22:53  sainthuang
 *  for consistent with macro of main clock in audio_drv, make clock source for SSP1 also from PLL5(the same as SSP0)
 *
 *  Revision 1.22  2015/02/16 03:15:58  andy_hsu
 *  modify GM828x ssp0 clock source[25:24] to pll5out
 *
...
static pmuReg_t plat_pmu_reg_8287[] = {
    ...
    {
     .reg_off   = 0x74,
     .bits_mask = 0x3F | (0x3F<<6) | (0x3F<<12),
     .lock_bits = 0x0,
     .init_val  = 0x0, //0x9<<12,
     .init_mask = 0x3F | (0x3F<<6) | (0x3F<<12) //0x3F<<12
    },
...
void pmu_cal_clk(void)
{
    ...
    if (pll5_out == 750000000) {
        ret = ftpmu010_write_reg(plat_pmu_fd, 0x28, (0x2 << 20), (0x3 << 20));
        if (ret < 0)
            panic("%s, register ssp2 clock on failed!\n", __func__);


        // set ssp0, 0x74[5:0] as 30(decimal) and SSP1 0x74 [11:6] and SSP2, 0x74 [17:12] 
        ret = ftpmu010_write_reg(plat_pmu_fd, 0x74, 0x1E | (0x1E << 6) | (0x1E << 12), 0x3F | (0x3F << 6) | (0x3F << 12));
        if (ret < 0)
            panic("%s, set reg 0x74[5:0] failed!\n", __func__);
