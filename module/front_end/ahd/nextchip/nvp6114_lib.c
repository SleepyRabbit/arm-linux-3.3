 ...
 *
 * $Revision: 1.10 $
 * $Date: 2015/10/02 06:04:47 $
 *
 * ChangeLog:
 *  $Log: nvp6114_lib.c,v $
 *  Revision 1.10  2015/10/02 06:04:47  shawn_hu
 *  Fix the typo of "nvp6124_i2c_write()".
 *
 *  Revision 1.9  2015/09/23 03:07:39  sainthuang
...
static int nvp6114_audio_init(int id, int ch_num)
{
	...
	ret = nvp6114_i2c_write(id, 0x40, 0x08); if(ret < 0) goto exit;     ///< Audio input gain init - Gain 1.0
    ret = nvp6114_i2c_write(id, 0x41, 0x08); if(ret < 0) goto exit;
    ret = nvp6114_i2c_write(id, 0x42, 0x08); if(ret < 0) goto exit;
    ret = nvp6114_i2c_write(id, 0x43, 0x08); if(ret < 0) goto exit;
    ...
    if (cascade) {
        if ((id == 0)||(id == 2)) {     // 0/2 is master, configure master
            for(i = 0; i < 4; i++) {
                j = i * 2;
                ch = nvp6114_ain_to_ch(id, j);
                ch1 = nvp6114_ain_to_ch(id, j+1);
                RSEQ[i] = (ch1 << 4 | ch);

                ch = nvp6114_ain_to_ch(id + 1, j);
                ch1 = nvp6114_ain_to_ch(id + 1, j+1);
                ch += 8;
                ch1 += 8;
                RSEQ[i+4] = (ch1 << 4 | ch);
            }
