 ...
 * ChangeLog:
 *  $Log: nvp6124_drv.c,v $
 *  Revision 1.7  2015/10/14 02:15:10  shawn_hu
 *  Rearrange NVP6124 output channel sequence (Reg@0xC0~C7) to fix the problem of a straight line shown on the video.
 *
 *  Revision 1.6  2015/09/16 02:16:46  shawn_hu
 *  Add a Proc node for register read/write during debugging, e.g. cat /proc/nvp6124.0/reg_rw.
 *
 *  Revision 1.5  2015/03/31 11:48:58  sainthuang
 *  add ioctl interface for audio
 *
 *  Revision 1.5  2015/03/31 19:43:35  saint_huang
 *  Add ioctl interface for audio 
 *
 *  Revision 1.4  2015/03/31 17:54:55  saint_huang
 *  Add audio driver support
 *
 *  Revision 1.3  2015/03/31 02:26:03  shawn_hu
 *  Fix the error input format detection if the camera was plug on&off quickly.
 *
 *  Revision 1.2  2015/03/24 07:16:13  shawn_hu
...
static int vmode[NVP6124_DEV_MAX] = {[0 ... (NVP6124_DEV_MAX - 1)] = NVP6124_VMODE_1CH_BYPASS};
...
15:NTSC_CIF_AFHD_2CH, 16:PAL_CIF_AFHD_2CH");
...
static struct proc_dir_entry *nvp6124_proc_vmode[NVP6124_DEV_MAX]     = {[0 ... (NVP6124_DEV_MAX - 1)] = NULL};
...
static struct proc_dir_entry *nvp6124_proc_reg_rw[NVP6124_DEV_MAX]    = {[0 ... (NVP6124_DEV_MAX - 1)] = NULL};
...
static u32 NVP6124_ACH_MAP[NVP6124_DEV_MAX*NVP6124_DEV_AUDIO_CH_MAX];
...
static int nvp6124_create_ch_map(int map_id)
{
    ...
    switch(map_id) {
        case 1: ///< for GM8210/GM8287 Socket Board
            for(i=0; i<dev_num; i++) {
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+0] = CH_IDENTIFY(dev_num-i-1, 0, NVP6124_DEV_VPORT_C);
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+1] = CH_IDENTIFY(dev_num-i-1, 1, NVP6124_DEV_VPORT_B);
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+2] = CH_IDENTIFY(dev_num-i-1, 2, NVP6124_DEV_VPORT_A);
        ...
        case 2: ///< for Generic 4CH
            for(i=0; i<dev_num; i++) {
                ...
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+1] = CH_IDENTIFY(i, 1, NVP6124_DEV_VPORT_A);
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+2] = CH_IDENTIFY(i, 2, NVP6124_DEV_VPORT_A);
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+3] = CH_IDENTIFY(i, 3, NVP6124_DEV_VPORT_A);
            }
            break; 
        case 3: ///< for 4CH Output on VPORT_C
            for(i=0; i<dev_num; i++) {
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+0] = CH_IDENTIFY(i, 0, NVP6124_DEV_VPORT_C);
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+1] = CH_IDENTIFY(i, 1, NVP6124_DEV_VPORT_C);
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+2] = CH_IDENTIFY(i, 2, NVP6124_DEV_VPORT_C);
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+3] = CH_IDENTIFY(i, 3, NVP6124_DEV_VPORT_C);
            }
            break; 
        ...
        default:
            for(i=0; i<dev_num; i++) {
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+0] = CH_IDENTIFY(i, 0, NVP6124_DEV_VPORT_A);
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+1] = CH_IDENTIFY(i, 1, NVP6124_DEV_VPORT_A);
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+2] = CH_IDENTIFY(i, 2, NVP6124_DEV_VPORT_B);
                NVP6124_VCH_MAP[(i*NVP6124_DEV_CH_MAX)+3] = CH_IDENTIFY(i, 3, NVP6124_DEV_VPORT_B);
            }
            break;
    }
	...
	//Audio channel map
	for (i=0; i<dev_num; i++) {		
		NVP6124_ACH_MAP[(i*NVP6124_DEV_AUDIO_CH_MAX)+0] = CH_IDENTIFY(dev_num-i-1, 0, NVP6124_DEV_VPORT_A);
		NVP6124_ACH_MAP[(i*NVP6124_DEV_AUDIO_CH_MAX)+1] = CH_IDENTIFY(dev_num-i-1, 1, NVP6124_DEV_VPORT_A);
		NVP6124_ACH_MAP[(i*NVP6124_DEV_AUDIO_CH_MAX)+2] = CH_IDENTIFY(dev_num-i-1, 2, NVP6124_DEV_VPORT_A);
		NVP6124_ACH_MAP[(i*NVP6124_DEV_AUDIO_CH_MAX)+3] = CH_IDENTIFY(dev_num-i-1, 3, NVP6124_DEV_VPORT_A);
		NVP6124_ACH_MAP[(i*NVP6124_DEV_AUDIO_CH_MAX)+4] = CH_IDENTIFY(dev_num-i-1, 4, NVP6124_DEV_VPORT_B);
		NVP6124_ACH_MAP[(i*NVP6124_DEV_AUDIO_CH_MAX)+5] = CH_IDENTIFY(dev_num-i-1, 5, NVP6124_DEV_VPORT_B);
		NVP6124_ACH_MAP[(i*NVP6124_DEV_AUDIO_CH_MAX)+6] = CH_IDENTIFY(dev_num-i-1, 6, NVP6124_DEV_VPORT_B);
		NVP6124_ACH_MAP[(i*NVP6124_DEV_AUDIO_CH_MAX)+7] = CH_IDENTIFY(dev_num-i-1, 7, NVP6124_DEV_VPORT_B);
	}
    return 0;
}
...
int nvp6124_ain_to_ch(int id, int vin_idx)
{
    int i;

    if(id >= NVP6124_DEV_MAX || vin_idx >= NVP6124_DEV_AUDIO_CH_MAX)
        return 0;

    for(i=0; i<(dev_num*NVP6124_DEV_AUDIO_CH_MAX); i++) {
        if((CH_IDENTIFY_ID(NVP6124_ACH_MAP[i]) == id) && (CH_IDENTIFY_VIN(NVP6124_ACH_MAP[i]) == vin_idx))
            return (i%NVP6124_DEV_AUDIO_CH_MAX);
    }

    return 0;
}
EXPORT_SYMBOL(nvp6124_ain_to_ch);

int nvp6124_ch_to_ain(int id, int ch_idx)
{
    int i;

    if(id >= NVP6124_DEV_MAX || ch_idx >= NVP6124_DEV_AUDIO_CH_MAX)
        return 0;

    for(i=0; i<(dev_num*NVP6124_DEV_AUDIO_CH_MAX); i++) {
        if((CH_IDENTIFY_ID(NVP6124_ACH_MAP[i]) == id) && ((i%NVP6124_DEV_AUDIO_CH_MAX) == ch_idx))
            return CH_IDENTIFY_VIN(NVP6124_ACH_MAP[i]);
    }

    return 0;
}
EXPORT_SYMBOL(nvp6124_ch_to_ain);
...
int nvp6124_get_vch_id(int id, NVP6124_DEV_VPORT_T vport, int vport_seq)
{
    ...
    for(i=0; i<(dev_num*NVP6124_DEV_CH_MAX); i++) {
        if((CH_IDENTIFY_ID(NVP6124_VCH_MAP[i]) == id) &&
           ...
           ((CH_IDENTIFY_VIN(NVP6124_VCH_MAP[i])%vport_chnum) == vport_seq)) {
            return i;
        }
    }
...
static int nvp6124_proc_vmode_show(struct seq_file *sfile, void *v)
{
    int i, cur_vmode;
    ...
    seq_printf(sfile, "\n[NVP6124#%d]\n", pdev->index);
    for(i=0; i<NVP6124_VMODE_MAX; i++)
        seq_printf(sfile, "%02d: %s\n", i, vmode_str[i]);
    seq_printf(sfile, "----------------------------------\n");

    cur_vmode = vmode[pdev->index];
    seq_printf(sfile, "Current==> %s\n\n",(cur_vmode >= 0 && cur_vmode < NVP6124_VMODE_MAX) ? vmode_str[cur_vmode] : "Unknown");
...
static int nvp6124_proc_video_fmt_show(struct seq_file *sfile, void *v)
{
    ...
    for(i=0; i<NVP6124_DEV_CH_MAX; i++) {
        for(j=0; j<(dev_num*NVP6124_DEV_CH_MAX); j++) {
            if((CH_IDENTIFY_ID(NVP6124_VCH_MAP[j]) == pdev->index) && (CH_IDENTIFY_VIN(NVP6124_VCH_MAP[j]) == i)) {
                ret = nvp6124_video_get_output_format(pdev->index, i, &vfmt, vmode[pdev->index]);
...
static int nvp6124_proc_output_ch_show(struct seq_file *sfile, void *v)
{
    ...
    if (vin_idx >= 0 && vin_idx <= 7)
        ch = nvp6124_ain_to_ch(pdev->index, vin_idx);
...
static int nvp6124_proc_reg_rw_show(struct seq_file *sfile, void *v)
{
    struct nvp6124_dev_t *pdev = (struct nvp6124_dev_t *)sfile->private;

    down(&pdev->lock);
    seq_printf(sfile, "\nCommands for read/write registers: (all hexadecimal)");
    seq_printf(sfile, "\necho [read/write] [bank] [reg_offset] ([read count])/[write_value]");
    seq_printf(sfile, "\ne.g. \"echo r 0 F4 > /proc/nvpXXXX.x/reg_rw\" to read Bank#0 Reg@0xF4 (DEV_ID).");
    seq_printf(sfile, "\n     \"echo r 0 0 ff > /proc/nvpXXXX.x/reg_rw\" to dump all Bank#0 registers.");
    seq_printf(sfile, "\n     \"echo w 1 cA 0 > /proc/nvpXXXX.x/reg_rw\" to write Bank#1 Reg@0xCA to 0 (Disable all VCLK/VDO output).\n\n");
    up(&pdev->lock);
    return 0;
}
...
static ssize_t nvp6124_proc_output_ch_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    ...
    if (ch < 16) {
        if (ch >= 8)
            ch -= 8;
        vin_idx = nvp6124_ch_to_ain(pdev->index, ch);
        if (temp >= 8)
            vin_idx += 8;
    }
...
static ssize_t nvp6124_proc_vmode_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int cur_vmode;
    char value_str[32] = {'\0'};
    struct seq_file *sfile = (struct seq_file *)file->private_data;
    ...
    if(cur_vmode != vmode[pdev->index]) {
        if((cur_vmode < NVP6124_VMODE_1CH_BYPASS) || (cur_vmode >= NVP6124_VMODE_MAX)) {
        ...
        switch(vmode[pdev->index]) {
            ...
            case NVP6124_VMODE_NTSC_CIF_AFHD_2CH:
            case NVP6124_VMODE_PAL_CIF_AFHD_2CH:
                if((cur_vmode == NVP6124_VMODE_1CH_BYPASS) || (cur_vmode == NVP6124_VMODE_NTSC_960H_4CH)
                   || (cur_vmode == NVP6124_VMODE_PAL_960H_4CH) || (cur_vmode == NVP6124_VMODE_NTSC_CIF_AHD_4CH) 
                   || (cur_vmode == NVP6124_VMODE_PAL_CIF_AHD_4CH)) {
                    printk("Can't dynamically change vmode from 2CH to 1CH/4CH mode!\n");
                    goto exit;
                }
                else
                    ; ///< exit switch case & re-init NVP6124 for current vmode
                break;
            case NVP6124_VMODE_NTSC_960H_4CH:
            case NVP6124_VMODE_PAL_960H_4CH:
            case NVP6124_VMODE_NTSC_CIF_AHD_4CH:
            case NVP6124_VMODE_PAL_CIF_AHD_4CH:
                if((cur_vmode == NVP6124_VMODE_NTSC_960H_4CH) || (cur_vmode == NVP6124_VMODE_PAL_960H_4CH)
                   || (cur_vmode == NVP6124_VMODE_NTSC_CIF_AHD_4CH) || (cur_vmode == NVP6124_VMODE_PAL_CIF_AHD_4CH))
                    ; ///< exit switch case & re-init NVP6124 for current vmode
                else {
        ...
        nvp6124_video_init(pdev->index, cur_vmode);
...
static ssize_t nvp6124_proc_reg_rw_write(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
#define BANK_SEL 0xFF
    int  i, j, ret, bank, offset, value;
    char rw, arg_chk;
    char value_str[32] = {'\0'};
    struct seq_file *sfile = (struct seq_file *)file->private_data;
    struct nvp6124_dev_t *pdev = (struct nvp6124_dev_t *)sfile->private;
    ...
    value_str[count] = '\0';
    ret = sscanf(value_str, "%c %x %x %x %c\n", &rw, &bank, &offset, &value, &arg_chk);

    down(&pdev->lock);
    ///< parameter check
    if((bank > 0xFF) || (offset > 0xFF) || (value > 0xFF))  
        printk("Input should not greater than 0xFF!\n");
    else {
        if((rw == 'r') && (ret == 3)) {
            ret = nvp6124_i2c_write(pdev->index, BANK_SEL, (u8)bank); if(ret < 0) goto exit;
            printk("\nBank#%d Read:\nReg@0x%02x: 0x%02x\n", (u8)bank, (u8)offset, nvp6124_i2c_read(pdev->index, (u8)offset));
        }
        else if((rw == 'r') && (ret == 4)) {
            ret = nvp6124_i2c_write(pdev->index, BANK_SEL, (u8)bank); if(ret < 0) goto exit;
            printk("\nBank#%d Read:\n", (u8)bank);
            if(value == 0xFF) { ///< dump all the register of bankX
                printk("     ");
                for(j = 0; j < 16 ; j++)
                    printk("0x%.2x ", j);
                printk("\n     -------------------------------------------------------------------------------\n");
                ...
                for(i = 0; i < 16 ; i++){
                    printk("0x%X0|", i);
                    for(j = 0; j < 16 ; j++)
                        printk("0x%.2X ", nvp6124_i2c_read(pdev->index, (u8)(16*i+j)));
                    printk("\n");
                }
            }
            else {
                for(i=((offset+value)>0xFF)?(u8)(0xFF-offset):(u8)value, j=0; i > 0; i--, j++)
                    printk("Reg@0x%02x: 0x%02x\n", (u8)(offset+j), nvp6124_i2c_read(pdev->index, (u8)(offset+j)));
            }
        }
        else if((rw == 'w') && (ret == 4)) {
            ret = nvp6124_i2c_write(pdev->index, BANK_SEL, (u8)bank); if(ret < 0) goto exit;
            ret = nvp6124_i2c_write(pdev->index, (u8)offset, (u8)value); if(ret < 0) goto exit;
            printk("\nBank#%d\nWrite Reg@0x%02x: 0x%02x\n", (u8)bank, (u8)offset, (u8)value);
        }
    }
exit:
    up(&pdev->lock);
...
static int nvp6124_proc_vmode_open(struct inode *inode, struct file *file)
{
    return single_open(file, nvp6124_proc_vmode_show, PDE(inode)->data);
}
...
static int nvp6124_proc_reg_rw_open(struct inode *inode, struct file *file)
{
    return single_open(file, nvp6124_proc_reg_rw_show, PDE(inode)->data);
}

static struct file_operations nvp6124_proc_vmode_ops = {
    .owner  = THIS_MODULE,
    .open   = nvp6124_proc_vmode_open,
    .write  = nvp6124_proc_vmode_write,
    .read   = seq_read,
    .llseek = seq_lseek,
...
static struct file_operations nvp6124_proc_reg_rw_ops = {
    .owner  = THIS_MODULE,
    .open   = nvp6124_proc_reg_rw_open,
    .write  = nvp6124_proc_reg_rw_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};
...
static void nvp6124_proc_remove(int id)
{
    ...
    if(nvp6124_proc_root[id]) {
        if(nvp6124_proc_vmode[id]) {
            remove_proc_entry(nvp6124_proc_vmode[id]->name, nvp6124_proc_root[id]);
            nvp6124_proc_vmode[id] = NULL;
        }
        ...
        if(nvp6124_proc_reg_rw[id]) {
            remove_proc_entry(nvp6124_proc_reg_rw[id]->name, nvp6124_proc_root[id]);
            nvp6124_proc_reg_rw[id] = NULL;
        }
...
static int nvp6124_proc_init(int id)
{
    ...
    /* vmode */
    nvp6124_proc_vmode[id] = create_proc_entry("vmode", S_IRUGO|S_IXUGO, nvp6124_proc_root[id]);
    if(!nvp6124_proc_vmode[id]) {
        printk("create proc node '%s/vmode' failed!\n", nvp6124_dev[id].name);
        ret = -EINVAL;
        goto err;
    }
    nvp6124_proc_vmode[id]->proc_fops = &nvp6124_proc_vmode_ops;
    nvp6124_proc_vmode[id]->data      = (void *)&nvp6124_dev[id];
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    nvp6124_proc_vmode[id]->owner     = THIS_MODULE;
#endif
    ...
    /* register read/write */
    nvp6124_proc_reg_rw[id] = create_proc_entry("reg_rw", S_IRUGO|S_IXUGO, nvp6124_proc_root[id]);
    if(!nvp6124_proc_reg_rw[id]) {
        printk("create proc node '%s/reg_rw' failed!\n", nvp6124_dev[id].name);
        ret = -EINVAL;
        goto err;
    }
    nvp6124_proc_reg_rw[id]->proc_fops = &nvp6124_proc_reg_rw_ops;
    nvp6124_proc_reg_rw[id]->data      = (void *)&nvp6124_dev[id];
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    nvp6124_proc_reg_rw[id]->owner     = THIS_MODULE;
#endif
...
static long nvp6124_miscdev_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch(cmd) {
		...
		case NVP6124_GET_VOL:
			tmp = nvp6124_audio_get_mute(pdev->index);
            if(tmp < 0) {                
				ret = -EFAULT;                
				goto exit;            
			}
		...
		case NVP6124_SET_VOL:
			if(copy_from_user(&tmp, (void __user *)arg, sizeof(tmp))) {
				ret = -EFAULT;
				goto exit;
			}
			
			ret = nvp6124_audio_set_volume(pdev->index, tmp);
			if(tmp < 0) {
			    ret = -EFAULT;
				goto exit;
			}
		...
		case NVP6124_GET_OUT_CH:
			tmp = nvp6124_audio_get_output_ch(pdev->index);
			if(tmp < 0) {
				ret = -EFAULT;
				goto exit;
			}
		...
		case NVP6124_SET_OUT_CH:
			...
			if (tmp < 16) {
				if (tmp >= 8)
					tmp -= 8;
				vin_idx = nvp6124_ch_to_ain(pdev->index, tmp);
				if (ch >= 8)
					vin_idx += 8;
			}
			tmp = vin_idx;
		
			ret = nvp6124_audio_set_output_ch(pdev->index, tmp);
			if(tmp < 0) {
				ret = -EFAULT;
				goto exit;
			}
...
static int nvp6124_device_init(int id)
{
    ...
    /*====================== video init ========================= */
    ret = nvp6124_video_init(id, vmode[id]);
    if(ret < 0)
        goto exit;

    /*====================== audio init ========================= */
    ret = nvp6124_audio_set_mode(id, vmode[(id>>1)<<1], sample_size, sample_rate);
    if(ret < 0)
        goto exit;
...
{
    int i, ch, ret, novid;
    int debounce[NVP6124_DEV_MAX*NVP6124_DEV_CH_MAX] = {[0 ... (NVP6124_DEV_MAX*NVP6124_DEV_CH_MAX - 1)] = 0}; ///< debounce design to fix the wrong format detection if camera was plug on&off quickly
    int pre_vmode[dev_num];
    struct nvp6124_dev_t *pdev;
    struct nvp6124_video_fmt_t vout_fmt;
    unsigned char ch_seq[4] = {2, 1, 0, 3}; ///< Video output sequence for 1-CH bypass mode, should be the same as the ones in nvp6124_video_set_output_format() of nvp6124_lib.c
    ...
    do {
        ...
        for(i=0; i<dev_num; i++) {
            ...
            for(ch=0; ch<NVP6124_DEV_CH_MAX; ch++) {
                ...
                if((vmode[i] == NVP6124_VMODE_1CH_BYPASS) || (pre_vmode[i] != vmode[i])) { ///< dynamically update EQ & output format only during 1-CH bypass mode or vmode changed
                ...
                if((pdev->cur_input_fmt[ch] != pdev->pre_input_fmt[ch]) && (pdev->cur_input_fmt[ch] != NVP6124_VFMT_UNKNOWN)) {   ///< video input format was changed
                    if(debounce[i*NVP6124_DEV_CH_MAX+ch] < 3) 
                        debounce[i*NVP6124_DEV_CH_MAX+ch]++;
                    else {
                        ...
                        ret = nvp6124_video_set_output_format(i, ch, pdev->cur_input_fmt[ch], vmode[i]);
                        if(ret < 0)
                            continue;
                    ...
                    if(notify && pdev->vlos_notify) {
                        ...
                        debounce[i*NVP6124_DEV_CH_MAX+ch] = 0;
                    }

                    /* notify current video format */
                    if(notify && pdev->vfmt_notify) {
                        /* get video port output format */
                        ret = nvp6124_video_get_output_format(i, ch, &vout_fmt, vmode[i]);
                        if(ret < 0)
                            continue;
                        else {
                            switch(vmode[i]){
                                case NVP6124_VMODE_1CH_BYPASS:
                                default:
                                    pdev->vfmt_notify(i, ch_seq[ch], &vout_fmt);
                                    break;
                                case NVP6124_VMODE_NTSC_960H_2CH:
                                case NVP6124_VMODE_NTSC_720P_2CH:
                                case NVP6124_VMODE_PAL_960H_2CH:
