...
#undef PFX
#define PFX 					"ADDA308-0"

struct ftssp010_adda308_t {
    ftssp010_pcm_ops_t ops;
    ftssp010_card_t *card;
    u_int32_t sample_rate;
};

static struct ftssp010_adda308_t *my_chip = NULL;
...
static u_int32_t g_bclk[] =
    {512000, 1024000, 1499400, 2048000, 2998800, 3072000};
...
static int ftssp010_adda308_open(ftssp010_pcm_ops_t * ops, snd_pcm_substream_t * substream)
{
...
static int ftssp010_adda308_close(ftssp010_pcm_ops_t * ops, snd_pcm_substream_t * stream)
{
    return 0;
}

static int ftssp010_adda308_prepare(ftssp010_pcm_ops_t * ops, snd_pcm_substream_t * stream,
                                    struct snd_pcm_runtime * run)
{
    struct ftssp010_adda308_t *chip = container_of(ops, struct ftssp010_adda308_t, ops);
    
    DEBUG_ADDA308_PRINT("%s: set fs %d\n", __FUNCTION__, run->rate);
    ...
    if (chip->sample_rate != run->rate) {
        ...
        case 48000:
            ...
            break;
        default:
            DEBUG_ADDA308_PRINT("%s fails: wrong ADDA302_FS value\n", __FUNCTION__);
            return -EFAULT;
        }
...
static int ftssp010_adda308_get_dma_parameters(ftssp010_pcm_ops_t * ops,
                                               snd_pcm_substream_t * substream, int *dma_width)
{
    snd_pcm_format_t sample_size = substream->runtime->format;
    
    DEBUG_ADDA308_PRINT("In %s: sample_size(ALSA define SYM) = %d\n", __FUNCTION__,
    ...
    case SNDRV_PCM_FORMAT_U32_LE:
        *dma_width = 4;
        break;
    default:
        DEBUG_ADDA308_PRINT("%s fails: wrong sample size\n", __FUNCTION__);
        return -1;
    }
...
static int ftssp010_adda308_startup(ftssp010_pcm_ops_t * ops)
{
    return 0;
}

static struct ftssp010_pcm_ops ftssp010_adda308_ops = {
    .owner = THIS_MODULE,
    .startup = ftssp010_adda308_startup,
    .open = ftssp010_adda308_open,
    .close = ftssp010_adda308_close,
    .prepare = ftssp010_adda308_prepare,
    .get_dma_parameters = ftssp010_adda308_get_dma_parameters
};

static int fi2s_adda308_init(void)
{
    int ret = -ENOMEM;
    ftssp010_card_t *card = NULL;

    my_chip = kmalloc(sizeof(struct ftssp010_adda308_t), GFP_KERNEL);
    if (my_chip == NULL) {
        ret = -ENOMEM;
        goto exit_err1;
    }

    memset(my_chip, 0, sizeof(struct ftssp010_adda308_t));
    ...
    my_chip->card = card;
    my_chip->ops = ftssp010_adda308_ops;
    ...
    DEBUG_ADDA308_PRINT("I2S probe ok in %s mode.\n",
  ...
  exit_err2:
    kfree(my_chip);
  exit_err1:
    DEBUG_ADDA308_PRINT("I2S probe fail in %s mode.",
                        (card->SSP_CR0 & SSP_MS_MASK) ? "Master" : "Slave");

    return ret;
}

static void fi2s_adda308_exit(void)
{
    ftssp010_ssp_remove(my_chip->card);
    kfree(my_chip);
    my_chip = NULL;
...
module_init(fi2s_adda308_init);
module_exit(fi2s_adda308_exit);
