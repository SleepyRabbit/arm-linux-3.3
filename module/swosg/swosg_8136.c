 ...
 * VER0.0.2 2015/02/10 support rgb1555 color mode 
 * 
 */
...
#define SWOSG8136_VERSION   ((0 << 16) + (0 << 8) + 2) 
...
{
#if 1/*translate ARGB1555 to RGBa5650*/
    int i ;
    unsigned int real_sz = 0 ;
    ...
    for(i = 0 ; i< real_sz ; i+=2)
    {
        src_data = (unsigned short*)&src_image[i];
        dst_data = (unsigned short*)&dst_image[i];
        tmp_data = *src_data;

        if(!(tmp_data & 0x8000))
            *dst_data =  0x1 << 5;
        else
            *dst_data = (((tmp_data << 1)&0xffc0) | (tmp_data&0x1f));
...
{   
    ...
    gm2d_osg_blit_t  gm2d_blit_data;
    ...
    memset(&gm2d_blit_data,0x0,sizeof(gm2d_osg_blit_t));
    ...
    if(frm_get_addrinfo(&fram_buf_in,0) == 0){
        gm2d_blit_data.dst_paddr = (unsigned int)frm_va2pa(blit_param->dest_bg_addr);        
    }else{
    ...
    gm2d_blit_data.dst_w = blit_param->dest_bg_w;
    gm2d_blit_data.dst_h = blit_param->dest_bg_h;
    gm2d_blit_data.src_paddr = (unsigned int)canvas_info->canvas_paddr;
    gm2d_blit_data.src_w = canvas_info->canvas_w;
    gm2d_blit_data.src_h = canvas_info->canvas_h;
    gm2d_blit_data.dx    = dx;
    gm2d_blit_data.dy    = dy;
    gm2d_blit_data.src_colorkey = 0x20;//COLOUR5551(0x0,0x0,0x0,0x0);//COLOUR8888(0xff,0xff,0xff,0xff);

    if(gm2d_osg_do_blit(&gm2d_blit_data,fire) < 0){        
        DEBUG_M(LOG_WARNING,"gm2d_osg_do_blit is error\n");
        return -1;
    }
    //if(dx%2 == 1)
    //    dx-=1;    
...
{   
    ...
    memset(&t2d_osgmask,0x0,sizeof(gm2d_osg_mask_t));
#if 1 /*8287 dest was flush by last job*/   
    memset(&fram_buf_in,0x0,sizeof(frammap_buf_t));
    ...
    if(gm2d_osg_do_mask(&t2d_osgmask,fire) < 0){        
        DEBUG_M(LOG_WARNING,"gm2d_osg_do_mask is error\n");
        return -1;
    }
