...
struct property_map_t property_map[] = {
    ...
    {ID_REPEAT, "repeat", "1 means repeat the previous frame"},
#endif
    {ID_NULL, ""}
};
...
#ifdef SUPPORT_REPEAT
          case ID_REPEAT:
            chan_param->repeat = property[i].value;
            break;
#endif /* SUPPORT_REPEAT */
                                        ...
                                        struct video_entity_job_t *cur, ftdi220_param_t *param)
{
    ...
    if (prev) {
        if (command & OPT_CMD_DI) {
            ...
            if (node->chan_param.repeat) {
                param->cr_y_ptr = param->ff_y_ptr;
                param->nt_y_ptr = param->fw_y_ptr;
            } else {
                param->cr_y_ptr = cur->in_buf.buf[0].addr_pa;
                param->nt_y_ptr = cur->in_buf.buf[0].addr_pa + line_ofs;
            }
                                        ...
                                        struct video_entity_job_t *cur, ftdi220_param_t *param)
{
    ...
    if (prev_top_frame) {
        ...
        if (cur_node->chan_param.repeat) {
            cur_top_param->cr_y_ptr = cur_top_param->ff_y_ptr;
            cur_top_param->nt_y_ptr = cur_top_param->fw_y_ptr;
        } else {
