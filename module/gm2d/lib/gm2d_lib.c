...
#define GM2D_GETREG(sdrv, reg) (*(volatile unsigned int *)((unsigned long)sdrv->io_mem + reg))

/// Writes a device register.
#define GM2D_SETREG(sdrv, reg, val) do { *(volatile unsigned int  *)((unsigned long)sdrv->io_mem + reg) = (val); } while (0)
 ...
 * 
 */
void GM2D_EngineReset( gm2dge_desc_t *tdrv )
{
    DEBUG_PROC_ENTRY;
 ...
 */
int GM2D_EmitCommands(gm2dge_desc_t *tdrv ,int b_wait)
{
    int i = 0;
    unsigned int ** p_buffer;
    struct gm2d_shared *sharemem = (struct gm2d_shared *)tdrv->sharemem;
	
    DEBUG_PROC_ENTRY;

		
    if (sharemem->hw_running || GM2D_GETREG(tdrv,GM2D_STATUS) )
    {
        printf("%s list hw=%x state=%x\n",__FUNCTION__,sharemem->hw_running,GM2D_GETREG(tdrv,GM2D_STATUS));
	return GM2D_FAIL;
    }
	...
	ioctl(tdrv->fd, GM2D_START_BLOCK, 0);
	/*clear cmd count*/
	tdrv->llst->cmd_cnt = 0;
    }
    else
        printf("hw is running %2x %2x\n",sharemem->hw_running,GM2D_GETREG(tdrv, GM2D_STATUS));
	
    //unlock semaphore
    pthread_mutex_unlock(&tdrv->mutex);

    if(b_wait){
        while (GM2D_GETREG(tdrv, GM2D_STATUS) && ioctl(tdrv->fd, GM2D_WAIT_IDLE) < 0)
	{
 ...
 */
int GM2D_DrawLine( gm2dge_desc_t *tdrv,gm2d_region_t  *line)
{
    DEBUG_PROC_ENTRY;
    ...
    //stat_lines++;
    if(tdrv->llst->cmd_cnt + 6 > GM2D_CWORDS)
        GM2D_EmitCommands((gm2dge_desc_t*)tdrv, 1);
    //lock semaphore
    pthread_mutex_lock(&tdrv->mutex);

    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_STARTXY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(line->x1, line->y1);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_ENDXY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(line->x2, line->y2);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_CMDLIST_WAIT | GM2D_DRAW_CMD;
...
int GM2D_DrawRectangle( gm2dge_desc_t *tdrv , gm2d_retangle_t *rect  )
{
    DEBUG_PROC_ENTRY;
    ...
    if(tdrv->llst->cmd_cnt + 24 > GM2D_CWORDS)
        GM2D_EmitCommands((gm2dge_desc_t*)tdrv, 1);
    ...
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_STARTXY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(x1, y1);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_ENDXY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(x1, y2);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_CMDLIST_WAIT | GM2D_DRAW_CMD;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = 0x01;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_STARTXY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(x1, y2);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_ENDXY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(x2, y2);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_CMDLIST_WAIT | GM2D_DRAW_CMD;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = 0x01;         
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_STARTXY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(x1, y1);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_ENDXY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(x2, y1);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_CMDLIST_WAIT | GM2D_DRAW_CMD;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = 0x01;         
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_STARTXY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(x2, y1);
...
/**
 * @param gm2dge_desc_t *tdrv
 * @param rect for target
 */
int GM2D_FillRectangle(gm2dge_desc_t *tdrv , gm2d_retangle_t *rect )
{
    ...
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_STARTXY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(rect->x, rect->y);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_ENDXY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(rect->x + rect->w - 1, rect->y + rect->h - 1);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_CMDLIST_WAIT | GM2D_DRAW_CMD;
 ...
 *
 * @param gm2dge_desc_t *tdrv
 * @param struct gm2d_target_surface_t *tdata
 */
int GM2D_Target_Set_Mode( gm2dge_desc_t *tdrv,struct gm2d_target_surface_t *tdata )
{
    unsigned int i,n ,j;
    DEBUG_PROC_ENTRY;
    ...
    if(!n || !GM2D_CHECK_CMD_IS_ENOUGH(tdrv->llst->cmd_cnt , n))
    {
        printf("GM2D_CHECK_CMD_IS_ENOUGH %s" , __FUNCTION__);
	    return GM2D_FAIL;
    }
...
/*
 * @This function is used to open a gm2d channel for drawing.
 *
 * @function gm2dge_desc_t *gm2dge_lib_open
 * @Parmameter reb_type indicates which RGB is used in this framebuffer
 * @return non zero means success, null for fail
 */
gm2dge_desc_t *GM2Dge_Lib_Open(GM2D_MODE_T rgb_type)
{
    ...
    desc = malloc(sizeof(gm2dge_desc_t));
    if (!desc) {
        printf("%s, allocate memory fail! \n", __func__);
        return NULL;
    }

    memset(desc, 0, sizeof(gm2dge_desc_t));
    ...
    desc->fd = open("/dev/gm2d", O_RDWR);
    if (desc->fd < 0) {
        printf("Error to open GM2D fd! \n");
        close(desc->lcd_fd);
        return NULL;
    }
    ...
    byte_per_pixel = GM2D_modesize(rgb_type);
    ...
    desc->llst = (gm2dge_llst_t *)malloc(sizeof(gm2dge_llst_t));
    if (!desc->llst) {
        printf("%s, allocate llst fail! \n", __func__);
        goto err_exit;
    }
    memset((void *)desc->llst, 0, sizeof(gm2dge_llst_t));
    ...
    if (GM2D_GETREG(desc, GM2D_IDREG) != GM2D_MAGIC)
    {
...
/*
 * @This function is used to close a gm2d channel.
 *
 * @void gm2dge_lib_close(void *descriptor));
...
{
    int byte_per_pixel;
    gm2dge_desc_t *desc = (gm2dge_desc_t *)descriptor;

    /* release semphore */
    pthread_mutex_destroy(&desc->mutex);

    byte_per_pixel = GM2D_modesize(desc->bpp_type);
 ...
 * @param color
 */
int GM2D_Set_Drawing_Color( gm2dge_desc_t *tdrv,gm2d_color_t* color)
{
    DEBUG_PROC_ENTRY;

    if(tdrv->llst->cmd_cnt + 2 > GM2D_CWORDS)
        GM2D_EmitCommands((gm2dge_desc_t*)tdrv, 1);

    //lock semaphore
    pthread_mutex_lock(&tdrv->mutex);

    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_DRAW_COLOR;
 ...
 */
int GM2D_Set_Dest_Surface( gm2dge_desc_t *tdrv)
{
    struct gm2d_target_surface_t tdata;

    DEBUG_PROC_ENTRY;

    memset(&tdata,0x0,sizeof(struct gm2d_target_surface_t));

    tdata.command =	TARGET_MODE_SET|TARGET_BADDR_SET|TARGET_STRIDE_SET|TARGET_RESOLXY_SET;
    tdata.target_baddr = tdrv->fb_paddr;
    tdata.target_pitch = ((tdrv->width * GM2D_modesize(tdrv->bpp_type)) & 0xffff);
    tdata.target_res_xy = GM2D_XYTOREG(tdrv->width,tdrv->height);
    tdata.target_mode = (1 << 31) | (0 << 29) | tdrv->bpp_type;
    //tdata.target_mode = (GM2D_modesize(tdrv->bpp_type) == 4 ? 0 : ((1 << 31) | (1 << 23))) | (0 << 29) | tdrv->bpp_type; // IS:bit 29 disables 16->32 packing , bits 31/23 set on 32-bit mode

    if(GM2D_Target_Set_Mode(tdrv , &tdata) < 0)
    {   
    	printf("GM2D_Target_Set_Mode fail\n");
        return GM2D_FAIL;
    }
    return GM2D_SUCCESS;
}
...
/**
 * GM2D_Set_Clip_Window.
 * @param tdrv Driver data
 * @param rect clip window
 */
int GM2D_Set_Clip_Window( gm2dge_desc_t *tdrv,gm2d_retangle_t *rect)
{
    struct gm2d_target_surface_t tdata;
    ...
    tdata.target_clip_min = GM2D_XYTOREG(rect->x, rect->y);
    tdata.target_clip_max = GM2D_XYTOREG(rect->w, rect->h);
 ...
 * @param tdrv Driver data
 */
int GM2D_Set_Drawing_Blend(gm2dge_desc_t *tdrv)
{
    int dst_colorkey = 0;
    struct gm2d_target_surface_t tdata;
    ...
    if (tdrv->drawing_flags & GM2D_DRAW_DST_COLORKEY)
        dst_colorkey = 1;

    tdata.command =	TARGET_BLEND_SET;

    if (tdrv->drawing_flags & GM2D_DRAW_DSBLEND)
    	tdata.target_blend = (dst_colorkey << 31) | (tdrv->v_dstBlend << 16) | (tdrv->v_srcBlend);
    else
    	tdata.target_blend = (dst_colorkey << 31) | GM2D_SRCCOPY_MODE;
 ...
 * @param tdrv Driver data
 */
int GM2D_Set_Blit_Blend( gm2dge_desc_t *tdrv)
{
    int dst_colorkey = 0;
    int colorize = 0;
    struct gm2d_target_surface_t tdata;
    ...
    if (tdrv->src_blt.blt_src_flags & GM2D_BLIT_DST_COLORKEY)
    	dst_colorkey = 1;
    /* will we be using the FG_COLOR register for COLORIZE _or_ SRC_PREMULTIPLY? */
    if (tdrv->src_blt.blt_src_flags & GM2D_BLIT_COLORIZE)
    	colorize = 1;


    tdata.command = TARGET_BLEND_SET;

    if (tdrv->src_blt.blt_src_flags & (GM2D_BLIT_BLEND_COLORALPHA | GM2D_BLIT_BLEND_ALPHACHANNEL))
        tdata.target_blend = (dst_colorkey << 31) | (tdrv->v_dstBlend << 16) | (tdrv->v_srcBlend);
    else
        tdata.target_blend = (dst_colorkey << 31) | (colorize << 30) | GM2D_SRCCOPY_MODE;
...
/**
 * gm2d_open_src_surface
 *
 * @param tdrv Driver data
 * @param file_name open file name for image
 */
int GM2D_Open_Src_Surface(gm2dge_desc_t *tdrv , char * file_name)
{
    FILE * filer;
    ...
    tdrv->src_blt.blt_src_size  = (((tdrv->src_blt.blt_src_width * tdrv->src_blt.blt_src_height * GM2D_modesize(tdrv->bpp_type))+4095) >>12)<<12;
...
/**
 * GM2D_Close_Src_Surface
 *
 * @param tdrv Driver data
 */
void GM2D_Close_Src_Surface(gm2dge_desc_t *tdrv)
{
    close(tdrv->src_blt.blt_src_fd);
...
/**
 * GM2D_Set_Src_Stride to set source stride
 *
 * @param tdrv Driver data
 */
void GM2D_Set_Src_Stride(gm2dge_desc_t *tdrv )
{
    DEBUG_PROC_ENTRY;
    ...
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_BLIT_SRCSTRIDE;
 ...
 * @param color RGBA
 */
void GM2D_Set_Dst_Colorkey( gm2dge_desc_t *tdrv,gm2d_color_t* color)
{
    DEBUG_PROC_ENTRY;
 ...
 */
void GM2D_Set_Src_Colorkey( gm2dge_desc_t *tdrv,gm2d_color_t* color)
{
    DEBUG_PROC_ENTRY;
    ...
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_BLIT_SRCCOLORKEY;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = COLOUR8888(color->r, color->g,color->b, color->a);
...
void GM2D_Set_FG_Color(gm2dge_desc_t *tdrv,gm2d_color_t* color)
{
    DEBUG_PROC_ENTRY;
    ...
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_BLIT_FG_COLOR;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = COLOUR8888(color->r, color->g,color->b, color->a);
 ...
 */
int GM2D_StretchBlit( gm2dge_desc_t *tdrv,gm2d_retangle_t*srect,gm2d_retangle_t* drect)           
{
    DEBUG_PROC_ENTRY;
    ...
    tdrv->src_blt.blt_src_paddr+ srect->x *	GM2D_modesize(tdrv->bpp_type) + srect->y * tdrv->src_blt.blt_src_stride;

    rotation = tdrv->src_blt.blt_src_rotation;
    /* will we do rotation? */
    if (rotation == GM2D_DEG090)
        dy += srect->h - 1;
    else if (rotation == GM2D_DEG180)
        dx += srect->w - 1, dy += srect->h - 1;
    else if (rotation == GM2D_DEG270)
        dx += srect->h - 1;
    else if (rotation == GM2D_MIR_X)/*FLIP_HORIZONTAL*/
        dx += srect->w - 1;
    else if (rotation == GM2D_MIR_Y)/*FLIP_VERTICAL*/
        dy += srect->h - 1;
    else
        rotation = GM2D_DEG000;


    /* will we do SRC_COLORKEY? */
    if (tdrv->src_blt.blt_src_flags & GM2D_BLIT_SRC_COLORKEY)
    	src_colorkey = 1;

    /* will we do COLORALPHA? */
    if (tdrv->src_blt.blt_src_flags & GM2D_BLIT_BLEND_COLORALPHA)
    	color_alpha = 1;
    ...
    if(tdrv->llst->cmd_cnt + 14 > GM2D_CWORDS)
    	GM2D_EmitCommands((gm2dge_desc_t*)tdrv, 1);

    pthread_mutex_lock(&tdrv->mutex);		

    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_BLIT_SRCADDR;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = srcAddr;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_BLIT_SRCRESOL;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(tdrv->src_blt.blt_src_width,tdrv->src_blt.blt_src_height);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_BLIT_DSTADDR;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(dx,dy);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_BLIT_DSTYXSIZE;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_XYTOREG(drect->w, drect->h);
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_BLIT_SCALE_XFN;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = scale_xfn;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_BLIT_SCALE_YFN;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = scale_yfn;
    tdrv->llst->cmd[tdrv->llst->cmd_cnt++] = GM2D_CMDLIST_WAIT | GM2D_BLIT_CMD;
 ...
 */
int GM2D_Blit( gm2dge_desc_t *tdrv,gm2d_retangle_t* rect,int dx,int dy )
{
    ...
    int color_alpha = 0;
    unsigned long srcAddr = \
    tdrv->src_blt.blt_src_paddr + rect->x * GM2D_modesize(tdrv->bpp_type) + rect->y * tdrv->src_blt.blt_src_stride;
    ...
    if(tdrv->llst->cmd_cnt + 8 > GM2D_CWORDS)
        GM2D_EmitCommands((gm2dge_desc_t*)tdrv, 1);
...
#if 1 /*Fill rectange example*/
void main(void)
{
    gm2dge_desc_t            *desc;
    gm2d_color_t             color;
    gm2d_retangle_t          rect;
    int i, j;

    desc = GM2Dge_Lib_Open(GM2D_RGBA5650);

    if(desc == NULL)
        return;  
    /* Set target surface mode base address stride, resolution */                                                          
    GM2D_Set_Dest_Surface((gm2dge_desc_t*)desc);

    /*Set Clip window*/
    rect.x = 0 ,rect.y = 0;
    rect.w = desc->width , rect.h = desc->height;
    GM2D_Set_Clip_Window(desc,&rect);

    /*Set drawing blend*/
    desc->drawing_flags &=~(GM2D_DRAW_DST_COLORKEY|GM2D_DRAW_DSBLEND);
    GM2D_Set_Drawing_Blend(desc);

    /*Set drawing color*/
    color.r = 0xff;
    color.g = 0x00;
    color.b = 0x00;    
    GM2D_Set_Drawing_Color((gm2dge_desc_t*)desc ,&color);

    printf("gm2d Start to test >>>>>>>>>> \n");
	...
	rect.w = 10;
	rect.h = 10;
	GM2D_FillRectangle((gm2dge_desc_t*)desc ,&rect);   
    }   
    GM2D_EmitCommands((gm2dge_desc_t*)desc, 1);
    printf("gm2d Test complete. >>>>>>>>>> \n");
    
    GM2Dge_Lib_Close(desc);
}
#endif
...
void main(void)
{
    ...
    GM2D_DrawLine((gm2dge_desc_t*)desc ,&line);
...
void main(void)
{
    ...
    desc->src_blt.blt_src_flags = GM2D_BLIT_COLORIZE;
    GM2D_Set_Blit_Blend((gm2dge_desc_t*)desc);    
    /*Create source surface and open image file         */
    /*File format include 8bytes WxH and RGB565 RAW data*/
    sprintf(file_name,"%s","test.bin");
    GM2D_Open_Src_Surface((gm2dge_desc_t*)desc,file_name);
    /*Set source stride*/
    desc->src_blt.blt_src_stride = desc->src_blt.blt_src_width * GM2D_modesize(desc->bpp_type);  
    GM2D_Set_Src_Stride((gm2dge_desc_t*)desc);
    /*Set FG color*/
    color.r =0x0,color.g =0xff,color.b=0x0;    
    GM2D_Set_FG_Color(desc,&color);    
    /*Set rotation*/
    desc->src_blt.blt_src_rotation = GM2D_DEG000;    
    for(j = 0 ; j < desc->height ;j+=desc->src_blt.blt_src_height){
        for(i = 0 ; i < desc->width ; i+=desc->src_blt.blt_src_width){
            ...
            GM2D_Blit((gm2dge_desc_t*)desc ,&rect,i,j);
        }
    }
    GM2D_EmitCommands((gm2dge_desc_t*)desc, 1);
    GM2D_Close_Src_Surface((gm2dge_desc_t*)desc);
    printf("gm2d Test complete. >>>>>>>>>> \n");
    GM2Dge_Lib_Close(desc);
    return;
}
...
void main(void)
{
    ...
    GM2D_Set_Clip_Window(desc,&srect);
    /*Set source blit blend mode*/
    ...
    GM2D_StretchBlit(desc ,&srect ,&drect);
