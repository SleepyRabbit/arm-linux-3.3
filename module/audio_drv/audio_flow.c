...
#ifdef USE_AEC
// speciall buffer
//static void *tmp_play_buf[32]={0};
u16 copy_play_buf[32768]={0};
//static void *tmp_record_buf[6]={0};
static u16 copy_record_buf[8192]={0};
int copy_record_buf_write = 0;
static int find_sync_pos = 0;
static int play_status=0; //, record_status=0;
static int tmpidx_play_read = 0;
static int first_time_do_aec = 0;
//static int tmpidx_record = 6;
//static int playcount=0;
//static int recordcount=0;
char sync_flag=0;
static int sync_point=0;
static int R_len=0;
static u16 tempRes[1024]={0};
static u16 RIn[1024];
static u16 PIn[1024];
int pflag;
int tmpidx_play = 0;
int countflag=0;
char insert_sync_pattern=0;
char dummyflag=0;
volatile char dummyflag2=0;
extern u16 dummybuf[1024];

u16 copy_rec_buf[32768]={0};
static int idx_rec_aec = 0;

//--- AEC parameter ---//
aec_st_int 				*st_int=NULL;
aec_func_control 	aec_fc;
aec_parameter 		aec_p;
lp_dely_line			lp1, lp2;
//--------------------//
#ifdef LIMITED_ENABLE

unsigned short gain_limit = 32768;
int gain_index = 0;
int gain_index_prev = 0;
int limit_fg = 0;
unsigned short limit_gain_tbl[40] = 
  {
		32768, 32440, 32113, 31785, 31457, 31130, 30802, 30474, 
		30147, 29819, 29491, 29164, 28836, 28508, 28180, 27853, 
		27525, 27197, 26870, 26542, 26214, 25887, 25559, 25231, 
		24904, 24576, 24248, 23921, 23593, 23265, 22938, 22610, 
		22282, 21955, 21627, 21299, 20972, 20644, 20316, 19988
  };
#endif
#endif
...
#define DECODE_REMAIN_INTERVAL	50
...
static unsigned char g711_dec_bit_stream_remain[AULAW_BLK_SZ];
static unsigned int pre_remain_sz = 0;
...
#ifdef USE_AEC
#define RECORD_FILE_PATH    "/mnt/sd_card/record.pcm"
#define PLAYBACK_FILE_PATH  "/mnt/sd_card/playback.pcm"
#define AECOUT_FILE_PATH    "/mnt/sd_card/AECOUT.pcm"
#endif
...
#ifdef USE_AEC
int loop_cnt = 0;
//int reset_aec_flag = 0;
void audio_aec_handler(struct work_struct *work)
{
	struct audio_job_t *audio_job_in = container_of(work, struct audio_job_t, in_work);	
	//struct my_work_stuct *p_test_work = container_of(work, struct my_work_stuct, tmp_work); // catch audio in buffer	

	u16 *buf2, *buf3, *buf_return;	
	u16 bufSize = 0, bufIdx = 0 ;
	int j, ret;
#ifdef DEBUG_LOG
	int ret_play=0;
	int ret_record=0;
	static unsigned long long offset1 = 0;
	static unsigned long long offset3 = 0;
	static unsigned long long offset5 = 0;	
#endif
#ifdef AEC_FILE_TEST	
	int ret_play=0;
	int ret_record=0;
	...
	// decide PCM buffer: in_pa[bufIdx] //
	if (((audio_job_in->codec_type & BITMASK(16, 16)) >> 16) > AU_TYPE_PCM)
		bufIdx = 1;

	buf_return 		= (u16 *)audio_job_in->data_offset[bufIdx][0];
	bufSize = audio_job_in->data_length[bufIdx][0]; // bufSize = 2048	
	
//#ifdef DEBUG_LOG	
#if 0
	//if((sil_flag == 1)&&(reset_aec_flag==1))
  if(insert_sync_pattern == 1)
	{
		//printk("play_status = %d, sync_flag = %d\n",play_status, sync_flag);	
        	//if(tmp_play_buf[0]!=tmp_play_buf[1])
            //    	printk("tmp_play_buf pointer different\n");

  		ret_record = audio_write_file(AECOUT_FILE_PATH, (void *) buf_return, (bufSize), &offset5);
  		if (ret_record < 0)
    			printk("write recored error!\n");	
	}
#endif	
	//data_dump((u16 *)audio_job_in->data_offset[bufIdx][0], 8, "In", 1); //0x85023c00 ~ 8504a800 32個的版本		
	//printk("play_status=%d , sync_flag=%d , countflag = %d , sil_flag = %d , tmpidx_play = %d \n", play_status , sync_flag , countflag , sil_flag , tmpidx_play);
	//printk("R0(%8p) , R1(%8p) , R2(%8p) , R3(%8p) \n", (u16 *)(tmp_play_buf[0]) , (u16 *)(tmp_play_buf[1]) , (u16 *)(tmp_play_buf[2]) , (u16 *)(tmp_play_buf[3]));

//	tmp_record_buf[0] = tmp_record_buf[1];
//	tmp_record_buf[1] = tmp_record_buf[2];
//	tmp_record_buf[2] = tmp_record_buf[3];
//	tmp_record_buf[3] = tmp_record_buf[4];
//	tmp_record_buf[4] = tmp_record_buf[5];
//	tmp_record_buf[5] = (void *)buf_return;
...
//		countflag=0;
	...
	if( sync_flag == 1 )
	{
		...
		{
			read_pos = GET_READ_IDX(copy_record_buf_write , 8);
			buf2 = (u16 *)(copy_record_buf)+ read_pos*1024;
			memcpy( buf2 , buf_return  , bufSize );
			copy_record_buf_write++;
			
			
#ifdef AEC_FILE_TEST			
			if( rflag == 0 )
			{
					ret_record = audio_read_file(AEC_NearIn_PATH, (u16 *)tempNearIn, 851968, &offset1);
					if (ret_record < 0)
						printk("read NearIn error!\n");
					pNear = tempNearIn;

					ret_play = audio_read_file(AEC_FarIn_PATH, (u16 *)tempFarIn, 851968, &offset3);
					if (ret_play < 0)
						printk("read FarIn error!\n");
					pFar = tempFarIn;
			...
			memcpy( RIn , pNear , (2048) );
			memcpy( PIn , pFar  , (2048) );			
			ret = aec_process( st_int, &aec_fc, (short *)RIn , (short *)PIn  );			
			memcpy( buf , RIn , bufSize ); // 送回去record path
			pNear += (1024);
			pFar  += (1024);
				
			ret_record = audio_write_file(AECOUT_FILE_PATH, (void *) RIn, (bufSize), &offset5);
			if (ret_record < 0)
			   printk("write recored error!\n");
#else
			
			/*memcpy( RIn , tempRes , (R_len<<1) );
			buf2 = (u16 *)(RIn);
			buf2 += R_len;
			
			buf = (u16 *)(copy_rec_buf)+ read_pos_aec*1024;
			memcpy( buf2 , buf , (sync_point<<1) );
			
			buf2 = (u16 *)(copy_rec_buf)+ read_pos_aec*1024;
			buf2 += sync_point;
			memcpy( tempRes , buf2 , (R_len<<1) );*/
			
			/*for(i=0 , j=0; i<1024 ; i++, j++){
				if(i < R_len)
					RIn[i] = tempRes[i];
				else
				  RIn[i] = copy_rec_buf[(read_pos_aec*1024) + j];
			}
			
			for(i=0 ; i<R_len ; i++)
				tempRes[i] = copy_rec_buf[(read_pos_aec*1024) + sync_point + i];*/
				
			memcpy( RIn , tempRes , (R_len<<1) );
			buf2 = (u16 *)(RIn);
			buf2 += R_len;
			read_pos = GET_READ_IDX(find_sync_pos , 8);
			buf3 = (u16 *)(copy_record_buf)+ read_pos*1024;
			memcpy( buf2 , buf3 , (sync_point<<1) );
			find_sync_pos++;
			buf3 += sync_point;
			memcpy( tempRes , buf3 , (R_len<<1) );

			//if(bufSize != 2048)
			//	printk("bufSize = %d\n", bufSize);
			if(first_time_do_aec == 0)
				printk("out RIn[0,1,2,3,4,5] = %d, %d, %d, %d, %d, %d\n",*((short *)RIn) ,*((short *)RIn+1),*((short *)RIn+2),*((short *)RIn+3),*((short *)RIn+4),*((short *)RIn+5));
			if(aec_p.aec_preprocess_enable)
				lpf((short *)RIn, &lp2, bufSize>>1);	
			
			read_pos = GET_READ_IDX(tmpidx_play_read , 32);
			buf2 = (u16 *)(copy_play_buf)+ read_pos*1024;
			memcpy( PIn , buf2  , bufSize );
			if(first_time_do_aec == 0)
			{
				printk("out PIn[0,1,2,3,4,5] = %d, %d, %d, %d, %d, %d\n",*((short *)PIn) ,*((short *)PIn+1),*((short *)PIn+2),*((short *)PIn+3),*((short *)PIn+4),*((short *)PIn+5));
				first_time_do_aec = 1;	
			}
			//if( (RIn[R_len]) != (PIn[R_len]) )
			//{
			//	printk("error find_sync_pos = %d \n", find_sync_pos);
			//	data_dump((u16 *)buf_return, 8, "In", 1);
			//	printk("RIn[%d]=0x%4x, RIn[%d]=0x%4x, RIn[%d]=0x%4x, RIn[%d]=0x%4x, RIn[%d]=0x%4x, RIn[%d]=0x%4x\n", 0, RIn[0], 1, RIn[1], 2, RIn[2], R_len, RIn[R_len], R_len+1, RIn[R_len+1], R_len+2, RIn[R_len+2]);
			//	printk("PIn[%d]=0x%4x, PIn[%d]=0x%4x, PIn[%d]=0x%4x, PIn[%d]=0x%4x, PIn[%d]=0x%4x, PIn[%d]=0x%4x \n", 0, PIn[0], 1, PIn[1], 2, PIn[2], R_len, PIn[R_len], R_len+1, PIn[R_len+1], R_len+2, PIn[R_len+2]);
			//}
#ifdef DEBUG_LOG	
			ret_play = audio_write_file(PLAYBACK_FILE_PATH, (void *) PIn, (bufSize), &offset3);
			if (ret_play < 0)
			   printk("write playback error!\n");
#endif
#ifdef DEBUG_LOG
			ret_record = audio_write_file(RECORD_FILE_PATH, (void *) RIn, (bufSize), &offset1);
			if (ret_record < 0)
			   printk("write recored error!\n");
#endif		
			loop_cnt++;
			...
			ret = aec_process( st_int, &aec_fc, (short *)RIn , (short *)PIn  );
			//endTick = jiffies;
			//printk("AEC handler ticks = %ul\n", endTick - beginTick);    			

			memcpy( buf_return , RIn , bufSize ); // 送回去record path
	...
	//if( (play_status == 2) && (sync_flag == 0) ) // when playback, not find sync point  
	if( (insert_sync_pattern == 1) && (sync_flag == 0) ) // when playback, not find sync point
	{
		  ...
		  if( (sync_flag == 0)&&(tmpidx_play < 31) )
			{
					printk("sync_flag=%d , copy_record_buf_write=%d\n ", sync_flag, copy_record_buf_write);
					//buf2 = (u16 *)copy_record_buf;
					//read_pos = GET_READ_IDX(copy_record_buf_write , 8);
					//for(i=1 ; i<=read_pos ; i++) 
					//{
						//buf2 = (u16 *)copy_record_buf + (i*1024);						
						for(j=0 ; j<((bufSize>>1)-2) ; j++)
						{
							if( buf2[j] == 255 )
							{
								if( buf2[j+1] == 238 )
								{
									if( buf2[j+2] == 221 )
									{
										if( ((buf2[j+3]) != 255)||((buf2[j+4]) != 238)||((buf2[j+5]) != 221))
												printk("this is the second sync pattern offset should be 0, pratical offset = %d\n", j);
															
										sync_flag = 1;
										sync_point = j;
										R_len = (bufSize>>1) - j;										
										tmpidx_play_read = 0;															
										adda302_set_loopback(0); //turn off REBACK
										printk("find sync_pos = %d , offset = %d\n", read_pos, j);

										buf2 += sync_point;
										memcpy( tempRes , buf2 , (R_len<<1) ); //store last record buffer
										//countflag = 0;
										find_sync_pos = read_pos + 1;
        						...
        						nlp_init_update_parameters(st_int->nlp, &aec_p);
        						ng_init_update_parameters(st_int->ng,   &aec_p);   
        						
        						display_update_parameters(st_int);    						
        						printk("trigger_AEC_power_thd = %lld\n",aec_p.trigger_AEC_power_thd);
      							printk("ng_enable = %d\n",aec_p.ng_enable);
        						printk("aec_RER_Thr = %d\n",aec_p.aec_RER_Thr);
        						printk("aec_preprocess_enable = %d\n",aec_p.aec_preprocess_enable);
        						printk("farend_RShift = %d\n",aec_p.farend_RShift);
        						printk("log = %d\n",aec_p.log);
										printk("aec_switch = %d\n", aec_p.aec_switch);        						
						...
						//printk("***************sync_pos = %d, offset = %d \n", read_pos, j) ;
			}	
			else
			{
					//countflag = 0;
					sync_flag = 0; // repeat send sync pattern
					tmpidx_play = 0;
					insert_sync_pattern = 0;
					copy_record_buf_write = 0;
			}
...
finish_AEC:
	idx_rec_aec++;
	play_status = 2; // record status
	audio_job_in->status = JOB_STS_ACK;

}
EXPORT_SYMBOL(audio_aec_handler);
#endif
...
void audio_in_handler(struct work_struct *work)
{
    ...
    if (audio_job->data)
		...
		//-------------------- peter debug code ----------------------
#ifdef DISPLAY_DEBUG_MESSAGE		
		printk("audio_in_handler: enter\n");
#endif		
		//-------------------- end -----------------------------------
    play_status = audio_job->status; //record
#endif
    ...
    *rec_dbg_stage |= (1 << IN_PCM_DONE);
	...
	//-------------------- peter debug code ----------------------
#ifdef DISPLAY_DEBUG_MESSAGE	
	printk("audio_aec_handler: enter\n");
#endif	
	//-------------------- end -----------------------------------
	audio_aec_handler(work); 
	//-------------------- peter debug code ----------------------
#ifdef DISPLAY_DEBUG_MESSAGE
	printk("audio_aec_handler: leave\n");
#endif	
	//-------------------- end -----------------------------------	   
#endif    
...
static int playback_audio(void *job, void *playBuf, unsigned playSize, unsigned ssp_idx)
{
    ...
    if((playSize!=0)&&(sync_flag==1))
    {
	//if( playSize != 1024)
	//		printk("playsize = %d\n", playSize);
	//printk("playBuf[1] = %d\n",*((short *)playBuf+1));
	    if(aec_p.aec_preprocess_enable)
    		lpf((short *)playBuf, &lp1, playSize>>1);
...
//#ifdef LIMITED_ENABLE
			...
			{			
				...
				do
				{
					if( (sptr[i] > 20480)||((sptr[i] < -20480)) )
					{

						gain_index++;
						gain_limit = limit_gain_tbl[gain_index];
						printk("gain_index = %d\n", gain_index);
						break;
					}								
				}while(++i<(playSize>>1));
			}
			//if(gain_index != gain_index_prev)		
				//-------------- debug code ------------------------------------------
				//printk("s far_end[1] = %d, far_end[1023] = %d\n", sptr[1], sptr[1023]);
				//-------------- end --------------------------------------------------
	   	//apply limit gain
	   	if(gain_index != 0)
	   	{
				...
				do
				{
					sptr[i] = (short)(((int)sptr[i]*gain_limit)>>15);
				}while(++i<(playSize>>1));
	   	}
	   	//if(gain_index != gain_index_prev)	
	   		//-------------- debug code -------------------------------------
				//printk("e far_end[1] = %d, far_end[1023] = %d\n", sptr[1], sptr[1023]);	
				//-------------- end --------------------------------------------
			gain_index_prev = gain_index;	
#else
			sptr = (short *)playBuf;
	   	gain_limit = limit_gain_tbl[39];
			i = 0;
			do
			{
				sptr[i] = (short)(((int)sptr[i]*gain_limit)>>15);
...
//  #ifdef DEBUG_LOG
//			ret_record = audio_write_file(RECORD_FILE_PATH, (void *) sptr, playSize, &offset1);
//			if (ret_record < 0)
//			   printk("write recored error!\n");
//  #endif						
#endif
...
#ifdef USE_AEC
		if ((playBuf == silence_buf)) {
				//------------- peter debug code -----------------------------------------
				printk("finished play 1024 silence\n");
				//printk("insert_sync_pattern = %u\n", (unsigned char)insert_sync_pattern);
				//------------- end -------------------------------------------------------		
	   		tmpidx_play = 0;
    }else {
				//------------- peter debug code ------------------------------------------
				//printk("insert_sync_pattern = %u\n", (unsigned char)insert_sync_pattern);
				//------------- end -------------------------------------------------------    
				Idx = GET_READ_IDX(tmpidx_play, 32);
				//tmp_play_buf[Idx] = (void *)playBuf;
				buf = (u16 *)(copy_play_buf) + Idx*1024; 
				memcpy(buf, (u16 *)playBuf, (2048)); //save real playback data
				tmpidx_play++;
				//------------- peter debug code -----------------
				//printk("put into copy_play_buf index = %d, start 1024 samples = %d, %d\n", Idx, buf[0], buf[1]);
				//------------- end ------------------------------				    	
    }   
#endif
...
#ifdef USE_AEC
    if (playSize != 2048) // To double check the audio_out_handler is ending
		dummyflag = 1;
#endif
...
#if defined(CONFIG_PLATFORM_GM8287)
                do_div(tmp, ((drv_param->sample_rate[def_sr_ssp])*100));          
#else
...
/*        
    		//---for AEC to open ADDA308 REBACK ---//
    		//other condition AEC_on = 1        
    		if ((sync_flag == 0) && (sil_flag==0) ) {
    				audio_job->time_sync.mute_samples = 1 * 2048;	
    				adda302_set_loopback(1); //1:on , 0:off
    		}
*/
        if (audio_job->time_sync.mute_samples > 0 && !is_live_on) {
	    //------------------ trace code -----------------------------------------
	    printk("initial mute_samples = %d\n", audio_job->time_sync.mute_samples);			
	    //------------------ end ------------------------------------------------	
            audio_job->time_sync.mute_samples *= (audio_job->sample_size / BITS_PER_BYTE) * ratio / (100 * 100);
            //------------------ trace code ---------------------------------------------------------------------------------------------------------
            printk("sample_size = %d\n, ratio = %d\n, adjusted mute_samples = %d\n",audio_job->sample_size, ratio, audio_job->time_sync.mute_samples);
            //------------------ end ---------------------------------------------------------------------------------------------------------------- 
        ...
        GM_LOG(GM_LOG_PB_PKT_FLOW, "ssp%d, job(%d) playback %d bytes\n", ssp_num, job_id, final_sz);
    		...
    		//---for AEC to open ADDA308 REBACK ---//
    		//other condition AEC_on = 1
    		if ((sync_flag == 0) && (insert_sync_pattern==0) ) {	
				  	char *cptr;    			
				  	
    				adda302_set_loopback(1); //1:on , 0:off
      			printk("set loopback mode\n");
      			
				  	cptr = (char *)final_buf;
						cptr[0] = 0xff;
						cptr[1] = 0;//[0xff][0x00] => 0x00ff = 255
						cptr[2] = 0xee;
						cptr[3] = 0;//[0xee][0x00] => 0x00ee = 238
						cptr[4] = 0xdd;
						cptr[5] = 0;//[0xdd][0x00] => 0x00dd = 221
						cptr[6] = 0xff;
						cptr[7] = 0;//[0xff][0x00] => 0x00ff = 255
						cptr[8] = 0xee;
						cptr[9] = 0;//[0xee][0x00] => 0x00ee = 238
						cptr[10] = 0xdd;
						cptr[11] = 0;//[0xdd][0x00] => 0x00dd = 221			
    				printk("insert sync pattern\n");
    				insert_sync_pattern = 1; //least buffer      			
    		}
#endif
...
/*
The bit stream size may not multiple of block size(decoder need the block size).
Keep the remainder  for next bit stream coming and combine it to fit a block.
*/
static int gm_g711_dec(void *job, void *inputBuf, unsigned int inputSize)
{
	//unsigned int i, decOutputSize = 0;
    unsigned int decOutputSize = 0;
	unsigned int blk_cnt = 0, repair_sz = 0, stream_remain_sz = 0;
	unsigned int RemainBeginTick = 0, RemainEndTick = 0;
	void *remain_ptr;
	void *blk_start_ptr;
	...
	blk_start_ptr = inputBuf;
	stream_remain_sz = inputSize;
	RemainEndTick = jiffies;
	//printk("%s, RemainEndTick = %ul\n", __func__, RemainEndTick);

	if (RemainBeginTick) {
		if ( (RemainEndTick - RemainBeginTick) > DECODE_REMAIN_INTERVAL) {
			printk("%s, decode remain interval = %ul\n", __func__, (RemainEndTick - RemainBeginTick));			
			pre_remain_sz = 0;
		}
	}
	/*Previous bit stream has remain data, copy part of coming bit stream to fit a block and send to decoder*/
	if (pre_remain_sz) {				
		repair_sz = blk_sz - pre_remain_sz;
		memcpy((g711_dec_bit_stream_remain + pre_remain_sz), (void *)((unsigned)blk_start_ptr), repair_sz);	
				
		G711_Dec(inparam_dec, g711_dec_bit_stream_remain, g711_dec_output_buf);
		...
		blk_start_ptr += repair_sz;
		stream_remain_sz -= repair_sz;	
	}
	/*decode deal with bit stream by block*/
	while (stream_remain_sz >= blk_sz) {		
		memcpy(g711_dec_input_buf, (void *)((unsigned)blk_start_ptr + blk_cnt * blk_sz), blk_sz);
		...
		if (*audio_drv_get_resample_save_flag()) {
			...
			if (stored_sz >= 0) {
				int ret = 0;			    
				ret = audio_write_file(RESAMPLE_FILE_PATH, (void *)g711_dec_output_buf, decOutputSize, &offset);
				if (ret < 0)
                	printk("%s: write PCM error!\n", __func__);
			} else {
				/* reset */
	...
	/*Bit stream is not multiple of block size, keep the remain data and record it size*/
	if (stream_remain_sz) {
		RemainBeginTick = jiffies;
		//printk("%s, RemainBeginTick = %ul\n", __func__, RemainBeginTick);
		remain_ptr = blk_start_ptr + (blk_cnt * blk_sz);
		memcpy(g711_dec_bit_stream_remain, (void *)((unsigned)remain_ptr), stream_remain_sz);	
		pre_remain_sz = stream_remain_sz;
	} else {
		RemainBeginTick = 0;
		pre_remain_sz = 0;
	}
...
#ifdef USE_AEC
	play_status = audio_job->status; //playback
#endif
    ...
    switch (audio_job->status) {
        ...
        case JOB_STS_PLAY_STOP:
            ...
            *play_dbg_stage |= (1 << OUT_PLAY_STOP);
			...
			/*clear previous remain size for G711 decode when stop playing*/
			pre_remain_sz = 0;
            return;
        default:
...
decode_finish:
    if (retval > PROCESS_SUCCESS) {
		...
		/*clear previous remain size for G711 decode when error occurs*/
		pre_remain_sz = 0;
    }
