...
#ifdef USE_AEC
static struct proc_dir_entry *audio_proc_aec_root = NULL;
static struct proc_dir_entry *audio_proc_aec_pw_threshold  = NULL;
static struct proc_dir_entry *audio_proc_aec_nlp_min_gain  = NULL;
static struct proc_dir_entry *audio_proc_aec_nlp_max_gain  = NULL;
static struct proc_dir_entry *audio_proc_aec_nlp_step_gain  = NULL;
static struct proc_dir_entry *audio_proc_aec_nlp_threshold  = NULL;
static struct proc_dir_entry *audio_proc_aec_ng_ltrhold  = NULL;
static struct proc_dir_entry *audio_proc_aec_ng_utrhold  = NULL;
static struct proc_dir_entry *audio_proc_aec_ng_rel		 = NULL;
static struct proc_dir_entry *audio_proc_aec_ng_att		 = NULL;
static struct proc_dir_entry *audio_proc_aec_ng_inv_rel = NULL;
static struct proc_dir_entry *audio_proc_aec_ng_inv_att = NULL;
static struct proc_dir_entry *audio_proc_aec_ng_ht	 = NULL;
static struct proc_dir_entry *audio_proc_aec_avgr_threshold 	= NULL;
static struct proc_dir_entry *audio_proc_aec_sample_rate = NULL;
static struct proc_dir_entry *audio_proc_aec_log 		 = NULL;
static struct proc_dir_entry *audio_proc_aec_ng_enable = NULL;
static struct proc_dir_entry *audio_proc_aec_farend_RShift = NULL;
static struct proc_dir_entry *audio_proc_aec_Pre_Proc_enable = NULL;
static struct proc_dir_entry *audio_proc_aec_switch = NULL;
#endif
...
#ifdef USE_AEC
static ssize_t audio_proc_aec_pw_threshold_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_PW_THRESHOLD, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_nlp_min_gain_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NLP_MIN_GAIN, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_nlp_max_gain_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NLP_MAX_GAIN, val);	
...
static ssize_t audio_proc_aec_nlp_step_gain_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NLP_STEP_GAIN, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_nlp_threshold_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NLP_THRESHOLD, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_ng_enable_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NG_ENABLE, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_farend_RShift_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_FAR_RSHIFT, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_Pre_Proc_enable_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_PRE_PROC_ENABLE, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_switch_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_SWITCH, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_ng_ltrhold_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NG_LTRHOLD, val);	
...
static ssize_t audio_proc_aec_ng_utrhold_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NG_UTRHOLD, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_ng_rel_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NG_REL, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_ng_att_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NG_ATT, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_ng_inv_rel_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NG_INV_REL, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_ng_inv_att_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NG_INV_ATT, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_ng_ht_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_NG_HT, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_avgr_threshold_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_RER_THR, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_sample_rate_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_SAMPLE_RATE, val);	
	
exit:
    return count;
}

static ssize_t audio_proc_aec_log_write(struct file *file, const char __user *buffer,
                                                  size_t count, loff_t *ppos)
{
    char value_str[32] = {'\0'};
	unsigned int val;
	...
	value_str[count] = '\0';
    sscanf(value_str, "%d\n", &val);
    au_vg_set_aec_info(AEC_LOG, val);	
...
#ifdef USE_AEC
static int audio_proc_aec_pw_threshold_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_PW_THRESHOLD);
    seq_printf(sfile, "aec power threshold :%d\n", value);

    return 0;
}

static int audio_proc_aec_nlp_min_gain_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_NLP_MIN_GAIN);
    seq_printf(sfile, "nlp_min_gain :%d\n", value);

    return 0;
}

static int audio_proc_aec_nlp_max_gain_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_NLP_MAX_GAIN);
    seq_printf(sfile, "nlp_max_gain:%d\n", value);

    return 0;
}

static int audio_proc_aec_nlp_step_gain_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_NLP_STEP_GAIN);
    seq_printf(sfile, "nlp_step_gain:%d\n", value);

    return 0;
}

static int audio_proc_aec_nlp_threshold_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_FAR_RSHIFT);
    seq_printf(sfile, "nlp_threshold:%d\n", value);

    return 0;
}

static int audio_proc_aec_ng_enable_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_NG_ENABLE);
    seq_printf(sfile, "ng_enable:%d\n", value);

    return 0;
}

static int audio_proc_aec_farend_RShift_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_FAR_RSHIFT);
    seq_printf(sfile, "farend_RShift:%d\n", value);

    return 0;
}

static int audio_proc_aec_Pre_Proc_enable_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_PRE_PROC_ENABLE);
    seq_printf(sfile, "Pre_Proc_enable:%d\n", value);

    return 0;
}

static int audio_proc_aec_switch_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_SWITCH);
    seq_printf(sfile, "aec_switch:%d\n", value);

    return 0;
}

static int audio_proc_aec_ng_ltrhold_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_NG_LTRHOLD);
    seq_printf(sfile, "ng_ltrhold:%d\n", value);

    return 0;
}

static int audio_proc_aec_ng_utrhold_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_NG_UTRHOLD);
    seq_printf(sfile, "ng_utrhold:%d\n", value);

    return 0;
}

static int audio_proc_aec_ng_rel_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_NG_REL);
    seq_printf(sfile, "ng_rel:%d\n", value);

    return 0;
}

static int audio_proc_aec_ng_att_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_NG_ATT);
    seq_printf(sfile, "ng_att:%d\n", value);

    return 0;
}

static int audio_proc_aec_ng_inv_rel_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_NG_INV_REL);
    seq_printf(sfile, "ng_inv_rel:%d\n", value);

    return 0;
}

static int audio_proc_aec_ng_inv_att_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_NG_INV_ATT);
    seq_printf(sfile, "ng_inv_att:%d\n", value);

    return 0;
}

static int audio_proc_aec_ng_ht_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_NG_HT);
    seq_printf(sfile, "ng_ht:%d\n", value);

    return 0;
}

static int audio_proc_aec_avgr_threshold_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_RER_THR);
    seq_printf(sfile, "avgr_threshold:%d\n", value);

    return 0;
}

static int audio_proc_aec_sample_rate_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_SAMPLE_RATE);
    seq_printf(sfile, "sample_rate:%d\n", value);

    return 0;
}

static int audio_proc_aec_log_show(struct seq_file *sfile, void *v)
{
	unsigned int value = au_vg_get_aec_info(AEC_LOG);
    seq_printf(sfile, "log:%d\n", value);
...
#ifdef USE_AEC
static int audio_proc_aec_pw_threshold_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_pw_threshold_show, PDE(inode)->data);
}

static int audio_proc_aec_nlp_min_gain_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_nlp_min_gain_show, PDE(inode)->data);
}

static int audio_proc_aec_nlp_max_gain_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_nlp_max_gain_show, PDE(inode)->data);
}

static int audio_proc_aec_nlp_step_gain_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_nlp_step_gain_show, PDE(inode)->data);
}

static int audio_proc_aec_nlp_threshold_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_nlp_threshold_show, PDE(inode)->data);
}

static int audio_proc_aec_ng_enable_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_ng_enable_show, PDE(inode)->data);
}

static int audio_proc_aec_farend_RShift_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_farend_RShift_show, PDE(inode)->data);
}

static int audio_proc_aec_Pre_Proc_enable_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_Pre_Proc_enable_show, PDE(inode)->data);
}

static int audio_proc_aec_switch_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_switch_show, PDE(inode)->data);
}

static int audio_proc_aec_ng_ltrhold_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_ng_ltrhold_show, PDE(inode)->data);
}

static int audio_proc_aec_ng_utrhold_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_ng_utrhold_show, PDE(inode)->data);
}

static int audio_proc_aec_ng_rel_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_ng_rel_show, PDE(inode)->data);
}

static int audio_proc_aec_ng_att_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_ng_att_show, PDE(inode)->data);
}

static int audio_proc_aec_ng_inv_rel_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_ng_inv_rel_show, PDE(inode)->data);
}

static int audio_proc_aec_ng_inv_att_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_ng_inv_att_show, PDE(inode)->data);
}

static int audio_proc_aec_ng_ht_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_ng_ht_show, PDE(inode)->data);
}

static int audio_proc_aec_avgr_threshold_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_avgr_threshold_show, PDE(inode)->data);
}

static int audio_proc_aec_sample_rate_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_sample_rate_show, PDE(inode)->data);
}

static int audio_proc_aec_log_open(struct inode *inode, struct file *file)
{
    return single_open(file, audio_proc_aec_log_show, PDE(inode)->data);
}
#endif
...
#ifdef USE_AEC
static struct file_operations audio_proc_aec_pw_threshold_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_pw_threshold_open,
    .write  = audio_proc_aec_pw_threshold_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_nlp_min_gain_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_nlp_min_gain_open,
    .write  = audio_proc_aec_nlp_min_gain_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_nlp_max_gain_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_nlp_max_gain_open,
    .write  = audio_proc_aec_nlp_max_gain_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_nlp_step_gain_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_nlp_step_gain_open,
    .write  = audio_proc_aec_nlp_step_gain_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_nlp_threshold_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_nlp_threshold_open,
    .write  = audio_proc_aec_nlp_threshold_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_ng_enable_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_ng_enable_open,
    .write  = audio_proc_aec_ng_enable_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_farend_RShift_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_farend_RShift_open,
    .write  = audio_proc_aec_farend_RShift_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_Pre_Proc_enable_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_Pre_Proc_enable_open,
    .write  = audio_proc_aec_Pre_Proc_enable_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_switch_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_switch_open,
    .write  = audio_proc_aec_switch_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_ng_ltrhold_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_ng_ltrhold_open,
    .write  = audio_proc_aec_ng_ltrhold_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_ng_utrhold_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_ng_utrhold_open,
    .write  = audio_proc_aec_ng_utrhold_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_ng_rel_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_ng_rel_open,
    .write  = audio_proc_aec_ng_rel_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};
...
static struct file_operations audio_proc_aec_ng_att_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_ng_att_open,
    .write  = audio_proc_aec_ng_att_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_ng_inv_rel_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_ng_inv_rel_open,
    .write  = audio_proc_aec_ng_inv_rel_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_ng_inv_att_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_ng_inv_att_open,
    .write  = audio_proc_aec_ng_inv_att_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_ng_ht_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_ng_ht_open,
    .write  = audio_proc_aec_ng_ht_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_avgr_threshold_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_avgr_threshold_open,
    .write  = audio_proc_aec_avgr_threshold_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_sample_rate_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_sample_rate_open,
    .write  = audio_proc_aec_sample_rate_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations audio_proc_aec_log_ops = {
    .owner  = THIS_MODULE,
    .open   = audio_proc_aec_log_open,
    .write  = audio_proc_aec_log_write,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};
...
#ifdef USE_AEC
	/* audio aec info */
	audio_proc_aec_root = audio_proc_create_entry("au_aec", S_IFDIR|S_IRUGO|S_IXUGO, NULL);	
    if(!audio_proc_aec_root) {
        printk("create proc node 'au_aec' failed!\n");
        ret = -EINVAL;
        goto err_tab;
    }
	
    /* AEC_PW_THRESHOLD */
    audio_proc_aec_pw_threshold = audio_proc_create_entry("aec_pw_threshold", S_IRUGO|S_IXUGO, audio_proc_aec_root);
    if(!audio_proc_aec_pw_threshold) {
        printk("create proc node 'aec_pw_threshold' failed!\n");
        ret = -EINVAL;
        goto err_aec_root;
    }
    audio_proc_aec_pw_threshold->proc_fops = &audio_proc_aec_pw_threshold_ops;

    /* aec_nlp_min_gain */
    audio_proc_aec_nlp_min_gain = audio_proc_create_entry("aec_nlp_min_gain", S_IRUGO|S_IXUGO, audio_proc_aec_root);
    if(!audio_proc_aec_nlp_min_gain) {
        printk("create proc node 'aec_nlp_min_gain' failed!\n");
        ret = -EINVAL;
        goto err_aec_root;
    }
    audio_proc_aec_nlp_min_gain->proc_fops = &audio_proc_aec_nlp_min_gain_ops;
	
    /* aec_nlp_max_gain */
    audio_proc_aec_nlp_max_gain = audio_proc_create_entry("aec_nlp_max_gain", S_IRUGO|S_IXUGO, audio_proc_aec_root);
    if(!audio_proc_aec_nlp_max_gain) {
        printk("create proc node 'aec_nlp_max_gain' failed!\n");
        ret = -EINVAL;
        goto err_aec_root;
    }
    audio_proc_aec_nlp_max_gain->proc_fops = &audio_proc_aec_nlp_max_gain_ops;

	/* aec_nlp_step_gain */
	audio_proc_aec_nlp_step_gain = audio_proc_create_entry("aec_nlp_step_gain", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_nlp_step_gain) {
		printk("create proc node 'aec_nlp_step_gain' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_nlp_step_gain->proc_fops = &audio_proc_aec_nlp_step_gain_ops;

	/* aec_nlp_threshold */
	audio_proc_aec_nlp_threshold = audio_proc_create_entry("aec_nlp_threshold", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_nlp_threshold) {
		printk("create proc node 'aec_nlp_threshold' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_nlp_threshold->proc_fops = &audio_proc_aec_nlp_threshold_ops;

	/* aec_ng_enable */
	audio_proc_aec_ng_enable = audio_proc_create_entry("aec_ng_enable", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_ng_enable) {
		printk("create proc node 'aec_ng_enable' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_ng_enable->proc_fops = &audio_proc_aec_ng_enable_ops;

	/* aec_farend_RShift */
	audio_proc_aec_farend_RShift = audio_proc_create_entry("aec_farend_RShift", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_farend_RShift) {
		printk("create proc node 'aec_farend_RShift' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_farend_RShift->proc_fops = &audio_proc_aec_farend_RShift_ops;

	/* aec_Pre_Proc_enable */
	audio_proc_aec_Pre_Proc_enable = audio_proc_create_entry("aec_Pre_Proc_enable", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_Pre_Proc_enable) {
		printk("create proc node 'aec_Pre_Proc_enable' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_Pre_Proc_enable->proc_fops = &audio_proc_aec_Pre_Proc_enable_ops;

	audio_proc_aec_switch = audio_proc_create_entry("aec_switch", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_switch) {
		printk("create proc node 'aec_switch' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_switch->proc_fops = &audio_proc_aec_switch_ops;
				
	/* aec_ng_ltrhold */
	audio_proc_aec_ng_ltrhold = audio_proc_create_entry("aec_ng_ltrhold", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_ng_ltrhold) {
		printk("create proc node 'aec_ng_ltrhold' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_ng_ltrhold->proc_fops = &audio_proc_aec_ng_ltrhold_ops;

	/* aec_ng_utrhold  */
	audio_proc_aec_ng_utrhold = audio_proc_create_entry("aec_ng_utrhold", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_ng_utrhold) {
		printk("create proc node 'aec_ng_utrhold' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_ng_utrhold ->proc_fops = &audio_proc_aec_ng_utrhold_ops;

	/* aec_ng_rel */
	audio_proc_aec_ng_rel = audio_proc_create_entry("aec_ng_rel", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_ng_rel) {
		printk("create proc node 'aec_ng_rel' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_ng_rel->proc_fops = &audio_proc_aec_ng_rel_ops;	

	/* aec_ng_att */
	audio_proc_aec_ng_att = audio_proc_create_entry("aec_ng_att", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_ng_att) {
		printk("create proc node 'aec_ng_att' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_ng_att->proc_fops = &audio_proc_aec_ng_att_ops;	
	
	/* aec_ng_inv_rel */
	audio_proc_aec_ng_inv_rel = audio_proc_create_entry("aec_ng_inv_rel", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_ng_inv_rel) {
		printk("create proc node 'aec_ng_inv_rel' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_ng_inv_rel->proc_fops = &audio_proc_aec_ng_inv_rel_ops;	

	/* aec_ng_inv_att  */
	audio_proc_aec_ng_inv_att = audio_proc_create_entry("aec_ng_inv_att", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_ng_inv_att) {
		printk("create proc node 'aec_ng_inv_att' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_ng_inv_att ->proc_fops = &audio_proc_aec_ng_inv_att_ops;

	/* aec_ng_ht */
	audio_proc_aec_ng_ht = audio_proc_create_entry("aec_ng_ht", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_ng_ht) {
		printk("create proc node 'aec_ng_ht' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_ng_ht->proc_fops = &audio_proc_aec_ng_ht_ops;	

	/* aec_avgr_thr */
	audio_proc_aec_avgr_threshold = audio_proc_create_entry("aec_avgr_threshold", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_avgr_threshold) {
		printk("create proc node 'aec_ng_rel' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_avgr_threshold->proc_fops = &audio_proc_aec_avgr_threshold_ops;		

	/* aec_sample_rate */
	audio_proc_aec_sample_rate = audio_proc_create_entry("aec_sample_rate", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_sample_rate) {
		printk("create proc node 'aec_ng_rel' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_sample_rate->proc_fops = &audio_proc_aec_sample_rate_ops;		

	/* aec_log */
	audio_proc_aec_log = audio_proc_create_entry("aec_log", S_IRUGO|S_IXUGO, audio_proc_aec_root);
	if(!audio_proc_aec_log) {
		printk("create proc node 'aec_ng_rel' failed!\n");
		ret = -EINVAL;
		goto err_aec_root;
	}
	audio_proc_aec_log->proc_fops = &audio_proc_aec_log_ops;	
#endif	
...
#ifdef USE_AEC
err_aec_root:
    audio_proc_remove_entry(NULL, audio_proc_aec_root);
#endif
...
void audio_proc_remove(void)
{
	...
	if (audio_proc_aec_root) {		
        if(audio_proc_aec_pw_threshold)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_pw_threshold);
        if(audio_proc_aec_nlp_min_gain)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_nlp_min_gain);		
        if(audio_proc_aec_nlp_max_gain)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_nlp_max_gain);
        if(audio_proc_aec_nlp_step_gain)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_nlp_step_gain);
        if(audio_proc_aec_nlp_threshold)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_nlp_threshold);
        if(audio_proc_aec_ng_enable)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_ng_enable);            
        if(audio_proc_aec_farend_RShift)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_farend_RShift);
        if(audio_proc_aec_Pre_Proc_enable)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_Pre_Proc_enable); 
        if(audio_proc_aec_switch)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_switch);                                   
        if(audio_proc_aec_ng_ltrhold)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_ng_ltrhold);
        if(audio_proc_aec_ng_utrhold)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_ng_utrhold);
        if(audio_proc_aec_ng_rel)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_ng_rel);
        if(audio_proc_aec_ng_att)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_ng_att);
        if(audio_proc_aec_ng_inv_rel)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_ng_inv_rel);
        if(audio_proc_aec_ng_inv_att)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_ng_inv_att);
        if(audio_proc_aec_ng_ht)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_ng_ht);
        if(audio_proc_aec_avgr_threshold)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_avgr_threshold);
        if(audio_proc_aec_sample_rate)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_sample_rate);
        if(audio_proc_aec_log)
            audio_proc_remove_entry(audio_proc_aec_root, audio_proc_aec_log);
		
        audio_proc_remove_entry(NULL, audio_proc_aec_root);
        audio_proc_aec_root = NULL;
    }
#endif	
}
