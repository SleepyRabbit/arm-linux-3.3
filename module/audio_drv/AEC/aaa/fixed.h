...
**
** $Id: fixed.h,v 1.1 2016/02/04 05:34:09 sainthuang Exp $
**/
...
#else
	#define FRAC_SIZE 16 /* frac is a 32 bit integer */
	#define FRAC_BITS 15
	#define FRAC_PRECISION ((uint32_t)(1 << FRAC_BITS))
	#define FRAC_MAX 0x7FFF
#endif
...
#if 0
	tmp64 = (__int64)x1*c1 + (__int64)x2*c2;
	tmp = (int)(tmp64>>32);
	*y1 = tmp<<(FRAC_SIZE-FRAC_BITS);

	tmp64 = (__int64)x2*c1 - (__int64)x1*c2;
	tmp = (int)(tmp64>>32);
	*y2 = tmp<<(FRAC_SIZE-FRAC_BITS);
#else
	tmp64 = (__int64)x1*c1 + (__int64)x2*c2;
	tmp = (int)(tmp64>>16);
	*y1 = tmp<<(FRAC_SIZE-FRAC_BITS);
    ...
    real_t x1, real_t x2, real_t c1, real_t c2)
{
    ...
    __asm
    {
       SMULL tmp0,tmp1, x1,c1;
       SMLAL tmp0,tmp1, x2,c2;
    }
 #else
    asm volatile("smull %[tmp0], %[tmp1], %[x1], %[c1]"
    	:[tmp0]"+r"(tmp0), [tmp1]"+r"(tmp1)
    	: [x1]"r"(x1), [c1]"r"(c1)
    );
    asm volatile("smlal %[tmp0], %[tmp1], %[x2], %[c2]"
    	:[tmp0]"+r"(tmp0), [tmp1]"+r"(tmp1)
    	:[x2]"r"(x2), [c2]"r"(c2)
    );
 #endif   
    //*y1 = tmp1<<(FRAC_SIZE-FRAC_BITS);
    //*y1 = ((tmp1<<16)|(tmp0>>16))<<(FRAC_SIZE-FRAC_BITS);
      *y1 = (tmp1<<17)|(tmp0>>15);
    ...
    __asm
    {
       SMULL tmp0,tmp1, x2,c1;
       SMLAL tmp0,tmp1, x1,c2;
    }
#else
    asm volatile("smull %[tmp0], %[tmp1], %[x2], %[c1]"
    		:[tmp0]"+r"(tmp0), [tmp1]"+r"(tmp1)
    		: [x2]"r"(x2), [c1]"r"(c1)
    );
    asm volatile("smlal %[tmp0], %[tmp1], %[x1], %[c2]"
    		:[tmp0]"+r"(tmp0), [tmp1]"+r"(tmp1)
    		: [x1]"r"(x1), [c2]"r"(c2)
    );
#endif    
    //*y2 = tmp1<<(FRAC_SIZE-FRAC_BITS);
    *y2 = (tmp1<<17)|(tmp0>>15);  
...
/*
    ...
    unsigned int acc0;
  	...
  	asm volatile("smull %[o0], %[o1], %[in0], %[in0]"
  		:[o0]"+r"(acc0), [o1]"+r"(acc1)
  		:[in0] "r" (X[i].real)
  	); 
  	asm volatile("smlal %[o0], %[o1], %[in0], %[in0]"
  		:[o0]"+r"(acc0), [o1]"+r"(acc1)
  		:[in0] "r" (X[i].img)
  	);    	
*/  	
    ...
    real_t x1, real_t x2, real_t c1, real_t c2)
{
 ...
 #else
    asm volatile("SMULL %[tmp0], %[tmp1], %[x1], %[c1]": [tmp0] "=r" (tmp0): [tmp1] "=r" (tmp1): [x1]"r"(x1): [c1]"r"(c1));
    asm volatile("SMLAL %[tmp0], %[tmp1], %[x2], %[c2]": [tmp0] "=r" (tmp0): [tmp1] "=r" (tmp1): [x2]"r"(x2): [c2]"r"(c2));
 #endif   
    //*y1 = tmp1<<(FRAC_SIZE-FRAC_BITS);
    *y1 = ((tmp1<<16)|(tmp0>>16))<<(FRAC_SIZE-FRAC_BITS);
...
#else
    asm volatile("SMULL %[tmp0], %[tmp1], %[x2], %[c1]": [tmp0] "=r" (tmp0): [tmp1] "=r" (tmp1): [x2]"r"(x2): [c1]"r"(c1));
    asm volatile("SMLAL %[tmp0], %[tmp1], %[x1], %[c2]": [tmp0] "=r" (tmp0): [tmp1] "=r" (tmp1): [x1]"r"(x1): [c2]"r"(c2));
#endif    
    //*y2 = tmp1<<(FRAC_SIZE-FRAC_BITS);
    *y2 = ((tmp1<<16)|(tmp0>>16))<<(FRAC_SIZE-FRAC_BITS);    
