...
**
** $Id: structs.h,v 1.1 2016/02/04 05:34:10 sainthuang Exp $
**/
...
typedef struct
{
    ...
    uint8_t exclude_mask[MAX_channel];
    uint8_t additional_excluded_chns[MAX_channel];
...
typedef struct NeAACDecFrameInfo
{
    ...
    /*uint8_t*/ unsigned char channel_position[MAX_channel];
...
typedef struct
{
    ...
    */
    ...
    uint8_t window_shape_prev[MAX_channel];
#ifdef LTP_DEC
    uint16_t ltp_lag[MAX_channel];
#endif
    fb_info *fb;
    drc_info *drc;

    real_t *time_out[MAX_channel];
    real_t *fb_intermed[MAX_channel];
...
#ifdef SSR_DEC
    real_t *ssr_overlap[MAX_channel];
    real_t *prev_fmd[MAX_channel];
    real_t ipqf_buffer[MAX_channel][4][96/4];
#endif

#ifdef MAIN_DEC
    pred_state *pred_stat[MAX_channel];
#endif
#ifdef LTP_DEC
    int16_t *lt_pred_stat[MAX_channel];
#endif
    ...
    uint8_t pce_set;
    program_config pce;
    uint8_t element_id[MAX_channel];
    uint8_t internal_channel[MAX_channel];
