...
int FJpegEE_Tri(void *enc_handle, FJPEG_ENC_FRAME * ptFrame)
{
    ...
    cinfo->pub.max_bs_thd = ptFrame->max_bs_thd;
    #endif
...
#if defined(SEQ)
    bs_length =  compress_YUVsEnd(cinfo);
#elif defined(TWO_P_EXT)
    bs_length = compress_YUVeEnd(cinfo);
#else
    bs_length = compress_YUV(&cinfo->pub, cinfo->cur_comp_info);
#endif
#ifdef MJE_HANDLE_BITSTREAM_OVERFLOW
    // sync from node
    if (MJE_BITSTREAM_FULL_STATE == cinfo->pub.max_bs_thd) {
        cinfo->pub.enc_out_size = bs_length;
        bs_length = MJE_BITSTREAM_FULL_STATE;
    }
#endif
...
#ifdef MJE_HANDLE_BITSTREAM_OVERFLOW
int FJpegEE_GetStopMB(void *enc_handle, int *stop_mb, int *total_mb, unsigned int *enc_out_size)
{
    struct jpeg_compress_struct *cinfo = (struct jpeg_compress_struct *)enc_handle;
    int width = cinfo->pub.roi_right_mb_x - cinfo->pub.roi_left_mb_x;
    int height = cinfo->pub.roi_right_mb_y - cinfo->pub.roi_left_mb_y;

    *stop_mb = cinfo->pub.stop_y * width + cinfo->pub.stop_x;
    *total_mb = width*height;
    *enc_out_size = cinfo->pub.enc_out_size;
    return 0;
}
#endif
