...
unsigned int compress_YUVeEnd (j_compress_ptr cinfo)
{
	...
	if (je_int & (1L << 31)) {	// embedded CPU frame done
        ...
        cinfo->pub.stop_x = node->pub.stop_x;
        cinfo->pub.stop_y = node->pub.stop_y;
        cinfo->pub.max_bs_thd = node->pub.max_bs_thd;
    #endif
		return node->bitslen;
	}
...
unsigned int compress_YUV (jpeg_compress_struct_my *my,  jpeg_component_info * jci[MAX_COMPS_IN_SCAN])
{
...
#ifdef MJE_HANDLE_BITSTREAM_OVERFLOW
    unsigned int addr_thd = 0;
    unsigned int force_stop = 0;

    if (my->max_bs_thd > 0)
        addr_thd = my->max_bs_thd + my->bs_phy;
#endif
    ...
    #ifdef MJE_HANDLE_BITSTREAM_OVERFLOW
        // stop addr
        if (addr_thd > 0 && !force_stop) {
            unsigned int bs_addr;
            READ_ABADR(BASE_ADDRESS, bs_addr);
            if (bs_addr > addr_thd) {
                force_stop = 1;
                my->stop_x = mbx;
                my->stop_y = mby;
                mbx = my->roi_left_mb_x;
                mby = my->roi_right_mb_y;
                my->max_bs_thd = MJE_BITSTREAM_FULL_STATE;
            }
        }
    #endif
		if (pipe == 0)
