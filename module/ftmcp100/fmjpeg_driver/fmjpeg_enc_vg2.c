...
#ifdef MJE_HANDLE_BITSTREAM_OVERFLOW
    unsigned int mje_terminate_method = 1;      // 0: disable, 1: factor, 2: fix length
    unsigned int mje_early_over_detect_factor = 90;
    unsigned int mje_bitstream_limit = 200000;
#endif
...
#ifdef SUPPORT_VG2
    mje_print_property(job,job->in_prop.p, 0);
#else
...
#ifdef SUPPORT_VG2
    mje_print_property(job, job->out_prop.p, 1);
#else
...
#ifdef MJE_HANDLE_BITSTREAM_OVERFLOW
    extern int FJpegEE_GetStopMB(void *enc_handle, int *stop_mb, int *total_mb, unsigned int *enc_out_size);
#endif
...
int fmje_encode_trigger(struct job_item_t *job_item)
{
    ...
    if (encode_data->updated) {
        ...
        if (MJE_REINIT & encode_data->updated) {
            ...
            if (fmje_create(encode_data, &encp, encode_data->reinit) < 0)
                return -1;
		    // Tuba 20150120: force update rate control
        ...
        else if (MJE_RC_UPDATE & encode_data->updated) {
            if (fmje_init_rc(encode_data) < 0)
                encode_data->enable_rc = 0;
            else
                encode_data->enable_rc = 1;
	...
	*/
    ...
    if (0 == job->out_buf.buf[0].addr_pa) {
	...
	*/
    ...
    #ifdef MJE_HANDLE_BITSTREAM_OVERFLOW
        if (encode_data->force_quality > 0) {
            struct rc_frame_info_t rc_data;
            rc_data.force_quality = encode_data->force_quality;
            encode_data->cur_quality = 
            encf.u32ImageQuality = mje_rc_dev->rc_get_quant(encode_data->rc_handler, &rc_data);
            encode_data->force_quality = -1;
        }
        else {
            encode_data->cur_quality = 
    ...
    #ifdef MJE_HANDLE_BITSTREAM_OVERFLOW
        if (encode_data->force_quality > 0) {
            encode_data->cur_quality = encf.u32ImageQuality = encode_data->force_quality;
            encode_data->force_quality = -1;
        }
        else {
            encode_data->cur_quality = 
...
#ifdef MJE_HANDLE_BITSTREAM_OVERFLOW
    if (1 == mje_terminate_method)
        encf.max_bs_thd = job->out_buf.buf[0].size * mje_early_over_detect_factor/100;
    else if (2 == mje_terminate_method)
        encf.max_bs_thd = mje_bitstream_limit;
    else
        encf.max_bs_thd = 0;
#endif
	//if (dump_n_frame)
...
/*
average:
    162.91 (1~10)   183.94 (10~20)  205.25 (20~30)  220.13 (30~40)  225.43 (40~50)
    228.13 (50~60)  217.70 (60~70)  205.70 (70~80)  173.84 (80~90)  100.87 (90~100)
worse:
    [0]212.84 (10->1)   [1]210.51 (20->10)  [2]222.25 (30->20)  [3]229.17 (40->30)  [4]231.72 (50->40)
    [5]233.91 (60->50)  [6]222.93 (70->60)  [7]210.78 (80->70)  [8]184.32 (90->80)  [9]110.94 (100->90)
*/
static int mje_update_overflow_quant(int quant, int stop_pos, int total_mb)
{
    static int qp_ratio[12] = {
        213, 211, 223, 230, 232,
        234, 223, 211, 185, 111, 111 };
    int update_quant;
    unsigned int factor = (total_mb * 256 + 0x80) / stop_pos;

    for (update_quant = quant; update_quant >= 1; update_quant-=10) {
        factor = (factor * qp_ratio[(update_quant-1)/10] + 0x80) / 256;
        if (factor < 256)
            break;
    }
    return (update_quant-10 >= 1 ? update_quant-10 : 1);
}
#endif
...
#ifdef MJE_HANDLE_BITSTREAM_OVERFLOW
    if (MJE_BITSTREAM_FULL_STATE == bs_len) {
        // force quant update
        if (encode_data->cur_quality > encode_data->min_quant) {
            unsigned int stop_mb, total_mb, enc_out_size;
            FJpegEE_GetStopMB(encode_data->handler, &stop_mb, &total_mb, &enc_out_size);
            if (enc_out_size > encode_data->out_size) {
                DEBUG_E("{chn%d} jpg enc bs overflow (%d > %d)\n", 
                    encode_data->chn, enc_out_size, encode_data->out_size);
                damnit(MJE_MODULE_NAME);
            }
            encode_data->force_quality = mje_update_overflow_quant(encode_data->cur_quality, stop_mb, total_mb);
            DEBUG_M(LOG_WARNING, "{chn%d} jpeg overflow stop mb %d / %d, force quant %d (cur%d), (bs %d, buf %d)\n", 
                    encode_data->chn, stop_mb, total_mb, encode_data->force_quality, encode_data->cur_quality,
                    enc_out_size, encode_data->out_size);
            //mcp100_debug("{chn%d} jpeg overflow stop mb %d / %d, force quant %d (%d), (bs %d, buf %d)\n", 
            //        encode_data->chn, stop_mb, total_mb, encode_data->force_quality, encode_data->cur_quality,
            //        enc_out_size, encode_data->out_size);
            fmje_encode_trigger(job_item);
...
#ifndef MJE_HANDLE_BITSTREAM_OVERFLOW
...
#ifdef SUPPORT_VG2
static int mje_stop(unsigned int fd)
#else
