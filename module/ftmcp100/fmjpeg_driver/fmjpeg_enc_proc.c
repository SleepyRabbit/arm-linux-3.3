...
static struct proc_dir_entry *mje_dbg_proc = NULL;
#ifdef SUPPORT_VG
...
extern unsigned int mje_engine_time;
    ...
    extern unsigned int mje_terminate_method;
    extern unsigned int mje_early_over_detect_factor;
    extern unsigned int mje_bitstream_limit;
#endif
...
static int proc_dbg_write_mode(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    switch (cmd_idx) {
        ...
        case 1: 
            if (value >= 0 && value < 3)
                mje_terminate_method = value;
            else
                mje_terminate_method = 0;
            break;
        case 2:
            if (value > 0 && value < 100)
                mje_early_over_detect_factor = value;
            else
                mje_early_over_detect_factor = 0;
            break;
        case 3:
            if (value > 0)
                mje_bitstream_limit = value;
            else
                mje_bitstream_limit = 0;
            break;
    #endif
        default:
            break;
...
{
    if (mje_dbg_proc != 0)
        remove_proc_entry(mje_dbg_proc->name, mje_proc);
    if (mje_info_proc != 0)
...
int mje_proc_init(void)
{
    ...
    mje_dbg_proc = create_proc_entry("dbg", S_IRUGO | S_IXUGO, mje_proc);
    if (mje_dbg_proc == NULL) {
        printk("Error to create driver proc %s/dbg\n", MJE_PROC_DIR_NAME);
        goto mje_init_proc_fail;
    }
    mje_dbg_proc->read_proc = (read_proc_t *)proc_dbg_read_mode;
    mje_dbg_proc->write_proc = (write_proc_t *)proc_dbg_write_mode;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,28))
    mje_dbg_proc->owner = THIS_MODULE;
#endif
