...
extern unsigned int mp4_dec_max_chn;
...
/* variable */
struct mp4d_data_t      *mp4d_private_data = NULL;
...
static int fmp4d_decode_Tri(struct job_item_t *job_item)
{
    ...
    chn = job_item->chn;
    decode_data = mp4d_private_data + chn;
    ...
    tResult.u32BufferYSize = EM_PARAM_WIDTH(decode_data->dst_bg_dim)*EM_PARAM_HEIGHT(decode_data->dst_bg_dim);
#endif
/*
	if (dump_n_frame)
...
{
	int ret;
	struct mp4d_data_t *decode_data = mp4d_private_data + job_item->chn;
...
int mp4d_drv_init(void)
{
...
#ifndef MULTI_FORMAT_DECODER
    mpeg4_dec_entity.minors = mp4_dec_max_chn;
    ...
    mp4d_property_record = kzalloc(sizeof(struct mp4d_property_record_t)*mp4_dec_max_chn,GFP_KERNEL);
    mp4d_private_data = kzalloc(sizeof(struct mp4d_data_t)*mp4_dec_max_chn, GFP_KERNEL);
    if (NULL == mp4d_private_data) {
        DEBUG_E("Fail to allocate mpg4 dec private data\n");
        goto mp4d_vg_init_fail;
    }
    memset(mp4d_private_data, 0, sizeof(struct mp4d_data_t)*mp4_dec_max_chn);
    
    for (chn = 0; chn < mp4_dec_max_chn; chn++) {
        decode_data = mp4d_private_data + chn;
        decode_data->engine = 0;
        decode_data->chn = chn;
    #ifdef REF_POOL_MANAGEMENT
        decode_data->res_pool_type = -1;
        decode_data->recon_buf = &decode_data->ref_buffer[0];
        decode_data->refer_buf = &decode_data->ref_buffer[1];            
    #endif
    }
    
    if (mcp100_register(&mp4d_dev, TYPE_MPEG4_DECODER, mp4_dec_max_chn) < 0)
        goto mp4d_vg_init_fail;
...
mp4d_vg_init_fail:
    ...
    if (mp4d_private_data)
        kfree(mp4d_private_data);
    return 0;
} 
