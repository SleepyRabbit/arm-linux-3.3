...
//printk("%d: hw start %x\n", frame_cnt, hw_start);
    ...
    pEnc->max_bit_length = pFrame->max_bit_length;
#endif
...
#ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
    if (STATUS_OVERFLOW == pEnc->max_bit_length) {
        //printk("bs length = 0x%x\n", bs_sz);
        pEnc->enc_out_size = bs_sz;
        SWAP(pEnc->current1, pEnc->reference);  // becasuse swap @ trigger process
...
#ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
int ee_GetStopXY(Encoder_x *pEnc_x, unsigned int *x, unsigned int *y, unsigned int *enc_size)
{
    Encoder *pEnc = &(pEnc_x->Enc);
    *x = pEnc->stop_x;
    *y = pEnc->stop_y;
    *enc_size = pEnc->enc_out_size;    
    return 0;
}
#endif
