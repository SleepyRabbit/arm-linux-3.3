...
void encoder_iframe(Encoder_x * pEnc_x)
{
    ...
    uint32_t u32AddressThd;// = pEnc->bitstream + pEnc->max_bit_length;
    uint8_t bForceStop = 0;
    if (pEnc->max_bit_length)
        u32AddressThd = (uint32_t)pEnc->bitstream + pEnc->max_bit_length;
    else
        u32AddressThd = 0;
    #endif
...
//~~change to next mb~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        #ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
        if (bForceStop) {
            //pEnc->stop_x = mbb_mc->x;
            //pEnc->stop_y = mbb_mc->y;
            mbb_img->x = 0;
            mbb_img->y = pEnc->mb_height;
        }
        else
        #endif
        {
...
// wait for  DMA_done
    ...
    #ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
        if (u32AddressThd && 0 == bForceStop) {
            if (ptMP4->ASADR >= u32AddressThd) {
                bForceStop = 1;
                pEnc->max_bit_length = STATUS_OVERFLOW;
                pEnc->stop_x = mbb_img->x;
                pEnc->stop_y = mbb_img->y;
            }
        }
    #endif
		if (u32pipe == 0)
