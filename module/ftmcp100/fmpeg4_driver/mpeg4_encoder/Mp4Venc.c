...
extern int ee_GetStopXY(Encoder_x *pEnc_x, unsigned int *x, unsigned int *y, unsigned int *enc_size);
int FMpeg4EE_GetStopXY(void *enc_handle, unsigned int *x, unsigned int *y, unsigned int *enc_size)
{
    if (enc_handle)
        return ee_GetStopXY((Encoder_x*)enc_handle, x, y, enc_size);
    else
        return -1;
    return 0;
}
