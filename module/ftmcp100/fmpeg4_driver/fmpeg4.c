...
//#endif
unsigned int mp4_enc_max_chn = MP4E_MAX_CHANNEL;
unsigned int mp4_dec_max_chn = 0;//MP4D_MAX_CHANNEL;
module_param(mp4_enc_max_chn, uint, S_IRUGO|S_IWUSR);
module_param(mp4_dec_max_chn, uint, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(mp4_enc_max_chn, "mpeg4 enc max channel");
MODULE_PARM_DESC(mp4_dec_max_chn, "mpeg4 dec max channel");
...
#ifdef REF_POOL_MANAGEMENT
    if (init_mem_pool(mp4_max_width, mp4_max_height, mp4_enc_max_chn, mp4_dec_max_chn) < 0)
        return -1;
#endif
#ifdef MPEG4_COMMON_PRED_BUFFER // Tuba 20131108: use common pred buffer
