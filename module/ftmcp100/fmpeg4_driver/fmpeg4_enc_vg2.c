...
#ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
    extern int FMpeg4EE_GetStopXY(void *enc_handle, unsigned int *x, unsigned int *y, unsigned int *enc_size);
#endif
...
extern unsigned int mp4_enc_max_chn;
...
/* variable */
struct mp4e_data_t  *mp4e_private_data = NULL;

struct rc_entity_t *mp4e_rc_dev = NULL;
unsigned int gOverflowReturnLength = 1024; // Tuba 20141113
#ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
    unsigned int mp4_terminate_method = 1;      // 0: disable, 1: factor, 2: fix length
    unsigned int mp4_early_over_detect_factor = 90;
    unsigned int mp4_bitstream_limit = 150000;
#endif
...
{
	struct mp4e_data_t *encode_data = mp4e_private_data + job_item->chn;
...
int fmp4e_encode_trigger(struct job_item_t *job_item)
{
	...
	*/
    ...
    if (mp4e_rc_dev && encode_data->rc_handler) {
    #ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
        if (encode_data->force_quant > 0) {
            struct rc_frame_info_t rc_data;
            rc_data.force_quant = encode_data->force_quant;
            encode_data->quant = mp4e_rc_dev->rc_get_quant(encode_data->rc_handler, &rc_data);
            encode_data->force_quant = -1;
            DEBUG_M(LOG_WARNING, "{chn%d} bs buf full, update quant %d\n", encode_data->chn, encode_data->quant);
        }
        else
    #endif
    ...
    #ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
        if (encode_data->force_quant > 0) {
            encode_data->quant = encode_data->force_quant;
            encode_data->force_quant = -1;
        }
        else
    #endif
...
//printk("input quant = %d\n", encf.quant);
    ...
    if (encode_data->motion_enable) {
        if (encode_data->mbinfo_length > encode_data->out_size) {
            DEBUG_E("{chn%d} motion buffer is over bitstream buffer (%d > %d)\n", 
                encode_data->chn, encode_data->mbinfo_length, encode_data->out_size);
            damnit(MP4E_MODULE_NAME);
        }
        encf.bitstream = (unsigned char *)(job->out_buf.buf[0].addr_pa + encode_data->mbinfo_length);
    #ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
        max_bs_length = encode_data->out_size - encode_data->mbinfo_length;
    #endif
    }
    else {
        encf.bitstream = (unsigned char *)(job->out_buf.buf[0].addr_pa + BS_OFFSET);
    #ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
        max_bs_length = encode_data->out_size;
    #endif
    }
#ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
    if (1 == mp4_terminate_method)
        encf.max_bit_length = max_bs_length*mp4_early_over_detect_factor/100;
    else if (2 == mp4_terminate_method)
        encf.max_bit_length = mp4_bitstream_limit;
    else
        encf.max_bit_length = 0;
#endif
...
/*  
average:
    153.07 183.83 199.47 208.32 215.18 220.36 225.02 228.55 
    231.63 233.53 235.73 236.98 238.63 239.65 240.97 241.61 
    242.65 243.25 244.25 244.47 245.44 245.65 246.34 246.09 
    246.54 247.02 247.45 247.75 247.96 248.42 
worse:
    172.80 203.43 215.73 222.24 226.92 230.15 232.77 234.76
    236.46 237.56 240.32 241.96 243.98 245.43 246.46 246.85
    248.16 248.52 249.45 249.68 250.45 250.66 251.20 250.76
    251.39 251.28 251.72 252.09 252.24 252.67 
*/
static int mp4e_update_overflow_qp(int qp, int stop_pos, int total_mb)
{
    static int qp_ratio[33] = {
        256, 
    #if 0
        160, 185, 200, 209, 216, 221, 225, 229, // 1~8
        232, 234, 236, 237, 239, 240, 241, 242, // 9~16
        243, 244, 245, 245, 246, 246, 247, 247, // 17~24
        247, 247, 248, 248, 248, 249            // 25~30
    #else
        173, 204, 216, 223, 227, 231, 233, 235, // 1~8
        237, 238, 241, 242, 244, 246, 247, 247, // 9~16
        248, 249, 250, 250, 251, 251, 251, 251, // 17~24
        251, 251, 252, 252, 252, 253
    #endif
        };
    int update_qp;
    unsigned int factor = (total_mb * 256 + 0x80) / stop_pos;

    //factor = factor * qp_ratio[qp] / 256;
    for (update_qp = qp; update_qp < 31; update_qp++) {
        factor = (factor * qp_ratio[update_qp] + 0x80) / 256;
        if (factor < 256)
            break;
    }
    return (update_qp + 1 > 31 ? 31 : update_qp + 1);
}
#endif
...
static int fmp4e_process_done (struct job_item_t *job_item)
{
...
#ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
    if (STATUS_OVERFLOW == bs_len) {
        if (encode_data->quant < encode_data->max_quant) {
            // restart
            unsigned int stop_x, stop_y, enc_size;
            FMpeg4EE_GetStopXY(encode_data->handler, &stop_x, &stop_y, &enc_size);
            if (enc_size > encode_data->out_size) {
                DEBUG_E("{chn%d} mpeg4 bs buffer overflow, %d > %d\n", 
                    encode_data->chn, enc_size, encode_data->out_size);
                damnit(MP4E_MODULE_NAME);
            }
            encode_data->force_quant = mp4e_update_overflow_qp(encode_data->quant, 
                stop_y*encode_data->frame_width/16 + stop_x, 
                encode_data->frame_width/16*encode_data->frame_height/16);
            DEBUG_M(LOG_WARNING, "{chn%d} mpeg4 overflow stop xy = %d x %d, force quant %d (%d) (bs%d, buf%d)\n", 
                encode_data->chn, stop_x, stop_y, encode_data->force_quant, encode_data->quant,
                enc_size, encode_data->out_size);
            //mcp100_debug("{chn%d} mpeg4 overflow stop xy = %d x %d, force quant %d (%d) (bs%d, buf%d)\n", 
            //    encode_data->chn, stop_x, stop_y, encode_data->force_quant, encode_data->quant,
            //    enc_size, encode_data->out_size);
            release_ref_buffer(encode_data->recon_buf, encode_data->chn);
            fmp4e_encode_trigger(job_item);
            return ENCODE_ERR_BS_OVERFLOW;
        }
        else {
            bs_len = 0;
            ret = ENCODE_ERR_BS_FAIL;            
        }
    }
#endif
    ...
    if (encode_data->motion_enable) {
        ...
        if (encode_data->bitstream_offset + encode_data->bitstream_size > encode_data->out_size) {
            DEBUG_E("MPEG4 Encoder Alert! Ch:%d: bs_length:0x%x over buf size:0x%x\n", 
                  encode_data->chn, encode_data->bitstream_size, encode_data->out_size);
            damnit(MP4E_MODULE_NAME);
            // set out property
            // postprocess
            //return -1;
...
int mp4e_drv_init(void)
{
    ...
    mp4e_entity.minors = mp4_enc_max_chn;
    video_entity_register(&mp4e_entity);
    init |= REGISTER_VG;

    spin_lock_init(&mpeg4_enc_lcok);

    mp4e_property_record = kzalloc(sizeof(struct mp4e_property_record_t) * mp4_enc_max_chn, GFP_KERNEL);
    if (mp4e_proc_init() < 0)
        goto mp4e_init_proc_fail;
    init |= INIT_PROC;

    mp4e_private_data = kzalloc(sizeof(struct mp4e_data_t) * mp4_enc_max_chn, GFP_KERNEL);
    if (NULL == mp4e_private_data) {
        DEBUG_E("Fail to allocate mpg4 enc private data\n");
        goto mp4e_init_proc_fail;
    }
    memset(mp4e_private_data, 0, sizeof(struct mp4e_data_t)*mp4_enc_max_chn);

    for (chn = 0; chn < mp4_enc_max_chn; chn++) {
        struct mp4e_data_t *encode_data = mp4e_private_data + chn;
        encode_data->engine = 0;
        encode_data->chn = chn;
    #ifdef REF_POOL_MANAGEMENT
        encode_data->res_pool_type = -1;
        encode_data->recon_buf = &encode_data->ref_buffer[0];
        encode_data->refer_buf = &encode_data->ref_buffer[1];
    #endif
    }

    if (mcp100_register(&mp4edv, TYPE_MPEG4_ENCODER, mp4_enc_max_chn) < 0)
        goto mp4e_init_proc_fail;
...
mp4e_init_proc_fail:
    ...
    if (mp4e_private_data)
        kfree(mp4e_private_data);
