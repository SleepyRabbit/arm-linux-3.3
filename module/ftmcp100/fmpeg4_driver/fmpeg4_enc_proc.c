...
static struct proc_dir_entry *mp4e_dbg_proc = NULL;
#ifdef SUPPORT_VG
...
extern unsigned int mp4e_engine_time;
    ...
    extern unsigned int mp4_terminate_method;
    extern unsigned int mp4_early_over_detect_factor;
    extern unsigned int mp4_bitstream_limit;
#endif
...
#ifdef MP4E_HANDLE_BITSTREAM_OVERFLOW
    len += sprintf(page+len, " 1   %5d  method. 0: disable, 1: factor, 2: fix length\n", mp4_terminate_method);
    len += sprintf(page+len, " 2   %5d  factor (base 100)\n", mp4_early_over_detect_factor);
    len += sprintf(page+len, " 3   %5d  fix length\n", mp4_bitstream_limit);
#endif
    *eof = 1;
    return len;
}
...
static int proc_dbg_write_mode(struct file *file, const char *buffer, unsigned long count, void *data)
{
    ...
    switch (cmd_idx) {
        ...
        case 1:
            if (value >= 0 && value < 3)
                mp4_terminate_method = value;
            else
                mp4_terminate_method = 0;
            break;
        case 2:
            if (value > 0 && value < 100)
                mp4_early_over_detect_factor = value;
            else
                mp4_early_over_detect_factor = 0;
            break;
        case 3:
            if (value > 0)
                mp4_bitstream_limit = value;
            else
                mp4_bitstream_limit = 0;
            break;
    #endif
        default:
            break;
...
{
    if (mp4e_dbg_proc != 0)
        remove_proc_entry(mp4e_dbg_proc->name, mp4e_proc);
    if (mp4e_info_proc != 0)
...
int mp4e_proc_init(void)
{
    ...
    mp4e_dbg_proc = create_proc_entry("dbg", S_IRUGO | S_IXUGO, mp4e_proc);
    if (mp4e_dbg_proc == NULL) {
        printk("Error to create driver proc %s/dbg\n", MP4E_PROC_DIR_NAME);
        goto mp4e_init_proc_fail;
    }
    mp4e_dbg_proc->read_proc = (read_proc_t *)proc_dbg_read_mode;
    mp4e_dbg_proc->write_proc = (write_proc_t *)proc_dbg_write_mode;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,28))
    mp4e_dbg_proc->owner = THIS_MODULE;
#endif
