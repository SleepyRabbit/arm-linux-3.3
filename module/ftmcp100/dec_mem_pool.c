...
static unsigned int mp4d_mem_max_chn = MP4D_MAX_CHANNEL;
...
int dec_dump_chn_pool(char *str)
{
    ...
    dmp_ref("=====  =======  =======   ============  ============  ==========");
    spin_lock_irqsave(&mcp100_pool_lock, flags);
    for (chn = 0; chn < mp4d_mem_max_chn; chn++) {
        cnt = 0;
...
int dec_init_mem_pool(int max_width, int max_height, unsigned int dec_max_chn)
{
    int l, i;
    int ddr_idx;
    ...
    chn_pool = kzalloc(sizeof(struct channel_ref_buffer_t) * mp4d_mem_max_chn, GFP_KERNEL);
    if (NULL == chn_pool) {
...
{
    int i, ret = 0;
    memset(checker, 0, sizeof(unsigned char)*mp4d_mem_max_chn);
