...
#endif
/*
* 0.0.1: first version of mjpeg rate control (20141021)
* 1.0.2: add force quality to handle bitstream overflow (20150123)
*/

#define MJE_RC_VERISON      0x10000200
...
int mje_ratecontrol_get_quant(void *ptHandler, struct rc_frame_info_t *rc_data)
{
    ...
    if (ptHandler) {
        ...
        if (rc_data && rc_data->force_quality >= 0)
            qp = RC_CLIP3(rc_data->force_quality, 1, 100);
        else
...
#ifdef DUMP_CUR_QP
    #ifdef DUMP_BITRATE
        len += sprintf(page+len, " chn   mode         fps          br(Kbs)    max.br    init.q  min.q  max.q  quality   frame       bit\n");
        len += sprintf(page+len, "=====  ======  ===============  =========  =========  ======  =====  =====  =======  =======  ==========\n");
    #else
        len += sprintf(page+len, " chn   mode         fps          br(Kbs)    max.br    init.q  min.q  max.q  quality\n");
        len += sprintf(page+len, "=====  ======  ===============  =========  =========  ======  =====  =====  =======\n");
    #endif
#else
    len += sprintf(page+len, " chn   mode         fps          br(Kbs)    max.br    init.q  min.q  max.q\n");
    len += sprintf(page+len, "=====  ======  ===============  =========  =========  ======  =====  =====\n");
#endif
