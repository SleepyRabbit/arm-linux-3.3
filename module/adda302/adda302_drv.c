...
#endif
int reback = 0;
int ssp1_rec_src = 0; // 0:from ADDA; 1:from external codec
...
int adda302_get_ssp1_rec_src(void)
{
    return ssp1_rec_src;
}
EXPORT_SYMBOL(adda302_get_ssp1_rec_src);
 ...
 */
static void _adda302_ssp_mclk_init(int ssp_idx)
{
    int ret = 0;

    if (chip_id == 0x8137 || chip_id == 0x8138 || chip_id == 0x8139) {
        if (ISMASTER == 1)
            ret = plat_set_ssp_clk(ssp_idx, 18000000, clk_pll2);
        else
...
static void adda302_ssp_mclk_init(void)
{
    ...
    // ssp0
    _adda302_ssp_mclk_init(0);

    // ssp1
    if (audio_out_enable[1] == 1)
        _adda302_ssp_mclk_init(1);        
...
static void adda302_da_select(void)
{
    ...
    if ((audio_out_enable[0] == 1) && (audio_out_enable[1] == 1)) {  // SSP1 source select DECODER(DA)
        ret = plat_set_ssp1_src(PLAT_SRC_DECODER);
        if (ret < 0)
            printk("%s set ssp1 source fail\n", __func__);
    }
#endif
...
static void adda302_select_ssp1_rec_source(void)
{
    if (ssp1_rec_src == 0)
        ...
        printk(KERN_ERR"[ADDA302] Error, unknown SSP1 rec source value %d!\n", ssp1_rec_src);
    }
...
EXPORT_SYMBOL(adda302_i2s_apply);

void adda302_power_cycle(const enum _adda302_power_cycle target)
{
    if (target == ADDA302_ADC_POWER_CYCLE) {
        adda302_set_ad_powerdown(1);
        mdelay(10);
        adda302_set_ad_powerdown(0);
    } else if (target == ADDA302_DAC_POWER_CYCLE) {
        adda302_set_da_powerdown(1);
        mdelay(10);
        adda302_set_da_powerdown(0);
    }
}
EXPORT_SYMBOL(adda302_power_cycle);
...
void adda302_set_ad_fs(const enum adda302_fs_frequency fs)
{
	...
	*(volatile unsigned int *)(base + 0x0) = tmp;
	/*
	  May not reset adda after sampling rate setting, 
	  it may affect ad when tx & rx are not at the same i2s. 
	*/
	//adda302_i2s_apply();
...
void adda302_set_da_fs(const enum adda302_fs_frequency fs)
{
    ...
    *(volatile unsigned int *)(base + 0x0) = tmp;
	/*
	  May not reset adda after sampling rate setting, 
	  it may affect da when tx & rx are not at the same i2s. 
	*/
	//adda302_i2s_apply();
...
static int __init adda302_drv_init(void)
{
    ...
    adda302_select_ssp1_rec_source();
...
static void adda302_drv_power_down(void)
{
	u32 base, tmp;
	base = priv.adda302_reg;
	/* 0x00 */
	tmp = *(volatile unsigned int *)(base + 0x0);
    tmp |= (0x1 << 18);            // disable ADC right frontend power
	tmp |= (0x1 << 19);            // disable ADC left frontend power
    tmp |= (0x1 << 20);            // disable ADC right power
    tmp |= (0x1 << 21);            // disable ADC left power
    tmp |= (0x1 << 22);            // disable DAC right power
    tmp |= (0x1 << 23);            // disable DAC left power
