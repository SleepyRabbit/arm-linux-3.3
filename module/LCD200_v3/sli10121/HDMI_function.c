...
int edid_check_extblock_header(unsigned char * edid_ext);

enum {
RGBTOYCC_SDTV_LIMIT_RANGE_ARRAY =0  ,
RGBTOYCC_SDTV_FULL_RANGE_ARRAY     ,
RGBTOYCC_HDTV_60HZ_RANGE_ARRAY     ,
RGBTOYCC_HDTV_50HZ_RANGE_ARRAY     ,
RGBTOYCC_MAX_ENRTRY_ARRAY
};
...
#ifdef KENNY_CHANGE
    I2C_ByteWrite (X63_PACKET_PB0, scan_mode); // PB0    /* overscan */
#else
    I2C_ByteWrite (X63_PACKET_PB0, 0x00); // PB0    /* overscan */
#endif
    I2C_ByteWrite (X64_PACKET_PB1, 0x10 | PB1_A0); // PB1
...
//    P2 = ((HDMI_STATE & 0x0F) | (P2 & 0xF0));
	...
	if(g_dump_edid){
		printk("HDMI_STATE_IDLE, HDMI_STATE_CHK = 0x%x, PowerMode: 0x%x \n", HDMI_STATE_CHK, PowerMode);
		printk("SLI HDMI_interrupt, MASK_92h = 0x%x, INT_94h = 0x%x-0x%x, 0xdf=0x%x\n"
		        ,I2C_ByteRead(X92_INT_MASK1), INT_94h,GM_INT_94h,I2C_ByteRead(XDF_HPG_STATUS));
	}
    switch (HDMI_STATE)
    {
        ...
        case HDMI_STATE_HOTPLUG:
            ...
            if (HDMI_hotplug_check())
            {
                ...
                //EDID_retry = EDID_RETRY_COUNTER; move to detect hotplug2
                EDID_current = 0;
                EDID_process_done = 0;
                RE_SETTING = 0;
        ...
        case HDMI_STATE_EDID_READ:
            ...
            else
            {
                ...
                /*check ext block one is ok or not*/
                if(EDID_current > 0 && edid_check_extblock_header(EDID_Data + (EDID_current * 0x80)) < 0){
                    printk("%s, EDID ext block error header(%d)\n", __func__,EDID_current);
                    EDID_retry--;
                    HDMI_System_PD_A();
                    HDMI_clear_to_IDLE();
                }
                else if(EDID_current > 0){
                    HDMI_STATE = HDMI_STATE_EDID_PROCESS;
                }
...
#if 0/*no support audio info frame*/					
                    if (vendor_edid > 5)
                    {
                        // ACP, ISRC packet Set
                        CtrlPkt_Enable |= 0x06;
...
BYTE HDMI_hotplug_check (void)
{
    ...
    if ((STAT_DFh & HPG_MSENS) == HPG_MSENS) {
        ...
        count = 0;
        return 1;
    } else if ((STAT_DFh & HPG_MSENS) == HOT_PLUG) {
        DelayMs (15);
        STAT_DFh = I2C_ByteRead (XDF_HPG_STATUS);

        if ((STAT_DFh & HPG_MSENS) == HPG_MSENS) {
            printk("%s, -------- dectect HPG_MSENS2 --------  \n", __func__);
            count = 0;
            return 1;
        }
    }
...
void HDMI_control_packet_auto_Send (BYTE CtrlPktAuto, BYTE CtrlPkt)
{
    ...
    if (CtrlPktAuto)
		/*bit 7 control , bit 0 general packet*/
        I2C_ByteWrite(X41_SEND_CPKT_AUTO, CtrlPkt|0x81);
    else
...
//
//
void HDMI_System_PD_A (void)
{
	I2C_ByteWrite (X00_SYSTEM_CONTROL, PowerMode_A);
	PowerMode = PowerMode_A;
    rdy_cnt = 0;
}
void HDMI_System_PD_E (void)
{
	I2C_ByteWrite (X00_SYSTEM_CONTROL, PowerMode_E);
	PowerMode = PowerMode_E;
    rdy_cnt = 0;
}
...
//
//
void HDMI_Set_Color_Coefficient (unsigned int format,unsigned char int_format,unsigned char out_format)
{
    int i;
    u32 ycc_mode = RGBTOYCC_SDTV_FULL_RANGE_ARRAY;
    unsigned char temp;
    unsigned char* p_val;
    ...
    if((int_format == FORMAT_RGB) &&
       (out_format == FORMAT_YCC422 || out_format == FORMAT_YCC444)) {
		...
		if(ycc_mode == RGBTOYCC_AUTO){
			switch (format) {
	        	case VID_16_1920x1080p:
	      		case VID_04_1280x720p:
					ycc_mode = RGBTOYCC_HDTV_60HZ_RANGE_ARRAY;
					break;
				case VID_02_720x480p:
	      		case VID_17_720x576p:	
	        		ycc_mode = RGBTOYCC_SDTV_FULL_RANGE_ARRAY;
	        		break;
	            case VID_60_1024x768_RGB_EXT:
	            case VID_61_1280x1024_RGB_EXT:
					ycc_mode = RGBTOYCC_SDTV_FULL_RANGE_ARRAY;
					printk("%s %d no support YCC mode\n",__func__,__LINE__);
					break;
	      		default:
	       			ycc_mode = RGBTOYCC_SDTV_FULL_RANGE_ARRAY;
        	...
        	if(ycc_mode >= RGBTOYCC_MAX_ENRTRY)
				...
				switch (ycc_mode) {
					case RGBTOYCC_SDTV_LIMIT_RANGE:
						ycc_mode = RGBTOYCC_SDTV_LIMIT_RANGE_ARRAY;
						break;
					case RGBTOYCC_SDTV_FULL_RANGE:	
						ycc_mode = RGBTOYCC_SDTV_FULL_RANGE_ARRAY;
						break;
					case RGBTOYCC_HDTV_60HZ_RANGE:	
						ycc_mode = RGBTOYCC_HDTV_60HZ_RANGE_ARRAY;
						break;
					case RGBTOYCC_HDTV_50HZ_RANGE:	
						ycc_mode = RGBTOYCC_HDTV_50HZ_RANGE_ARRAY;
						break;	
					default:
						ycc_mode = RGBTOYCC_SDTV_FULL_RANGE_ARRAY;
						break;
    ...
    printk("HDMI_Set_Color_Coefficient is %d-%d-%d-%d\n",format,int_format,out_format,ycc_mode);
}
...
void HDMI_Video_set_format (unsigned char format)
{
    ...
    /* IF auto , no cea-81 vid , will be RGB out */
	 if(Sli10121_Get_Output_Mode() == FROCE_OUT_MODE_AUTO){
	 	switch (format) {
        	case VID_16_1920x1080p:
      		case VID_04_1280x720p:
			case VID_02_720x480p:
      		case VID_17_720x576p:	
        		HDMI_setting.output_format = HDMI_setting.output_format;
        		break;
            case VID_60_1024x768_RGB_EXT:
            case VID_61_1280x1024_RGB_EXT:        
      		default:
	...
	HDMI_Set_Color_Coefficient(format,sli10121_get_lcd_output_mode(), HDMI_setting.output_format);
#else
    ...
    /* Harry */
    if (HDMI_setting.output_format == FORMAT_YCC422){
        I2C_ByteWrite (X64_PACKET_PB1, 0x20 | PB1_A0);       // YCC422, No Active Format, No Bar Info, NO DATA, A0=1
    }
	/*refer to ECA-81 to support AVI format*/
    else if(HDMI_setting.output_format == FORMAT_YCC444)
        I2C_ByteWrite (X64_PACKET_PB1, 0x40 | PB1_A0);
    else
        I2C_ByteWrite (X64_PACKET_PB1, 0x00 | PB1_A0);

    /* See EIA-CEA-861.pdf p67 */
    switch (format) {
      case VID_16_1920x1080p:
      case VID_04_1280x720p:
        I2C_ByteWrite (X65_PACKET_PB2, 0xA8);       // colorimetry 709, No pic aspect, 16:9
        break;
      case VID_02_720x480p:
      case VID_17_720x576p:
      case VID_60_1024x768_RGB_EXT:
      case VID_61_1280x1024_RGB_EXT:
        I2C_ByteWrite (X65_PACKET_PB2, 0x98);       // colorimetry 709, No pic aspect, 4:3
        break;
      default:
        I2C_ByteWrite (X65_PACKET_PB2, 0x88);       // colorimetry 709, No pic aspect, same as org pic
...
void HDMI_Reset_EDID_Retry_Counter(void)
{
...
{
    if(edid_ext==NULL){
        printk("edid_check_extblock_header NULL point\n");
        return -1;
    }
