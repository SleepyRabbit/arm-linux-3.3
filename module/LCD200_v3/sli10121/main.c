 ...
 *
 * VER1.2 2015/3/20
 * 1.remove resetting proceess
 * 2.Power e mode don't reset pll
 * 3.if read edid ext fail , retry 5 times.
 * VER1.3 2015/3/24
 * 1.if detect hotplug then go to powerB ,reset PLL and do state machine.
 *   Otherwise , only do detecting in idle state which turn to power b
 *   (no reset PLL) then go to power a for 100ms.
 * 2.if only detect hotplug then go to power a after enter mode b.
 * VER1.4 2015/3/24
 * fix retry 5 times can't stop after edid ext error.
 * VER1.5 2015/3/24
 * for icatch spec version
 * VER1.6 2015/4/7
 * Set PD A and odle state when user change resolution.
 * VER1.7 2015/6/25
 * 1)	Active Format Information Present: 
 * 2)	RGB or YCbCr: HD is base edid , no ce1-861 is rgb out
 * 3)	ColoriMetry:  ITU-709.
 */
...
///////////////////////////////////////////////////Macro///////////////////////////////////////////////////
#define SLI_VERSION             "sli ver:1.7"
/* state flag */
...
u32 g_force_yccmode = RGBTOYCC_AUTO;
...
module_param(scan_mode, int, S_IRUGO|S_IWUSR);
MODULE_PARM_DESC(scan_mode, "HDMI Scan mode");
...
static int __init sli10121_init(void)
{
    ...
    printk("sli10121 sleep %d ms ver=%s\n",sleep_ms,SLI_VERSION);
#ifdef CONFIG_PLATFORM_GM8210
...
static int sli10121_polling_thread(void *private)
{
    ...
    do {
        ...
        if (int_val != last_int_val) {
			...
			I2C_ByteWrite (X00_SYSTEM_CONTROL, PowerMode_B);

            DFh = I2C_ByteRead (XDF_HPG_STATUS);
            if (DFh & HOT_PLUG) {
        ...
        if (!get_hdmi_is_ready(hdmi_test)) {
            ...
            if (!HDMI_hotplug_check2()) {
                printk("HDMI hotplug remove detect\n");
                int_val = -1;
                last_int_val = 0;
                cable_on = 0;
                SET_STATE_FLAG(SLI10121_IS_CHANGE);
                //HDMI_System_PD (PowerMode_A);
                HDMI_Reset_EDID_Retry_Counter();
                HDMI_System_PD_A();
                HDMI_clear_to_IDLE();
                if (edid_callback)
...
#if 1/*remove sli , state to idle and PD A*/
    HDMI_Reset_EDID_Retry_Counter();
    HDMI_System_PD_A();
    HDMI_clear_to_IDLE();
#endif
...
static ssize_t force_outm_proc_write(struct file *file, const char __user *buf, size_t size, loff_t *off)
{
	...
	value[len] = '\0';
    sscanf(value, "%d\n", &data);
    if (data < RGBTOYCC_AUTO || data >= RGBTOYCC_MAX_ENRTRY) {
        printk("out of range! Please input 0~4 \n");
        return size;
    }
			  ...
			  int *eof, void *data)
{
	...
	len += sprintf(page+len, "0 is auto\n");
	len += sprintf(page+len, "1 is RGBTOYCC_SDTV_LIMITED_RANGE\n");
    len += sprintf(page+len, "2 is RGBTOYCC_SDTV_FULL_RANGE\n");
    len += sprintf(page+len, "3 is RGBTOYCC_HDTV_60HZ_RANGE\n");
    len += sprintf(page+len, "4 is RGBTOYCC_HDTV_50HZ_RANGE\n");
