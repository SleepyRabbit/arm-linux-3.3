...
#ifdef CONFIG_VIDEO_FASTTHROUGH
    lcdinfo->engine_minor = ENTITY_FD(0, 0, ENTITY_ENGINES, ENTITY_MINORS);
#else
...
#ifdef CONFIG_VIDEO_FASTTHROUGH
            video_entity_notify(&lcd_entity, ENTITY_FD(lcd_entity.type, 0, 0, 0), VG_NO_NEW_JOB, 1);
#else
...
#ifdef CONFIG_PLATFORM_GM8136
    if (1) {
        extern int tv_bot_shift_1pixel;
        gm2d_lcd_blit_t  blit;

        if (tv_bot_shift_1pixel) {
            switch (g_info.in_prop.src_fmt) {
              case TYPE_YUV422_FRAME:
              case TYPE_YUV422_RATIO_FRAME:
              case TYPE_YUV422_FRAME_2FRAMES:
              case TYPE_YUV422_FRAME_FRAME:
                if (output_type <= OUTPUT_TYPE_PAL_1) {
                    /* top */
                    memset(&blit, 0, sizeof(gm2d_lcd_blit_t));
                    blit.src_bg_paddr = job_node->frame_addr;
                    blit.src_bg_w = in_prop->src_bg_dim_width << 1;
                    blit.src_bg_h = in_prop->src_bg_dim_height >> 1;
                    blit.src_clp_x = 0;   //1 pixel yuv422
                    blit.src_clp_y = 0;
                    blit.src_clp_w = in_prop->src_dim_width;
                    blit.src_clp_h = in_prop->src_dim_height >> 1;

                    blit.dst_bg_paddr = job_node->frame_addr;
                    blit.dst_bg_w = in_prop->src_bg_dim_width << 1;
                    blit.dst_bg_h = in_prop->src_bg_dim_height >> 1;
                    blit.dst_off_x = 0;
                    blit.dst_off_y = 0;
                    blit.dst_off_w = in_prop->src_dim_width;    //just tell the width, the clip width is src_clp_w
                    blit.dst_off_h = in_prop->src_dim_height >> 1;
                    blit.dst_dx = 0;
                    blit.dst_dy = 0;
                    if (gm2d_lcd_do_blit(&blit, 0))
                            panic("%s, do blit fail! \n", __func__);
                    ...
                    blit.src_bg_w = in_prop->src_bg_dim_width << 1;
                    blit.src_bg_h = in_prop->src_bg_dim_height >> 1;
                    blit.src_clp_x = in_prop->src_dim_width+2;   //1 pixel yuv422
                    blit.src_clp_y = 0;
                    blit.src_clp_w = in_prop->src_dim_width - 2;
                    blit.src_clp_h = in_prop->src_dim_height >> 1;

                    blit.dst_bg_paddr = job_node->frame_addr;
                    blit.dst_bg_w = in_prop->src_bg_dim_width << 1;
                    blit.dst_bg_h = in_prop->src_bg_dim_height >> 1;
                    blit.dst_off_x = in_prop->src_dim_width;
                    blit.dst_off_y = 0;
                    blit.dst_off_w = in_prop->src_dim_width;    //just tell the width, the clip width is src_clp_w
                    blit.dst_off_h = in_prop->src_dim_height >> 1;
                    blit.dst_dx = 1;
...
#ifdef CONFIG_VIDEO_FASTTHROUGH
    print_property(job, job->in_prop.p);
#else
...
#ifdef CONFIG_VIDEO_FASTTHROUGH
    print_property(job, job->out_prop.p);
#else
...
{
#ifdef CONFIG_VIDEO_FASTTHROUGH
    video_entity_notify(&lcd_entity, ENTITY_FD(lcd_entity.type, 0, 0, 0), VG_HW_CONFIG_CHANGE, 1);
#else
