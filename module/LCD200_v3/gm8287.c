...
static int lcd_gateclk[LCD_MAX_ID] = {0};
...
unsigned int platform_pmu_calculate_clk(int lcd200_idx, u_long pixclk, unsigned int b_Use_Ext_Clk)
{
    unsigned int div, pll, i = 0, gate_clk;
    ...
    switch (lcd200_idx) {
      case LCD_ID:
        ...
        gate_clk = lcd_gateclk[lcd200_idx];
        if (gate_clk != 0)  /* before changing cntp, we must turn off the gate_clk first */
            platform_pmu_clock_gate(lcd200_idx, 0, 0);
        ...
        if (gate_clk != 0)
            platform_pmu_clock_gate(lcd200_idx, 1, 0);
        break;
