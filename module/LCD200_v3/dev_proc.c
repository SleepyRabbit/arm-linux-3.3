...
static struct proc_dir_entry *tv_bot_shift_proc;
int tv_bot_shift_1pixel = 0;
...
{
    if (lcd_disable == 1)
        seq_printf(sfile, "lcd off \n");
    else
        seq_printf(sfile, "lcd on \n");
...
#ifdef CONFIG_PLATFORM_GM8136
static int tv_bot_shift_show(struct seq_file *sfile, void *v)
{
    seq_printf(sfile, "0 for OFF, 1 for ON, shift one pixel \n");
    seq_printf(sfile, "current value = %d \n", tv_bot_shift_1pixel);

    return 0;
}

static int tv_bot_shift_proc_open(struct inode *inode, struct file *file)
{
    return single_open(file, tv_bot_shift_show, NULL);
}

static ssize_t tv_bot_shift_proc_write(struct file *file, const char __user * buf, size_t size, loff_t * off)
{
    char value[10];
    int len = size;
...
/*
 * TV bottom shift proc
 */
static struct file_operations tv_bot_shift_proc_ops = {
    .owner = THIS_MODULE,
    .open = tv_bot_shift_proc_open,
    .read = seq_read,
    .llseek = seq_lseek,
    .release = single_release,
    .write = tv_bot_shift_proc_write,
};
#endif /* CONFIG_PLATFORM_GM8136 */
...
#ifdef CONFIG_PLATFORM_GM8136
    /* TV bottom shift */
    if (tv_bot_shift_proc != NULL)
        ffb_remove_proc_entry(parent, tv_bot_shift_proc);
#endif
    return;
}
...
#ifdef CONFIG_PLATFORM_GM8136
    /*
     * tv bottom line shift one pixel
     */
    tv_bot_shift_proc = ffb_create_proc_entry("tv_bot_shift", S_IRUGO | S_IXUGO, parent);
    if (tv_bot_shift_proc == NULL) {
        err("Fail to create tv_bot_shift_proc!\n");
        ret = -EINVAL;
        goto fail;
    }
    tv_bot_shift_proc->proc_fops = &tv_bot_shift_proc_ops;
#endif /* CONFIG_PLATFORM_GM8136 */
