...
static VIN_RES_T res_support_list[LCD_IP_NUM][30] = {
    /* LCD0 */
#if 1
    {VIN_NTSC, VIN_PAL, VIN_320x240, VIN_NONE},
#else
...
int platform_pmu_exit(void)
{
    ...
    /* turn-off AXI clock for lcdc and gp-enc */
    ftpmu010_write_reg(lcd_fd, 0xB0, 0x1 << 15 | 0x1 << 12, 0x1 << 15 | 0x1 << 12);
    /* turn-off TV(reg) */
    ftpmu010_write_reg(lcd_fd, 0xB4, 0x1 << 20, 0x1 << 20);
    /* turn-off adda_wrapper */
    ftpmu010_write_reg(lcd_fd, 0xB8, 0x1 << 6, 0x1 << 6);
