                                   ...
                                   loff_t * off)
{
    ...
    sscanf(value, "%u\n", &v0);
    if (v0 < 3) {
        info->ops->reg_write((struct ffb_dev_info *)info, FEATURE, 0x0, 0x1);     //disable LCD controller
        udelay(10);
...
static int vg_dbg_show(struct seq_file *sfile, void *v)
{
    ...
    seq_printf(sfile, "lcd underrun: %d \n", underrun);
                               ...
                               loff_t * off)
{
    ...
    /* fix issue that someone changes input_res but read it back immediately, and cause inconsistent
     * information during internal updating
     */
    msleep(500);
