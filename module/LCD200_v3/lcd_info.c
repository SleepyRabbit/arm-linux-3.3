...
/*
 * Internal VGA DAC: 320x240@60HZ for SRGB(Serial RGB)
 */
struct ffb_timing_info VGA_320x240_timing[] = {
    {
     .otype = VOT_320x240,
     .data.lcd.pixclock = 27000,
     .data.lcd.out.xres = 960/3,
     .data.lcd.out.yres = 240,
     .data.lcd.hsync_len = 1,   //Hor Sync Time
     .data.lcd.vsync_len = 1,   //Ver Sync Time
     .data.lcd.left_margin = 69/3,      //HBack Porch + HLeft Border
     .data.lcd.right_margin = 684/3,    //HFront Porch + HRight Border
     .data.lcd.upper_margin = 20,       //VBack Porch + Top Border
     .data.lcd.lower_margin = 1,        //VFront Porch + Bottom Border
     .data.lcd.sp_param = 0x7,
     },
};

struct OUTPUT_INFO VGA_320x240_info = {
    .name = "VGA_320x240",
    .config = LCDCONFIG(LCD_PANEL_TYPE_8BIT, 0, 0, 0, 0, 1, 1), //(PanelType,RGBSW,IPWR,IDE,ICK,IHS,IVS)
    .config2 = FFBCONFIG2(0),
    .timing_num = ARRAY_SIZE(VGA_320x240_timing),
    .timing_array = VGA_320x240_timing,
};
