...
#ifdef CONFIG_VIDEO_FASTTHROUGH
        if (property[i].pch > virtual_chn_num) {
        ...
        if (property[i].id >= ID_START) {
            ...
            switch (property[i].id) {
                case ID_SRC_FMT:
                    ...
                    break;
                case ID_SRC_XY:
                        node->property.vproperty[ch].src_x = EM_PARAM_X(property[i].value);
                        node->property.vproperty[ch].src_y = EM_PARAM_Y(property[i].value);
                        node->property.vproperty[ch].vch_enable = 1;
                        node->ch_id = ch;
                        node->ch_cnt++;
                    break;
                case ID_SRC_BG_DIM:
                    ...
                    break;
                case ID_SRC_DIM:
                        node->property.vproperty[ch].src_width = EM_PARAM_WIDTH(property[i].value);
                        node->property.vproperty[ch].src_height = EM_PARAM_HEIGHT(property[i].value);
                    break;
                case ID_DI_RESULT:
                       node->property.di_result = property[i].value;
                    break;
                case ID_SRC_INTERLACE:
                       node->property.vproperty[ch].src_interlace = property[i].value;
                    break;
                case ID_DST_FMT:
                        node->property.dst_fmt = property[i].value;
                    break;
                case ID_DST_XY:
                        node->property.vproperty[ch].dst_x = EM_PARAM_X(property[i].value);
                        node->property.vproperty[ch].dst_y = EM_PARAM_Y(property[i].value);
                    break;
                case ID_DST_BG_DIM:
                    ...
                    break;
                case ID_DST_DIM:
                        node->property.vproperty[ch].dst_width = EM_PARAM_WIDTH(property[i].value);
                        node->property.vproperty[ch].dst_height = EM_PARAM_HEIGHT(property[i].value);
                    break;
                case ID_SCL_FEATURE:
                        node->property.vproperty[ch].scl_feature = property[i].value;
                ...
                case ID_AUTO_ASPECT_RATIO:
                        node->property.vproperty[ch].aspect_ratio.pal_idx = EM_PARAM_X(property[i].value);
                        node->property.vproperty[ch].aspect_ratio.enable = EM_PARAM_Y(property[i].value);
                    break;
                case ID_AUTO_BORDER:
                        node->property.vproperty[ch].border.pal_idx = (property[i].value & 0xffff0000) >> 16;
                        node->property.vproperty[ch].border.width = (property[i].value & 0xff00) >> 8;
                        node->property.vproperty[ch].border.enable = property[i].value & 0xff;
                    break;
                case ID_DST2_BG_DIM:
                ...
                case ID_SRC2_BG_DIM:
                    ...
                    break;
                case ID_DST2_XY:
                        node->property.vproperty[ch].dst2_x = EM_PARAM_WIDTH(property[i].value);
                        node->property.vproperty[ch].dst2_y = EM_PARAM_HEIGHT(property[i].value);
                    break;
                case ID_DST2_DIM:
                        node->property.vproperty[ch].dst2_width = EM_PARAM_WIDTH(property[i].value);
                        node->property.vproperty[ch].dst2_height = EM_PARAM_HEIGHT(property[i].value);
                    break;
                case ID_SRC2_XY:
                        node->property.vproperty[ch].src2_x = EM_PARAM_WIDTH(property[i].value);
                        node->property.vproperty[ch].src2_y = EM_PARAM_HEIGHT(property[i].value);
                    break;
                case ID_SRC2_DIM:
                        node->property.vproperty[ch].src2_width = EM_PARAM_WIDTH(property[i].value);
                        node->property.vproperty[ch].src2_height = EM_PARAM_HEIGHT(property[i].value);
                    break;
                case ID_WIN2_BG_DIM:
                ...
                case ID_CAS_SCL_RATIO:
                    ...
                    break;
                case ID_BUF_CLEAN:
                        node->property.vproperty[ch].buf_clean = property[i].value;
                    break;
                case ID_VG_SERIAL:
...
#ifdef CONFIG_VIDEO_FASTTHROUGH
    /* assign id */
    job->out_prop.p[0].id = ID_SCL_RESULT;
    job->out_prop.p[0].value = 0;

    /* 0:do nothing  1:source from top  2:source from bottom */
    if (job_item->status == JOB_STS_DONOTHING)
        job->out_prop.p[0].value = 0; /* do nothing */
    else {
        if (job_item->param.ratio == 0)
            job->out_prop.p[0].value = 1; /* source from top */
        if (job_item->param.ratio == 1)
            job->out_prop.p[0].value = 2; /* source from bottom */
    }

    /* terminate */
    job->out_prop.p[1].id = ID_NULL;
    job->out_prop.p[1].value = 0;
#else /* CONFIG_VIDEO_FASTTHROUGH */
    /* assign id */
...
{
#if GM8210
    static unsigned int cpu_comm_sn = 0;
...
static int vg_proc_property_show(struct seq_file *sfile, void *v)
{
    ...
    do {
#ifdef CONFIG_VIDEO_FASTTHROUGH
        ch=property_record[idx].property[i].pch;
#else
