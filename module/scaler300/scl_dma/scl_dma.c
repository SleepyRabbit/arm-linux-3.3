...
static inline int dsanity_check(scl_dma_t *scl_dma)
{
    ...
    if (pci_id == FMEM_PCI_HOST) {
        printk("%s, error role. Don't execute 2D in RC!\n", __func__);
...
#if 0
    if (scl_dma->job_id < 0) {
        printk("%s, error parameter. invalid job_id: %d \n", __func__, scl_dma->job_id);
        g_dma_info.put_fail_cnt ++;
        return -1;
    }
#endif
    ...
    if (!scl_dma->width || !scl_dma->height) {
        printk("%s, error parameter. width: %d, heigth: %d \n", __func__, scl_dma->width,
                                scl_dma->height);
        g_dma_info.put_fail_cnt ++;
        return -1;
    }
    ...
    if ((scl_dma->x_offset + scl_dma->width) > scl_dma->bg_width) {
            printk("%s, error parameter. x_offset: %d, width: %d, bg_width: %d \n", __func__,
                                scl_dma->x_offset,
    ...
    if ((scl_dma->y_offset + scl_dma->height) > scl_dma->bg_height) {
            printk("%s, error parameter. y_offset: %d, height: %d, bg_height: %d \n", __func__,
                                scl_dma->y_offset,
    ...
    if (!scl_dma->src_addr || !scl_dma->dst_addr) {
        printk("%s, error parameter. src_addr: 0x%x, dst_addr: 0x%x \n", __func__,
