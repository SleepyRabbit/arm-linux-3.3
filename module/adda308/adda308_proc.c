...
static int proc_read_ALCHOLT(struct seq_file *sfile, void *v)
{
	int i, val;
	...
	seq_printf(sfile, "Range is 0~15\n");
	seq_printf(sfile, "0:0us, 1:250us, 2:500us, ..., Time is doubled with every step.\n");
	seq_printf(sfile, "current ALCHOLT = %d(%d us)\n", ALCHOLT, val);
    return 0;
}

static ssize_t proc_write_ALCHOLT(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int len = count;
    unsigned char value[20] = {'\0'};
...
static int proc_read_ALCTL(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "Range is -4 ~ -34\n");
    seq_printf(sfile, "current ALCMAX = %d dbFS\n", ALCTL);

    return 0;
}

static ssize_t proc_write_ALCTL(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int len = count;
    unsigned char value[20] = {'\0'};
...
static int proc_read_ALCDCYT(struct seq_file *sfile, void *v)
{
	int i, val=104;
	...
	seq_printf(sfile, "Range is 0~15\n");
	seq_printf(sfile, "0:104us, 1:208us, 2:416us, ..., Time is doubled with every step.\n");
    seq_printf(sfile, "current ALCMAX = %d(%d us)\n", ALCDCYT, val);

    return 0;
}

static ssize_t proc_write_ALCDCYT(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int len = count;
    unsigned char value[20] = {'\0'};
...
static int proc_read_ALCATKT(struct seq_file *sfile, void *v)
{
	int i, val;
	...
	seq_printf(sfile, "Range is 0~15\n");
	seq_printf(sfile, "0:63.3us, 1:125us, 2:250us, ..., Time is doubled with every step.\n");
    seq_printf(sfile, "current ALCMAX = %d(%d us)\n", ALCATKT, val);

    return 0;
}

static ssize_t proc_write_ALCATKT(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int len = count;
    unsigned char value[20] = {'\0'};
...
{
    seq_printf(sfile, "LDAV should be between -50~30dB\n");
...
{
    seq_printf(sfile, "SPV should be between 0 ~ 3(0db ~ 1.5db)\n");
...
static int proc_read_HPF(struct seq_file *sfile, void *v)
{
    seq_printf(sfile, "current HPF = %d\n", HPF);
    return 0;
}

static ssize_t proc_write_HPF(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int len = count;
    unsigned char value[20] = {'\0'};
...
static int proc_read_HPFAPP(struct seq_file *sfile, void *v)
{
    seq_printf(sfile, "current HPFAPP = %d\n", HPFAPP);
    return 0;
}

static ssize_t proc_write_HPFAPP(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int len = count;
    unsigned char value[20] = {'\0'};
    ...
    adda308_set_HPFAPP(HPFAPP);

    return count;
}

static int proc_read_HPFCUT(struct seq_file *sfile, void *v)
{
    seq_printf(sfile, "current HPFCUT = %d\n", HPFCUT);
    return 0;
}

static ssize_t proc_write_HPFCUT(struct file *file, const char __user *buffer, size_t count, loff_t *ppos)
{
    int len = count;
    int val = 0;
    unsigned char value[20] = {'\0'};
    ...
    if (val > 0x7) {
        printk("HPFCUT should between 0~7!\n");
        return count;
    }

    HPFCUT = val;

    adda308_set_HPFCUT(HPFCUT);
...
static struct proc_dir_entry *ALCHOLT_d = NULL;
static struct proc_dir_entry *ALCTL_d = NULL;
static struct proc_dir_entry *ALCDCYT_d = NULL;
static struct proc_dir_entry *ALCATKT_d = NULL;
...
static struct proc_dir_entry *HPF_d = NULL;
static struct proc_dir_entry *HPFAPP_d = NULL;
static struct proc_dir_entry *HPFCUT_d = NULL;
...
static int proc_open_ALCHOLT(struct inode *inode, struct file *file)
{
    return single_open(file, proc_read_ALCHOLT, PDE(inode)->data);
}
static int proc_open_ALCTL(struct inode *inode, struct file *file)
{
    return single_open(file, proc_read_ALCTL, PDE(inode)->data);
}
static int proc_open_ALCDCYT(struct inode *inode, struct file *file)
{
    return single_open(file, proc_read_ALCDCYT, PDE(inode)->data);
}
static int proc_open_ALCATKT(struct inode *inode, struct file *file)
{
    return single_open(file, proc_read_ALCATKT, PDE(inode)->data);
}
...
static int proc_open_HPF(struct inode *inode, struct file *file)
{
    return single_open(file, proc_read_HPF, PDE(inode)->data);
}

static int proc_open_HPFAPP(struct inode *inode, struct file *file)
{
    return single_open(file, proc_read_HPFAPP, PDE(inode)->data);
}

static int proc_open_HPFCUT(struct inode *inode, struct file *file)
{
    return single_open(file, proc_read_HPFCUT, PDE(inode)->data);
}
...
static struct file_operations ALCHOLT_proc_ops = {
    .owner  = THIS_MODULE,
    .open   = proc_open_ALCHOLT,
    .write  = proc_write_ALCHOLT,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations ALCTL_proc_ops = {
    .owner  = THIS_MODULE,
    .open   = proc_open_ALCTL,
    .write  = proc_write_ALCTL,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations ALCDCYT_proc_ops = {
    .owner  = THIS_MODULE,
    .open   = proc_open_ALCDCYT,
    .write  = proc_write_ALCDCYT,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations ALCATKT_proc_ops = {
    .owner  = THIS_MODULE,
    .open   = proc_open_ALCATKT,
    .write  = proc_write_ALCATKT,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};
...
static struct file_operations HPF_proc_ops = {
    .owner  = THIS_MODULE,
    .open   = proc_open_HPF,
    .write  = proc_write_HPF,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations HPFAPP_proc_ops = {
    .owner  = THIS_MODULE,
    .open   = proc_open_HPFAPP,
    .write  = proc_write_HPFAPP,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};

static struct file_operations HPFCUT_proc_ops = {
    .owner  = THIS_MODULE,
    .open   = proc_open_HPFCUT,
    .write  = proc_write_HPFCUT,
    .read   = seq_read,
    .llseek = seq_lseek,
    .release= single_release,
};
...
int adda308_proc_init(void)
{
    ...
    ALCHOLT_d = create_proc_entry("ALCHOLT", S_IRUGO | S_IXUGO, entity_proc);
    if (ALCNGTH_d == NULL) {
        printk("%s fails: create ALCHOLT proc not OK", __func__);
        ret = -EINVAL;
        goto err18;
    }
    ALCHOLT_d->proc_fops  = &ALCHOLT_proc_ops;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    ALCHOLT_d->owner = THIS_MODULE;
#endif

    ALCTL_d = create_proc_entry("ALCTL", S_IRUGO | S_IXUGO, entity_proc);
    if (ALCTL_d == NULL) {
        printk("%s fails: create ALCTL proc not OK", __func__);
        ret = -EINVAL;
        goto err18;
    }
    ALCTL_d->proc_fops  = &ALCTL_proc_ops;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    ALCTL_d->owner = THIS_MODULE;
#endif

    ALCDCYT_d = create_proc_entry("ALCDCYT", S_IRUGO | S_IXUGO, entity_proc);
    if (ALCDCYT_d == NULL) {
        printk("%s fails: create ALCDCYT proc not OK", __func__);
        ret = -EINVAL;
        goto err18;
    }
    ALCDCYT_d->proc_fops  = &ALCDCYT_proc_ops;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    ALCDCYT_d->owner = THIS_MODULE;
#endif

    ALCATKT_d = create_proc_entry("ALCATKT", S_IRUGO | S_IXUGO, entity_proc);
    if (ALCATKT_d == NULL) {
        printk("%s fails: create ALCATKT proc not OK", __func__);
        ret = -EINVAL;
        goto err18;
    }
    ALCATKT_d->proc_fops  = &ALCATKT_proc_ops;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    ALCATKT_d->owner = THIS_MODULE;
#endif
    ...
    HPF_d = create_proc_entry("HPF", S_IRUGO | S_IXUGO, entity_proc);
    if (HPF_d == NULL) {
        printk("%s fails: create HPF not OK", __func__);
        ret = -EINVAL;
        goto err24;
    }
    HPF_d->proc_fops  = &HPF_proc_ops;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    HPF_d->owner = THIS_MODULE;
#endif

    HPFAPP_d = create_proc_entry("HPFAPP", S_IRUGO | S_IXUGO, entity_proc);
    if (HPFAPP_d == NULL) {
        printk("%s fails: create HPFAPP not OK", __func__);
        ret = -EINVAL;
        goto err25;
    }
    HPFAPP_d->proc_fops  = &HPFAPP_proc_ops;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    HPFAPP_d->owner = THIS_MODULE;
#endif

    HPFCUT_d = create_proc_entry("HPFCUT", S_IRUGO | S_IXUGO, entity_proc);
    if (HPFCUT_d == NULL) {
        printk("%s fails: create HPFCUT not OK", __func__);
        ret = -EINVAL;
        goto err26;
    }
    HPFCUT_d->proc_fops  = &HPFCUT_proc_ops;
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(2,6,30))
    HPFCUT_d->owner = THIS_MODULE;
#endif

    return 0;

err26:
    remove_proc_entry(HPFCUT_d->name, entity_proc);
err25:
    remove_proc_entry(HPFAPP_d->name, entity_proc);
err24:
    remove_proc_entry(HPF_d->name, entity_proc);
err23:
