...
/**
 * @file sen_ps5220.c
 * OmniVision ps5220 sensor driver
 *
...
static int check_sensor_reg_status(void)
{
    ...
    read_reg(0x1B, &temp_reg);//Normalized_DigDac1[10:8]
    read_reg(0x1C, &temp_reg2);//Normalized_DigDac1[7:0]
    write_reg(0xEF, 0x01);//bank1
    read_reg(0xE1, &control_enh);//control_enh 
    read_reg(0xE2, &control_sel);//control_sel    
    temp_reg_total=((temp_reg&0x7)<<8)|(temp_reg2&0xFF);
	
	if ((0x0020<temp_reg_total)||(0x0200>a_gain)) {
        write_reg(0xEF, 0x01);//bank1
        write_reg(0xE1, control_enh|0x10);
        write_reg(0xE2, control_sel|0x08);		
        write_reg(0x09, 0x01);				
	} else if ((0x001C>temp_reg_total)&&(0x02AB<a_gain)) {
        write_reg(0xEF, 0x01);//bank1
...
// external functions
//=============================================================================
static void ps5220_deconstruct(void)
{
...
static int ps5220_construct(u32 xclk, u16 width, u16 height)
{
...
//=============================================================================
static int __init ps5220_init(void)
{
    int ret = 0;
    ...
    if ((ret = ps5220_construct(sensor_xclk, sensor_w, sensor_h)) < 0)
        goto init_err2;
...
static void __exit ps5220_exit(void)
{
...
module_init(ps5220_init);
module_exit(ps5220_exit);

MODULE_AUTHOR("GM Technology Corp.");
MODULE_DESCRIPTION("Sensor PS5220");
MODULE_LICENSE("GPL");
