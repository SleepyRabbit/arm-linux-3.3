...
#define MIPI_2LANE_27MHZ    //crop from QXGA
...
static sensor_reg_t sensor_init_regs[] = {
    ...
    {0x3052, 0x00},   // disable i2c timeout
    {0x3005, 0x02},
    {0x3007, 0x00},
    {0x3009, 0x01},
    {0x300A, 0xF0},
...
static sensor_reg_t sensor_mipi_init_regs[] = {
    ...
    {0x3000, 0x00},   // operating
    {DELAY_CODE, 20},
    {0x3044, 0x62},//DCKRST=1
    {0x3044, 0x22},//DCKRST=0
    {0x3002, 0x00},   //Master mode start
};
...
static u32 get_pclk(u32 xclk)
{
    ...
    if (g_psensor->interface == IF_LVDS){
...
#ifdef MIPI_2LANE_27MHZ
        if (xclk != 27000000 && xclk != 54000000){
            isp_warn("xclk must be 27000000 or 54000000 for MIPI QXGA mode\n");    
            xclk = 27000000;
        }
        pclk = 117000000;
#else
        if (g_psensor->img_win.height <= 1080){
            if (xclk != 37125000){
                isp_warn("xclk must be 37125000 for MIPI 1080p mode\n");    
                xclk = 37125000;
            }
            pclk = 74250000;
        }
...
static int set_size(u32 width, u32 height)
{
    ...
    if (g_psensor->interface == IF_LVDS){
        ...
        if (height <= 1080){
            write_reg(0x3007, 0x10);
            write_reg(0x3018, 0x65);
            write_reg(0x3019, 0x04);
            write_reg(0x301b, 0x4c);
            write_reg(0x301c, 0x04);
            write_reg(0x3061, 0xa1);
            write_reg(0x3103, 0x64);
            write_reg(0x3104, 0x04);
        }
        else{
            write_reg(0x3007, 0x00);
            write_reg(0x3018, 0x40);
            write_reg(0x3019, 0x06);
            write_reg(0x301b, 0xee);
            write_reg(0x301c, 0x02);
            write_reg(0x3061, 0xb1);
            write_reg(0x3103, 0x3f);
            write_reg(0x3104, 0x06);
        }
        ...
        if (ch_num == 2){
#ifdef MIPI_2LANE_27MHZ
            write_reg(0x3044, 0x22); 
            write_reg(0x350b, 0x01); 
            write_reg(0x3524, 0x6f); 
            write_reg(0x3528, 0x57); 
            write_reg(0x352a, 0x2f); 
            write_reg(0x352c, 0x2f); 
            write_reg(0x352e, 0x2f); 
            write_reg(0x3530, 0xbf); 
            write_reg(0x3532, 0x0f); 
            write_reg(0x3536, 0x27); 
            write_reg(0x3580, 0x2f); 
#else
            if (width <= 1920 && height <= 1080){
                write_reg(0x3007, 0x10);
                write_reg(0x3018, 0x65);
                ...
                write_reg(0x301b, 0x98);
                write_reg(0x301c, 0x08);
                write_reg(0x3061, 0x01);
                write_reg(0x3103, 0x64);
                write_reg(0x3104, 0x04);
                write_reg(0x316c, 0x20);
                write_reg(0x3170, 0x61);
                write_reg(0x3171, 0x18);
                write_reg(0x350b, 0x01); 
                write_reg(0x3516, 0x20); 
                write_reg(0x3517, 0x25); 
                write_reg(0x3524, 0x57); 
                write_reg(0x3528, 0x37); 
                write_reg(0x352a, 0x1f); 
                write_reg(0x352c, 0x1f); 
                write_reg(0x352e, 0x1f); 
                write_reg(0x3530, 0x77); 
                write_reg(0x3532, 0x0f); 
                write_reg(0x3536, 0x17); 
                write_reg(0x3580, 0x1f); 
            }
            else{
                isp_err("2 lane only support MIPI 1080p\n");
                return -EINVAL;
            }
#endif
        }
        else{
            if (sensor_xclk != 27000000 && sensor_xclk != 54000000){
                isp_err("4 lane only support 27000000 or 54000000 xclk\n");
                return -EINVAL;
            }
            if (sensor_xclk == 54000000){
                write_reg(0x316d, 0x0a);
    ...
    if (g_psensor->interface == IF_LVDS){
        ...
        g_psensor->out_win.width = 2048;
        g_psensor->out_win.height = 1552;
        g_psensor->img_win.x = ((2048 - width) / 2) &~BIT0;
        g_psensor->img_win.y = 1 + (((1536 - height) / 2) &~BIT0);
#else
        if (ch_num == 2){
            g_psensor->out_win.width = 1936;
            g_psensor->out_win.height = 1108;
            g_psensor->img_win.x = ((1936 - width) / 2) &~BIT0;
            g_psensor->img_win.y = 1 + (((1108 - height) / 2) &~BIT0);
        }
        else{
            g_psensor->out_win.width = 2048;
            g_psensor->out_win.height = 1552;
    ...
    if (g_psensor->interface == IF_LVDS){
        ...
        switch (reg_l){
            case 0x2:
                isp_info("Sensor interface : LVDS, CH : 4\n");
                break;
            default:
        ...
        if ((reg_l & 0x03) == 0x01)
            isp_info("Sensor interface : MIPI, Lane : 2\n");
        else
            isp_info("Sensor interface : MIPI, Lane : 4\n");
    }
...
static int set_wdr_en(int enable)
{
    ...
    if (g_psensor->interface == IF_MIPI){
        isp_warn("Only LVDS interface support Build-in WDR mode\n");
        return 0;
    }
...
//        g_psensor->max_exp = 5000; // 0.5 sec
	...
	if (g_psensor->fps)
    	msleep(1000 / g_psensor->fps); //skip 1 frame
