...
static void adjust_blanking(int fps)
{
    ...
    if (g_psensor->img_win.height > 1080){
        ...
        if (fps > 120)
            fps = 120;
        if (fps > 60)
            pinfo->VMax = vmax*120/fps;
        else if (fps > 30)
            pinfo->VMax = vmax*60/fps;
        else
            pinfo->VMax = vmax*30/fps;
        printk("adjust_blanking %d, %d, %d\n", fps, vmax, pinfo->VMax);
    }
    ...
    if (g_psensor->img_win.width <= 1280 && g_psensor->img_win.height <= 720) {
        ...
        read_reg(0x3044, &reg2);
        if (g_psensor->fps > 60){
            g_psensor->fmt = RAW10;
            write_reg(0x3044, reg2 & (~0x01));    //10-bit
            write_reg(0x3005, 0x00);    //10-bit
            write_reg(0x3009, 0x00);    //120 fps
            write_reg(0x300a, 0x3c);
            write_reg(0x301b, 0x34);
            write_reg(0x301c, 0x03);
            pinfo->HMax = 820*2;
        }
        else if (g_psensor->fps > 30){
            g_psensor->fmt = RAW12;
            write_reg(0x3044, reg2 | 0x01);    //12-bit
            write_reg(0x3005, 0x01);    //12-bit
            write_reg(0x3009, 0x01);
            write_reg(0x300a, 0xf0);
            write_reg(0x301b, 0x70);
