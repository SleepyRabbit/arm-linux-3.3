...
#define DEFAULT_INTERFACE       IF_LVDS
...
static sensor_reg_t sensor_init_regs[] = {
    //LVDS 2ch, input clk 27MHz, 12bit, 60fps, All pixel mode
    {0x3000, 0x01},   // standby
    {DELAY_CODE, 500},
...
static int write_reg(u32 addr, u32 val)
{
    ...
    //printk("write_reg 0x%04x 0x%02x\n", addr, val);
...
static u32 get_pclk(u32 xclk)
{
    ...
    isp_info("IMX225 Sensor: pclk(%d) XCLK(%d)\n", pclk, xclk);
...
static void adjust_blanking(int fps)
{
    ...
    isp_info("Sensor HMAX=%d, VMAX=%d\n", pinfo->HMax ,pinfo->VMax);
...
static int set_size(u32 width, u32 height)
{
    ...
    if (g_psensor->interface == IF_PARALLEL){
        write_reg(0x301b, 0x94);
        write_reg(0x301c, 0x11);
        write_reg(0x3044, 0x01);
        
        write_reg(0x3046, 0x01);
        write_reg(0x3047, 0x0a);
        write_reg(0x3048, 0xc0);
    }    

    if (sensor_xclk == 54000000){
        write_reg(0x305d, 0x10);
        write_reg(0x305f, 0x10);
    }
    ...
    g_psensor->out_win.height = 993;
    g_psensor->img_win.x = 16 + (((SENSOR_MAX_WIDTH - width) / 2) & ~BIT0);
    g_psensor->img_win.y = 20 + (((SENSOR_MAX_HEIGHT - height) / 2) & ~BIT0);
...
// external functions
//=============================================================================
static void imx225_deconstruct(void)
{
...
static int imx225_construct(u32 xclk, u16 width, u16 height)
{
...
//=============================================================================
static int __init imx225_init(void)
{
    int ret = 0;
    ...
    if ((ret = imx225_construct(sensor_xclk, sensor_w, sensor_h)) < 0)
        goto init_err2;
...
static void __exit imx225_exit(void)
{
...
module_init(imx225_init);
module_exit(imx225_exit);

MODULE_AUTHOR("GM Technology Corp.");
MODULE_DESCRIPTION("Sensor IMX225");
MODULE_LICENSE("GPL");
