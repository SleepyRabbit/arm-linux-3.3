 ...
 *
 * $Revision: 1.8 $
 * $Date: 2015/03/19 11:20:18 $
 *
 * ChangeLog:
 *  $Log: sen_ar0331.c,v $
 *  Revision 1.8  2015/03/19 11:20:18  jtwang
 *  *** empty log message ***
 *
 *  Revision 1.34  2015/03/19 11:17:13  jtwang
 *  fix bug of wrong fps if switch size from 3M to 2M
 *
 *  Revision 1.33  2015/02/25 08:58:20  jtwang
 *  fix wdr mode flicker issue
 *
 *  Revision 1.32  2015/01/22 09:09:29  jtwang
 *  fix linearity issue
 *
...
static sensor_reg_t sensor_init_regs[] = {
    ...
    {0x3008, 0x0805}, 	// X_ADDR_END
    {0x300A, 0x0697},  // FRAME_LENGTH_LINES
    {0x300C, 0x044C},  // LINE_LENGTH_PCK
#endif
...
static sensor_reg_t sensor_init_regs_wdr[] = {
    {0x301A, 0x0001 }, // RESET_REGISTER
    {DELAY_CODE, 200},
    {0x301A, 0x10D8 }, // RESET_REGISTER
    {DELAY_CODE, 200},  
...
//    {0x301E, 0x0000}, 	// DATA_PEDESTAL
    ...
    {0x3200, 0x0002},  // ADACD_CONTROL
    // [HDR Mode Setup]
    {0x31D0, 0x0001},     // Enable data companding
    {0x3082, 0x0008},     // OPERATION_MODE_CTRL  //16x16
...
#define NUM_OF_INIT_REGS_WDR (sizeof(sensor_init_regs_wdr) / sizeof(sensor_reg_t))


static sensor_reg_t sensor_init_regs_hispi[] = {
    //[HISPI Linear Initialization]
    {0x301A, 0x0001}, 	// RESET_REGISTER
    {DELAY_CODE, 200},
    {0x301A, 0x0058}, 	// RESET_REGISTER
    //LOAD = Linear sequencer 1.0
...
//    {0x3ED2, 0x9F46}, 	// DAC_LD_6_7
    {0x3ED2, 0x9F06}, 	// DAC_LD_6_7
    
    //[HiSPi Linear Full -2048x1536 30fps]
    {0x301A, 0x0058}, 	// RESET_REGISTER
    {DELAY_CODE, 100},
    ...
    {0x3006, 0x0605}, 	// @Y_ADDR_END
    {0x3008, 0x0805}, 	// X_ADDR_END
//    {0x300A, 0x0610}, 	// FRAME_LENGTH_LINES
//    {0x300C, 0x063A}, 	// LINE_LENGTH_PCK
    {0x300A, 0x08A1}, 	// @FRAME_LENGTH_LINES 2209
    {0x300C, 0x0460},	// @LINE_LENGTH_PCK 1120
//    {0x3012, 0x060E}, 	// COARSE_INTEGRATION_TIME
    {0x30A2, 0x0001}, 	// X_ODD_INC
    {0x30A6, 0x0001}, 	// Y_ODD_INC
    {0x3040, 0x0000}, 	// READ_MODE
...
static sensor_reg_t sensor_init_regs_hispi_wdr[] = {
    //[HISPI HDR Initialization] 
    {0x301A, 0x0001}, 	// RESET_REGISTER
    {DELAY_CODE, 200},
    {0x301A, 0x0058}, 	// RESET_REGISTER
    ...
    {0x318A, 0x0E10}, 	// HDR_MC_CTRL1
    {0x3064, 0x1802}, 	// @SMIA_TEST
    {0x2440, 0x0040}, 	// What is this for?
    //
    {0x31AE, 0x0304}, 	// SERIAL_FORMAT
    {0x31C6, 0x0404},  
    {0x306E, 0x9210}, 	// DATAPATH_SELECT; HiVCM
...
//    {0x3ED2, 0x9F46}, 	// DAC_LD_6_7
    ...
    //[HISPI HDR Full -2048x1536 30fps]
    {0x301A, 0x0058}, 	// RESET_REGISTER
    {DELAY_CODE, 100},
    // LOAD= PLL_settings - 4 Lane 12-bit HiSPi
    {0x302A, 0x0006}, 	// VT_PIX_CLK_DIV
...
//    {0x300C, 0x05F9},	// @LINE_LENGTH_PCK
    {0x300A, 0x06F2}, 	// @FRAME_LENGTH_LINES 1778
    {0x300C, 0x0570},	// @LINE_LENGTH_PCK 1392
//    {0x3012, 0x0460},	// COARSE_INTEGRATION_TIME 
    {0x30A2, 0x0001}, 	// X_ODD_INC
    {0x30A6, 0x0001}, 	// Y_ODD_INC
    {0x3040, 0x0000}, 	// READ_MODE
    {0x301A, 0x005C}, 	// RESET_REGISTER
...
#define NUM_OF_INIT_REGS_HISPI_WDR (sizeof(sensor_init_regs_hispi_wdr) / sizeof(sensor_reg_t))
...
#define WDR_MAX_EXP_LINE    1120
#define WDR_MIN_EXP_LINE    8
...
static int set_exp(u32 exp)
{
    ...
    if (wdr){
        if (exp_line < WDR_MIN_EXP_LINE)
            exp_line = WDR_MIN_EXP_LINE;
        if (exp_line > WDR_MAX_EXP_LINE)
            exp_line = WDR_MAX_EXP_LINE;
    }
...
//    ang_x_1 = gain_table[i].gain_x;
    ...
    if(wdr){
        if(ang_reg_1 <= 6)
            write_reg(0x3198, 0x201E);
        else if(ang_reg_1 <= 7)
            write_reg(0x3198, 0x203A);
        else if(ang_reg_1 <= 8)
            write_reg(0x3198, 0x203B);
        else if(ang_reg_1 <= 9)
            write_reg(0x3198, 0x203D);
        else if(ang_reg_1 <= 10)
            write_reg(0x3198, 0x203E);
        else if(ang_reg_1 <= 11)
            write_reg(0x3198, 0x203F);
        else if(ang_reg_1 <= 12)
            write_reg(0x3198, 0x2041);
        else if(ang_reg_1 <= 13)
            write_reg(0x3198, 0x2043);
        else if(ang_reg_1 <= 14)
            write_reg(0x3198, 0x2045);
        else if(ang_reg_1 <= 15)
            write_reg(0x3198, 0x2046);
        else if(ang_reg_1 <= 16)
            write_reg(0x3198, 0x2049);
        else if(ang_reg_1 <= 18)
            write_reg(0x3198, 0x204B);
        else if(ang_reg_1 <= 20)
            write_reg(0x3198, 0x204E);
        else if(ang_reg_1 <= 22)
            write_reg(0x3198, 0x2051);
        else if(ang_reg_1 <= 24)
            write_reg(0x3198, 0x2054);
        else if(ang_reg_1 <= 26)
            write_reg(0x3198, 0x2058);
        else if(ang_reg_1 <= 28)
            write_reg(0x3198, 0x205c);
        else if(ang_reg_1 <= 30)
            write_reg(0x3198, 0x2061);
        else if(ang_reg_1 <= 32)
            write_reg(0x3198, 0x2067);
        else if(ang_reg_1 <= 36)
            write_reg(0x3198, 0x206E);
        else if(ang_reg_1 <= 40)
            write_reg(0x3198, 0x2077);
        else if(ang_reg_1 <= 44)
            write_reg(0x3198, 0x2082);
        else
            write_reg(0x3198, 0x2092);
    }
    //Important : Blooming artifact workaround

     if(ang_reg_1 < 16){//(gain_table[i].gain_x < 128)
        write_reg(0x3ED4, 0x8F3C);
        write_reg(0x3ED6, 0x3366);
...
static int cal_wdr_min_exp(void)
{
    ...
    int exp = MAX(1, pinfo->t_row * WDR_MIN_EXP_LINE / 1000);
    return exp;
}
...
static int set_wdr_en(int enable)
{
    ...
    if(enable){
        ...
        if (g_psensor->interface == IF_HiSPi){
            pInitTable=sensor_init_regs_hispi_wdr;
            InitTable_Num=NUM_OF_INIT_REGS_HISPI_WDR;
        }
        else{
            pInitTable=sensor_init_regs_wdr;
            InitTable_Num=NUM_OF_INIT_REGS_WDR;
        }
        ...
        set_size(g_psensor->img_win.width, g_psensor->img_win.height);
#ifdef WDR_EXP_LIMIT
