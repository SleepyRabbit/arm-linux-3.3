 ...
 *
 * $Revision: 1.5 $
 * $Date: 2015/05/21 09:41:12 $
 *
 * ChangeLog:
 *  $Log: sen_ov9714.c,v $
 *  Revision 1.5  2015/05/21 09:41:12  jason_ha
 *  Fixed 50Hz VTS issue
 *
 *  Revision 1.4  2015/03/03 03:55:24  jtwang
 *  *** empty log message ***
 *
 *  Revision 1.3  2015/02/24 06:57:33  jason_ha
 *  Fixed Set_exp bug
 *
...
static int set_exp(u32 exp)
{
    ...
    if (lines > max_exp_line) {
        ...
        write_reg(0x0340, (max_exp_line >> 8) & 0xFF);
        write_reg(0x0341, (max_exp_line & 0xFF));
	      isp_dbg("max_exp_line = %d\n",max_exp_line);        
    }
...
static int ov9714_construct(u32 xclk, u16 width, u16 height)
{
    ...
    g_psensor->max_gain = 2048;//gain_table[NUM_OF_GAINSET - 1].total_gain;
