...
static void adjust_blanking(int fps)
{
    ...
    if (fps > max_fps)
        fps = max_fps;
    pinfo->sw_lines_max = pinfo->lines_max*max_fps/fps - 8;
...
static int set_size(u32 width, u32 height)
{
    ...
    if (width <= 1920 && height <= 1080){
        ...
        write_reg(0x0302, 0x1b);
        write_reg(0x0303, 0x01);
        write_reg(0x3501, 0x45);
        write_reg(0x3502, 0x80);
        write_reg(0x3508, 0x07);
        write_reg(0x3509, 0x78);
        write_reg(0x3800, 0x01);
        write_reg(0x3801, 0x88);
        write_reg(0x3802, 0x00);
        write_reg(0x3803, 0xe0);
        write_reg(0x3804, 0x09);
        write_reg(0x3805, 0x17);
        write_reg(0x3806, 0x05);
        write_reg(0x3807, 0x1F);
        write_reg(0x3808, 0x07);
        write_reg(0x3809, 0x80);
        write_reg(0x380a, 0x04);
        write_reg(0x380b, 0x38);
        write_reg(0x380c, 0x07);
        write_reg(0x380d, 0x10);
        write_reg(0x380e, 0x04);
        write_reg(0x380f, 0x60);
        write_reg(0x4000, 0xf1);
        write_reg(0x4022, 0x06);
        write_reg(0x4024, 0x07);
        write_reg(0x4026, 0x07);
        write_reg(0x4601, 0x77);
        write_reg(0x4837, 0x2c);
    }
    else{
        g_psensor->out_win.width = SENSOR_MAX_WIDTH;
        g_psensor->out_win.height = SENSOR_MAX_HEIGHT;

        write_reg(0x0302, 0x14);
        write_reg(0x0303, 0x00);
        write_reg(0x3501, 0x60);
        write_reg(0x3502, 0x00);
        write_reg(0x3508, 0x00);
        write_reg(0x3509, 0x80);
        write_reg(0x3800, 0x00);
        write_reg(0x3801, 0x08);
        write_reg(0x3802, 0x00);
        write_reg(0x3803, 0x04);
        write_reg(0x3804, 0x0A);
        write_reg(0x3805, 0x97);
        write_reg(0x3806, 0x05);
        write_reg(0x3807, 0xFB);
        write_reg(0x3808, 0x0A);
        write_reg(0x3809, 0x80);
        write_reg(0x380A, 0x05);
        write_reg(0x380B, 0xF2);
        write_reg(0x380C, 0x0a);
        write_reg(0x380D, 0x2e);
        write_reg(0x380E, 0x06);
        write_reg(0x380F, 0x12);
        write_reg(0x4000, 0x71);
        write_reg(0x4022, 0x09);
        write_reg(0x4024, 0x0A);
        write_reg(0x4026, 0x0A);
        write_reg(0x4601, 0xA7);
        write_reg(0x4837, 0x19);
    }
    ...
    g_psensor->img_win.x = (((g_psensor->out_win.width - width) >> 1)) &~BIT0;
    g_psensor->img_win.y = (((g_psensor->out_win.height - height) >> 1)) &~BIT0;
