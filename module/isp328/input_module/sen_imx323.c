...
static sensor_reg_t sensor_init_regs[] = {
    ...
    {0x02, 0x3F, 0x0A},  // For IMX323 add new reg
    {0x02, 0x7a, 0x00},
    {0x02, 0x7b, 0x00},
    {0x02, 0x98, 0x26},
    {0x02, 0x99, 0x02},
...
static sensor_reg_t sensor_init_regs_i2c[] = {
    ...
    {0x30, 0x22, 0x00},
    {0x30, 0x27, 0x20},
    {0x30, 0x3F, 0x0A},// For IMX323 add new reg
    {0x30, 0x7A, 0x40},
    {0x30, 0x7B, 0x02},
    {0x30, 0x98, 0x26},
    {0x30, 0x99, 0x02},
...
static int imx323_read_reg(u8 id, u8 addr, u32 *pval)
{
...
static int imx323_write_reg(u8 id, u8 addr, u32 val)
{
    sensor_info_t *pinfo = (sensor_info_t *)g_psensor->private;
    int i;
    u32 tmp;
...
static int read_reg(u32 addr, u32 *pval)
{
    ...
    id = (addr >> 8) & 0xFF;
    r_addr = addr & 0xFF;
    return imx323_read_reg(id, r_addr, pval);
}
...
static int write_reg(u32 addr, u32 val)
{
    ...
    return imx323_write_reg(id, r_addr, val);
}
...
static u32 get_pclk(u32 xclk)
{
    ...
    if (sensor_spi)
        imx323_read_reg(0x2, 0x11, &reg);
    else
        imx323_read_reg(0x30, 0x11, &reg);
    reg = reg & 0x03;
...
{    
    ...
    if (sensor_spi)
        imx323_write_reg(0x2, 0x00, 0x31);// STANDBY on  
    else
        imx323_write_reg(0x01, 0x00, 0x00);// STANDBY on  
    _delay(); 
    ...
    if (sensor_spi){
        imx323_write_reg(0x2, 0x03, reg_l);// HMAX_L
        imx323_write_reg(0x2, 0x04, reg_h);// HMAX_H                                       
    }
    else{
        imx323_write_reg(0x03, 0x43, reg_l);// HMAX_L
        imx323_write_reg(0x03, 0x42, reg_h);// HMAX_H                                       
    }
    //imx323_write_reg(0x2, 0x11, 0x00); //FRSEL  
    //imx323_write_reg(0x2, 0x9A, 0x26);
    //imx323_write_reg(0x2, 0x9B, 0x02); 
    _delay();               
    if (sensor_spi)
        imx323_write_reg(0x2, 0x00, 0x30); // STANDBY off           
    else
        imx323_write_reg(0x01, 0x00, 0x01);// STANDBY off  
...
static int set_size(u32 width, u32 height)
{
    ...
    if (width <= 1280 && height <= 720){
        if (sensor_spi){
            imx323_write_reg(0x02, 0x04, 0x03);
            imx323_write_reg(0x02, 0x03, 0x39);
            imx323_write_reg(0x02, 0x06, 0x02);
            imx323_write_reg(0x02, 0x05, 0xee);

            imx323_write_reg(0x02, 0x02, 0x01);
            imx323_write_reg(0x02, 0x11, 0x00);
            imx323_write_reg(0x02, 0x12, 0x80); //SPI only
            imx323_write_reg(0x02, 0x16, 0xf1);
            imx323_write_reg(0x02, 0x20, 0x3c); //SPI only
            imx323_write_reg(0x02, 0x21, 0x00);
            imx323_write_reg(0x02, 0x22, 0x80);
            imx323_write_reg(0x02, 0x7a, 0x00);
            imx323_write_reg(0x02, 0x7b, 0x00);
            imx323_write_reg(0x02, 0xce, 0x00);
            imx323_write_reg(0x02, 0xcf, 0x00);
        }
        else{
            imx323_write_reg(0x03, 0x40, 0x02);
            imx323_write_reg(0x03, 0x41, 0xee);
            imx323_write_reg(0x03, 0x42, 0x03);
            imx323_write_reg(0x03, 0x43, 0x39);

            imx323_write_reg(0x30, 0x02, 0x01);
            imx323_write_reg(0x30, 0x11, 0x00);
            imx323_write_reg(0x30, 0x16, 0xf0);
            imx323_write_reg(0x30, 0x21, 0x00);
            imx323_write_reg(0x30, 0x22, 0x80);
            imx323_write_reg(0x30, 0x7a, 0x00);
            imx323_write_reg(0x30, 0x7b, 0x00);
            imx323_write_reg(0x30, 0xce, 0x00);
            imx323_write_reg(0x30, 0xcf, 0x00);
        }
        ...
        if (sensor_spi){
            imx323_write_reg(0x02, 0x04, 0x04);
            imx323_write_reg(0x02, 0x03, 0x4c);
            imx323_write_reg(0x02, 0x06, 0x04);
            imx323_write_reg(0x02, 0x05, 0x65);

            imx323_write_reg(0x02, 0x02, 0x0f);
            imx323_write_reg(0x02, 0x11, 0x00);
            imx323_write_reg(0x02, 0x12, 0x82);
            imx323_write_reg(0x02, 0x16, 0x3c);
            imx323_write_reg(0x02, 0x20, 0xf0);
            imx323_write_reg(0x02, 0x21, 0x00);
            imx323_write_reg(0x02, 0x22, 0x41);
            imx323_write_reg(0x02, 0x7a, 0x00);
            imx323_write_reg(0x02, 0x7b, 0x00);
            imx323_write_reg(0x02, 0xce, 0x16);
            imx323_write_reg(0x02, 0xcf, 0x82);
        }
        else{
            imx323_write_reg(0x03, 0x40, 0x04);
            imx323_write_reg(0x03, 0x41, 0x65);
            imx323_write_reg(0x03, 0x42, 0x04);
            imx323_write_reg(0x03, 0x43, 0x4c);

            imx323_write_reg(0x30, 0x02, 0x0f);
            imx323_write_reg(0x30, 0x11, 0x00);
            imx323_write_reg(0x30, 0x16, 0x3c);
            imx323_write_reg(0x30, 0x21, 0x80);
            imx323_write_reg(0x30, 0x22, 0x00);
            imx323_write_reg(0x30, 0x7a, 0x40);
            imx323_write_reg(0x30, 0x7b, 0x02);
            imx323_write_reg(0x30, 0xce, 0x16);
            imx323_write_reg(0x30, 0xcf, 0x82);
        }
    ...
    if (sensor_spi){
        imx323_read_reg(0x2, 0x03, &val_l);
        imx323_read_reg(0x2, 0x04, &val_h);
        pinfo->HMax = (val_h << 8) | val_l;
        imx323_read_reg(0x2, 0x05, &val_l);
        imx323_read_reg(0x2, 0x06, &val_h);
    }
    else{
        imx323_read_reg(0x03, 0x43, &val_l);
        imx323_read_reg(0x03, 0x42, &val_h);
        pinfo->HMax = (val_h << 8) | val_l;
        imx323_read_reg(0x03, 0x41, &val_l);
        imx323_read_reg(0x03, 0x40, &val_h);
    }
...
static u32 get_exp(void)
{
    ...
    if (!g_psensor->curr_exp) {
        if (sensor_spi){
            ...
            imx323_read_reg(0x2, 0x06, &val_h);
            vmax = (val_h << 8) | val_l;
            imx323_read_reg(0x2, 0x08, &val_l);
            imx323_read_reg(0x2, 0x09, &val_h);
        }
        else{
            imx323_read_reg(0x03, 0x41, &val_l);
            imx323_read_reg(0x03, 0x40, &val_h);
            vmax = (val_h << 8) | val_l;
            imx323_read_reg(0x02, 0x03, &val_l);
            imx323_read_reg(0x02, 0x02, &val_h);
        }
        shs1 = (val_h << 8) | val_l;
...
static int set_exp(u32 exp)
{
    ...
    if (lines > pinfo->VMax) {
        ...
        if (sensor_spi){
            imx323_write_reg(0x2, 0x03, (HMax & 0xFF));
            imx323_write_reg(0x2, 0x04, ((HMax >> 8) & 0xFF));
            pinfo->t_row = (HMax * 20000) / (g_psensor->pclk / 1000); 
            lines = ((exp * 1000)+pinfo->t_row/2) / pinfo->t_row; 
    	      imx323_write_reg(0x2, 0x05, (lines & 0xFF));
    	      imx323_write_reg(0x2, 0x06, ((lines >> 8) & 0xFF));
            imx323_write_reg(0x2, 0x08, 0);
            imx323_write_reg(0x2, 0x09, 0);
        }
        else{
            imx323_write_reg(0x03, 0x43, (HMax & 0xFF));
            imx323_write_reg(0x03, 0x42, ((HMax >> 8) & 0xFF));
            pinfo->t_row = (HMax * 20000) / (g_psensor->pclk / 1000); 
            lines = ((exp * 1000)+pinfo->t_row/2) / pinfo->t_row; 
    	      imx323_write_reg(0x03, 0x41, (lines & 0xFF));
    	      imx323_write_reg(0x03, 0x40, ((lines >> 8) & 0xFF));
            imx323_write_reg(0x02, 0x03, 0);
            imx323_write_reg(0x02, 0x02, 0);
        }
        ...
        if (sensor_spi){
            ...
            pinfo->t_row = (HMax * 20000) / (g_psensor->pclk / 1000); 
            lines = ((exp * 1000)+pinfo->t_row/2) / pinfo->t_row; 
    	      imx323_write_reg(0x2, 0x05, (pinfo->VMax & 0xFF));
    	      imx323_write_reg(0x2, 0x06, ((pinfo->VMax >> 8) & 0xFF));
            shs1 = pinfo->VMax - lines;
            imx323_write_reg(0x2, 0x08, (shs1 & 0xFF));
            imx323_write_reg(0x2, 0x09, ((shs1 >> 8) & 0xFF));
        }
        else{
            imx323_write_reg(0x03, 0x43, (HMax & 0xFF));
            imx323_write_reg(0x03, 0x42, ((HMax >> 8) & 0xFF));
            pinfo->t_row = (HMax * 20000) / (g_psensor->pclk / 1000); 
            lines = ((exp * 1000)+pinfo->t_row/2) / pinfo->t_row; 
    	      imx323_write_reg(0x03, 0x41, (pinfo->VMax & 0xFF));
    	      imx323_write_reg(0x03, 0x40, ((pinfo->VMax >> 8) & 0xFF));
            shs1 = pinfo->VMax - lines;
            imx323_write_reg(0x02, 0x03, (shs1 & 0xFF));
            imx323_write_reg(0x02, 0x02, ((shs1 >> 8) & 0xFF));        
        }
...
static u32 get_gain(void)
{
    ...
    if (g_psensor->curr_gain == 0) {        
        ...
        if (sensor_spi)
            imx323_read_reg(0x2, 0x1E, &ad_gain);
        else
            imx323_read_reg(0x30, 0x1E, &ad_gain);
...
static int set_gain(u32 gain)
{
    ...
    if (sensor_spi)
        imx323_write_reg(0x2, 0x1E, ad_gain);
    else
        imx323_write_reg(0x30, 0x1E, ad_gain);
...
static u32 get_a_gain(void)
{
    ...
    if (pinfo->curr_a_gain == 0) {        
        ...
        if (sensor_spi)
            imx323_read_reg(0x2, 0x1E, &a_gain);
        else
            imx323_read_reg(0x30, 0x1E, &a_gain);
...
static int set_a_gain(u32 gain)
{
    ...
    if (sensor_spi)
        imx323_write_reg(0x2, 0x1E, reg);
    else
        imx323_write_reg(0x30, 0x1E, reg);
...
static u32 get_d_gain(void)
{
    ...
    if (pinfo->curr_d_gain == 0) {        
        ...
        if (sensor_spi)
            imx323_read_reg(0x2, 0x1E, &d_gain);
        else
            imx323_read_reg(0x30, 0x1E, &d_gain);
...
static int get_mirror(void)
{
    ...
    if (sensor_spi){
        imx323_read_reg(0x2, 0x01, &reg);
        return (reg & BIT1) ? 1 : 0;
    }
    else{
        imx323_read_reg(0x01, 0x01, &reg);
        return (reg & BIT0) ? 1 : 0;
    }
}
...
static int set_mirror(int enable)
{
    ...
    if (sensor_spi){
        ...
        if (enable)
            ...
            reg &=~BIT0;
        imx323_write_reg(0x01, 0x01, reg);
    }        
...
static int init(void)
{
    ...
    if (sensor_spi) {
        for (i=0; i<NUM_OF_INIT_REGS; i++) {
            if (imx323_write_reg(sensor_init_regs[i].id, sensor_init_regs[i].addr, sensor_init_regs[i].val) < 0) {
        ...
        for (i=0; i<NUM_OF_INIT_REGS_I2C; i++) {
            ...
            else if (imx323_write_reg(sensor_init_regs_i2c[i].id, sensor_init_regs_i2c[i].addr, sensor_init_regs_i2c[i].val) < 0) {
                isp_err("%s init fail!!", SENSOR_NAME);            
                return -EINVAL;
            }
        }
...
// external functions
//=============================================================================
static void imx323_deconstruct(void)
{
...
static int imx323_construct(u32 xclk, u16 width, u16 height)
{
...
//=============================================================================
static int __init imx323_init(void)
{
    int ret = 0;
    ...
    if ((ret = imx323_construct(sensor_xclk, sensor_w, sensor_h)) < 0){
        printk("imx323_construct Error \n");
        goto init_err2;
        }
...
static void __exit imx323_exit(void)
{
...
module_init(imx323_init);
module_exit(imx323_exit);

MODULE_AUTHOR("GM Technology Corp.");
MODULE_DESCRIPTION("Sensor IMX323");
MODULE_LICENSE("GPL");
