...
static int auto_adjustment(void)
{
    ...
    while (1) {
        ...
        printf("19. Get DPC adjustment\n");
        printf("20. Set DPC adjustment\n");
        printf("99. Quit\n");
        printf("----------------------------------------\n");
        ...
        case 18:
            ...
            break;
        case 19:
            ioctl(isp_fd, ISP_IOC_GET_ADJUST_DPC_EN, &enable);
            if (enable)
                printf("DPC adjustment is enabled\n");
            else
                printf("BLC adjustment is disabled\n");
            break;
        case 20:
            do {
                printf("DPC adjustment (0:Disable, 1:Enable) >> ");
                enable = get_choose_int();
            } while (enable < 0 || enable > 1);
            ioctl(isp_fd, ISP_IOC_SET_ADJUST_DPC_EN, &enable);
            break;
        case 99:
            return 0;
        }
...
static int adjust_daynight_mode(void)
{
    int mode;
    int ret;

    ret = ioctl(isp_fd, ISP_IOC_GET_DAYNIGHT_MODE, &mode);
    if (ret < 0)
        return ret;
    printf("Current daynight mode = %s\n", (mode == 0) ? "DAY" : "NIGHT");

    do {
        printf("dynamic range mode (0:DAY, 1:NIGHT) >> ");
        mode = get_choose_int();
    } while (mode < 0);

    return ioctl(isp_fd, ISP_IOC_SET_DAYNIGHT_MODE, &mode);
}
...
static int get_sensor_max_gain(void)
{
    u32 value;
    int ret;

    ret = ioctl(isp_fd, AE_IOC_GET_MAX_SENSOR_GAIN, &value);
    if (ret < 0)
        return ret;
    printf("MAX Sensor Gain = %d\n", value);   
...
static int ae_adjust_min_gain(void)
{
    ...
    return ioctl(isp_fd, AE_IOC_SET_MIN_GAIN, &gain);
}
...
static int ae_get_status(void)
{
    u32 status;
    int ret;

    ret = ioctl(isp_fd, AE_IOC_GET_STATUS, &status);
    if (ret < 0)
        return ret;
    printf("Status => 2:Shooting, 4:Converge\n");
    printf("Current Status is %d\n", status);
    
    return 0;
}

static int ae_get_isp_max_gain(void)
{
    u32 value;
    int ret;

    ret = ioctl(isp_fd, AE_IOC_GET_MAX_ISP_GAIN, &value);
    if (ret < 0)
        return ret;

    printf("MAX ISP GAIN is %d\n", value);
    
    return 0;
}

static int ae_get_curren_gain(void)
{
    u32 value;
    int ret;

    ret = ioctl(isp_fd, AE_IOC_GET_CURRENT_GAIN, &value);
    if (ret < 0)
        return ret;
...
static int ae_get_total_gain(void)
{
    u32 ISP_gain, Sensor_gain, value;
    int ret;

    ret = ioctl(isp_fd, AE_IOC_GET_MAX_ISP_GAIN, &ISP_gain);
    if (ret < 0)
        return ret;

    ret = ioctl(isp_fd, AE_IOC_GET_MAX_SENSOR_GAIN, &Sensor_gain);
    if (ret < 0)
        return ret;

    value = (ISP_gain/128) * (Sensor_gain/64);

    printf("Total Gain is %d\n", value);
...
int gm_ae_ioctl_menu(void)
{
    ...
    while (1) {
        ...
        printf("13. Get status\n");
        printf("14. Get ISP Max Gain\n");
        printf("15. Get Current Gain\n");
        printf("16. Get Total Gain\n");
        printf("99. Quit\n");
        printf("----------------------------------------\n");
        do {
            printf(">> ");
...
static int awb_get_status(void)
{
    u32 status;
    int ret;

    ret = ioctl(isp_fd, AWB_IOC_GET_STATUS, &status);
    if (ret < 0)
        return ret;
    printf("Status => 2:Shooting, 3:Converge\n");
    printf("Current status is %d\n", status);
...
static int af_get_status(void)
{
    ...
    ret = ioctl(isp_fd, AF_IOC_GET_STATUS, &status);
    if (ret < 0)
        return ret;
    printf("Status => 3,4,5:Searching, 6:Converge\n");
    printf("Current status is %d\n", status);
...
static int adjust_vcap_yuv_range(void)
{
    int range;
    int ret;

    ret = ioctl(isp_fd, ISP_IOC_GET_VCAP_YUV_RANGE, &range);
    if (ret < 0)
        return ret;
    printf("YUV range is %d level!\n", (range == 0) ? 256 : 240);

    do {
        printf("YUV range (0:256_Level, 1:240_Level) >> ");
        range = get_choose_int();
    } while ((range < 0) || (range > 1));

    return ioctl(isp_fd, ISP_IOC_SET_VCAP_YUV_RANGE, &range);
}
