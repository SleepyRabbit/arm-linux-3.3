#ifndef _FE_TP2822_H_
#define _FE_TP2822_H_

#include <linux/ioctl.h>

/*************************************************************************************
 *  TP2822 Device Definition
 *************************************************************************************/
#define TP2822_DEV_MAX                 4
#define TP2822_DEV_CH_MAX              4

#define TP2822_DEV_ID               0x22    ///< TP2822

typedef enum {
    TP2822_DEV_VOUT0 = 0,              ///< VD1
    TP2822_DEV_VOUT1,                  ///< VD2
    TP2822_DEV_VOUT_MAX
} TP2822_DEV_VOUT_T;

typedef enum {
    TP2822_VOUT_FORMAT_BT656 = 0,
    TP2822_VOUT_FORMAT_BT1120,
    TP2822_VOUT_FORMAT_MAX
} TP2822_VOUT_FORMAT_T;

typedef enum {
    TP2822_VOUT_MODE_1CH_BYPASS = 0,            ///< ch#0 rising edge
    TP2822_VOUT_MODE_2CH_DUAL_EDGE,             ///< ch#0 rising edge, ch#1 falling edge, only tp2822
    TP2822_VOUT_MODE_MAX
} TP2822_VOUT_MODE_T;

typedef enum {
    TP2822_VFMT_720P60 = 0,
    TP2822_VFMT_720P50,
    TP2822_VFMT_1080P30,
    TP2822_VFMT_1080P25,
    TP2822_VFMT_720P30,
    TP2822_VFMT_720P25,
    TP2822_VFMT_HALF_1080P30,
    TP2822_VFMT_HALF_1080P25,
    TP2822_VFMT_HALF_720P30,
    TP2822_VFMT_HALF_720P25,
    TP2822_VFMT_960H30, ///< For hybrid design
    TP2822_VFMT_960H25, ///< For hybrid design
    TP2822_VFMT_MAX
} TP2822_VFMT_T;

struct tp2822_video_fmt_t {
    TP2822_VFMT_T fmt_idx;       ///< video format index
    unsigned int  width;         ///< video source width
    unsigned int  height;        ///< video source height
    unsigned int  prog;          ///< 0:interlace 1:progressive
    unsigned int  frame_rate;    ///< currect frame rate
};

/*************************************************************************************
 *   TP2822 IOCTL
 *************************************************************************************/
struct tp2822_ioc_data_t {
    int vin_ch;                 ///< access tp2822 vin channel number
    int data;                   ///< read/write data value
};

struct tp2822_ioc_vfmt_t {
    int          vin_ch;        ///< tp2822 vin channel number
    unsigned int width;         ///< tp2822 channel video width
    unsigned int height;        ///< tp2822 channel video height
    unsigned int prog;          ///< 0:interlace, 1: progressive
    unsigned int frame_rate;    ///< currect frame rate
};

#define  TP2822_IOC_MAGIC       't'

#define  TP2822_GET_NOVID       _IOR( TP2822_IOC_MAGIC,   1, struct tp2822_ioc_data_t)
#define  TP2822_GET_VIDEO_FMT   _IOR( TP2822_IOC_MAGIC,   2, struct tp2822_ioc_vfmt_t)

/*************************************************************************************
 *   TP2822 API Export Function Prototype
 *************************************************************************************/
#ifdef __KERNEL__
int  tp2822_get_dev_id(int id);
int  tp2822_get_revision(int id);
int  tp2822_get_dev_id(int id);
int  tp2822_get_device_num(void);
int  tp2822_get_vch_id(int id, TP2822_DEV_VOUT_T vout, int seq_id);
int  tp2822_get_vout_id(int id, int vin_idx);
int  tp2822_get_vout_seq_id(int id, int vin_idx);
int  tp2822_get_vout_format(int id);
int  tp2822_get_vout_mode(int id);
int  tp2822_vin_to_vch(int id, int vin_idx);

u8   tp2822_i2c_read(u8 id, u8 reg);
int  tp2822_i2c_read_ext(u8 id, u8 reg);
int  tp2822_i2c_write(u8 id, u8 reg, u8 data);
int  tp2822_i2c_array_write(u8 id, u8 addr, u8 *parray, u32 array_cnt);

int  tp2822_notify_vfmt_register(int id, int (*nt_func)(int, int, struct tp2822_video_fmt_t *));
void tp2822_notify_vfmt_deregister(int id);
int  tp2822_notify_vlos_register(int id, int (*nt_func)(int, int, int));
void tp2822_notify_vlos_deregister(int id);

int  tp2822_status_get_video_input_format(int id, int ch, struct tp2822_video_fmt_t *vfmt);
int  tp2822_status_get_video_loss(int id, int ch);

int  tp2822_video_init(int id, TP2822_VOUT_FORMAT_T vout_fmt, TP2822_VOUT_MODE_T vout_mode);
int  tp2822_video_get_video_output_format(int id, int ch, struct tp2822_video_fmt_t *vfmt);
int  tp2822_video_set_video_output_format(int id, int ch, TP2822_VFMT_T vfmt, TP2822_VOUT_FORMAT_T vout_fmt);

#endif

#endif  /* _FE_TP2822_H_ */
