...
int decoder_msg(char *str)
{
    int len = 0;
    if (NULL == str)
        return len;
    len += sprintf(str+len, "Decoder v%d.%d.%d", DECODER_VER_MAJOR, DECODER_VER_MINOR, DECODER_VER_MINOR2);
    if (DECODER_VER_BRANCH)
        len += sprintf(str+len, ".%d", DECODER_VER_BRANCH);
    len += sprintf(str+len, ", built @ %s %s\n", __DATE__, __TIME__);
    len += sprintf(str+len, " codec   en  register\n");
    len += sprintf(str+len, "=======  ==  ========\n");
    len += sprintf(str+len, " H264    %d      %d\n", en_h264, (NULL == decdev[TYPE_H264] ? 0: 1));
    len += sprintf(str+len, " MPEG4   %d      %d\n", en_mpeg4, (NULL == decdev[TYPE_MPEG4] ? 0: 1));
    len += sprintf(str+len, " JPEG    %d      %d\n", en_jpeg, (NULL == decdev[TYPE_JPEG] ? 0: 1));
...
static int proc_level_read_mode(char *page, char **start, off_t off, int count,int *eof, void *data)
{
    ...
    *eof = 1;
    return sprintf(page, "Log level =%d (1:emerge 1:error 2:debug)\n", log_level);
}
...
static int __init decoder_vg_init(void)
{
    ...
    info_proc = create_proc_entry("info", S_IRUGO | S_IXUGO, entity_proc);
    if (NULL == info_proc)
        return -EFAULT;
...
static void __exit decoder_vg_close(void)
{
    ...
    if(info_proc!=0)
        remove_proc_entry(info_proc->name, entity_proc);
    if(entity_proc!=0)
